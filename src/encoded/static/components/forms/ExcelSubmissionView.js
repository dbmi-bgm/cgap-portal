...
export default class ExcelSubmissionView extends React.PureComponent {
    constructor(props) {
        ...
        this.handleLoadedIngestionSubmission =
            this.handleLoadedIngestionSubmission.bind(this);
    ...
    handleComplete(e) {
        ...
        switch (ingestionType) {
            case 'metadata_bundle':
                navigate(
                    `/search/?type=Case&ingestion_ids=${uuid}&proband_case=true`
                );
                break;
            case 'genelist':
                navigate(genelist);
            ...
            case 'family_history':
                if (value === 'View Family Info') {
                    ...
                    navigate(
                        `/search/?type=Case&sample_processing.families.ingestion_ids=${uuid}&proband_case=true`
                    );
                }
                break;
            default:
    ...
    closeAlert(i) {
        const { localAlerts = [] } = this.state;
        const newAlertState = [];
        localAlerts.forEach((alert, j) => {
            if (i !== j) newAlertState.push(alert);
        });
        ...
        return (
            <div className="ingestion-submission-view">
                <div className="container">
                    ...
                    <LocalAlertsContainer
                        {...{ localAlerts }}
                        closeAlert={this.closeAlert}
                    />

                    <PanelSelectionMenu
                        {...{ panelIdx, panelsComplete, submissionItem }}
                    ...
                    <PanelOne
                        {...this.props}
                        {...this.state}
                        {...{ setIsSubmitting, userDetails }}
                        markCompleted={this.markCompleted}
                        onLoadUser={this.handleLoadedUser}
                        onSubmitIngestionSubmission={
                            this.handleLoadedIngestionSubmission
                        }
                        pushNewAlert={this.pushNewAlert}
            ...
            {localAlerts.map((alert, i) => {
                ...
                return (
                    <div
                        key={title}
                        className={
                            'alert alert-dismissable alert-' +
                            (style || 'danger') +
                            (noCloseButton === true ? ' no-close-button' : '')
                        }>
                        {noCloseButton !== true ? (
                            <button
                                type="button"
...
function PanelSelectionMenu(props) {
    ...
    const steps = ['Basic Information', 'Upload Files'];
    ...
    const renderedItems = steps.map(function (stepTitle, stepIdx) {
        ...
        return (
            <div
                data-for-panel={stepNum}
                onClick={
                    !disabled && !completed && onSelect ? onSelect : undefined
                }
                key={stepNum}
                className={cls}>
                <div className="row">
                    ...
                    <div className="col">
                        <span className="panel-title d-block small">
                            Step {stepNum}
                        </span>
                        <span>{stepTitle}</span>
                    </div>
    ...
    return <div className="panel-selection">{renderedItems}</div>;
}

class PanelOne extends React.PureComponent {
    static projectFromUser(user) {
        const { project_roles: { 0: { project = {} } = {} } = {} } = user || {};
        const initState = {
    ...
    constructor(props) {
        ...
        this.handleSelectSubmissionType =
            this.handleSelectSubmissionType.bind(this);

        const { href } = props;
        const { query: { submissionType = null } = {} } =
        ...
        this.unsetSelectingField = () => {
            this.setState({ selectingField: null });
        };
        this.setSelectingProject = () => {
            this.setState({ selectingField: 'project' });
        };
    ...
    componentDidUpdate(pastProps) {
        ...
        const {
            submissionItem: pastIngestionSubmissionItem = null,
            panelIdx: pastPanelIdx,
            user: pastUser,
        } = pastProps;
        ...
        if (submissionItem) {
            ...
            const valuesDiffer = this.memoized.checkIfChanged(
                submissionItem,
                projectID
            );
            if (
    ...
    loadUser() {
        ...
        ajax.load('/users/' + uuid + '/', (res) => {
            if (!(res && res['@id'])) {
                throw new Error(
                    "Couldn't fetch user info, make sure you're logged in."
                );
            }
            onLoadUser(res);
        });
    ...
    handleSelectProject(projectID) {
        ...
        for (let i = 0; i < project_roles.length; i++) {
            const {
                project: { '@id': atID = null, display_title = null } = {},
            } = project_roles[i];
            if (atID === projectID) {
    ...
    handleCreate(e) {
        ...
        const { user_institution: { '@id': institution = null } = {} } =
            user || {};
        ...
        const fb = (res) => {
            ...
            if (!res || Object.keys(res).length === 0) {
                pushNewAlert({
                    ...
                    message:
                        'Encountered unknown error, likely related to network connection. Please try again.',
                    style: 'danger',
                });
                return;
            }
            ...
            errorList.forEach(function (err, i) {
                let detail =
                    (err && (err.description || err.detail || err)) ||
                    'Unknown Error';
                if (err && err.name) {
                    detail += '. ' + err.name;
                }
        ...
        const ingestionTypeToSubmissionTypeMap = {
            Accessioning: 'metadata_bundle',
            'Gene List': 'genelist',
        ...
        const postData = {
            institution,
            project,
            processing_status: { state: 'created' },
            ingestion_type: submissionType
                ? ingestionTypeToSubmissionTypeMap[submissionType]
                : 'metadata_bundle',
        };
    ...
    render() {
        ...
        if (!user) {
            return (
                <div className="container text-center">
                    ...
                    <h5 className="d-inline text-300 align-middle">
                        Loading Information...
                    </h5>
                </div>
            );
        ...
        const valuesChanged =
            !submissionItem ||
            this.memoized.checkIfChanged(
                submissionItem,
                institutionID,
                projectID
            );
        const createDisabled =
            !valuesChanged || isCreating || !institutionID || !projectID;

        const projectList =
            PanelOne.projectRolesToListofProjects(project_roles);

        return (
            <form
                className={
                    'panel-form-container d-block' +
                    ...
                    <div className="row">
                        <div className="col-auto">
                            <DropdownButton
                                ...
                                <Dropdown.Item eventKey="Accessioning">
                                    Accessioning
                                </Dropdown.Item>
                                <Dropdown.Item eventKey="Family History">
                                    Family History
                                </Dropdown.Item>
                                <Dropdown.Item eventKey="Gene List">
                                    Gene List
                                </Dropdown.Item>
                            </DropdownButton>
                        </div>
    ...
    onAddedFile(response) {
        ...
        if (submission_uri) {
            message = (
                <React.Fragment>
                    <p className="mb-0">
                        <strong>Keep this window open</strong> for updates on
                        file processing status. Note: this may take a while.
                    </p>
                </React.Fragment>
            );
        }
    ...
    render() {
        ...
        if (statusIdx === 0) {
                ...
                <React.Fragment>
                    ...
                    <div className="mt-1">
                        ...
                        {ingestionType === 'genelist' ? (
                            <>
                                Click{' '}
                                <a
                                    href="/help/submission/gene-lists"
                                    target="_blank"
                                    rel="noreferrer">
                                    here
                                </a>{' '}
                                for more on how to format your genelist
                                submission document.
                            </>
                        ) : (
                            <>
                                Click{' '}
                                <a
                                    href="/help/submission/accessioning"
                                    target="_blank"
                                    rel="noreferrer">
                                    here
                                </a>{' '}
                                for more on how to format your accession
                                submission document.
                            </>
                        )}
                    </div>
                    ...
                    <div className="field-section mt-2">
                        ...
                        <AttachmentInputController
                            {...{ ingestionType, href, clearAllAlerts }}
                            context={submissionItem}
            ...
            panelContents = (
                <Poller
                    context={submissionItem}
                    setStatusIdx={this.setStatusIdx}
                    {...{
                        onLoadedIngestionSubmission,
                        setIsSubmitting,
                        pushNewAlert,
                ...
                <React.Fragment>
                    <div className="d-flex">
                        <div className="col">
                            ...
                            {ingestionType === 'genelist' ? (
                                <>
                                    <span className="text-300 text-large">
                                        Variants should begin updating shortly,
                                        but may take a few hours depending on
                                        server load.
                                    </span>
                                </>
                            ) : (
                                <span className="mb-0 text-small">
                                    To view full details of this Ingestion
                                    Submission, click{' '}
                                    <em>
                                        <a
                                            href={atID}
                            ...
                            ingestionType === 'family_history' ? (
                                <div className="text-small mt-05">
                                    <span
                                        className="mr-1 text-600"
                                        data-tip="Use this ID to upload fastq files in SubmitCGAP.">
                                        Ingestion Submission UUID:
                                    </span>{' '}
                                    <object.CopyWrapper
                            ...
                            ingestionType === 'genelist' ? (
                                ...
                                <>
                                    <button
                                        ...
                                        className="btn btn-success"
                                        onClick={handleComplete}
                                        value="View Family Info">
                                        View Family Info
                                    </button>
                                    <button
                                        type="button"
                                        className="btn btn-success ml-05"
                                        onClick={handleComplete}
                                        value="View Related Cases">
                                        View Related Cases
                                    </button>
                                </>
                            )}
        ...
        return <div className="panel-form-container">{panelContents}</div>;
    }
}
...
function Poller(props) {
    ...
    const { uuid } = context || {};
    const getURL =
        '/ingestion-submissions/' + uuid + '/?frame=object&datastore=database';

    const timeStarted = new Date(Date.now());
    const [lastUpdated, setLastUpdated] = useState(
        timeStarted.toLocaleTimeString('en-US')
    );
        ...
        ajax.promise(getURL, 'GET')
            ...
            .catch((error) => {
                ...
                if (typeof error === 'string') {
                    ...
                    console.error(error);
                    pushNewAlert({
                        title: 'An unknown error occurred. Consult an administrator or try again later.',
                        style: 'danger',
                    });
                }
    ...
    return (
        <React.Fragment>
            <div className="mt-2 text-center d-flex flex-column align-items-center justify-content-center">
                <span>
                    <i className="icon icon-sync icon-spin fas" />
                    &nbsp; Processing...
                </span>
...
function ExcelSubmissionFileAttachmentBtn(props) {
    ...
    switch (ingestionType) {
        ...
        case 'metadata_bundle':
            acceptedTypes =
                '.csv, .tsv, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet';
    ...
    const instructionsNode = (
        <div className="text-small font-italic ml-02 mb-1">
    ...
    return (
        <FileAttachmentBtn
            {...passProps}
            {...{ selectTitle, uploadTitle, instructionsNode, acceptedTypes }}
        />
    );
}
...
function FileAttachmentBtn(props) {
    ...
    const icon = loadingFileResult
        ? 'circle-notch fas icon-spin align-baseline'
        : 'upload fas';
            ...
            <React.Fragment>
                ...
                <div className="input-group">
                    <div className="input-group-prepend">
                        <span
                            className="input-group-text pr-5"
                    ...
                    <div className="input-group-append">
                        <label
                            htmlFor="test_file"
                            disabled={
                                loadingFileResult || postFileSuccess === true
                            }
                            className={
                                'mb-0 btn btn-primary ' +
                                (loadingFileResult || postFileSuccess
                            ...
                            <input
                                ...
                                type="file"
                                onChange={
                                    !loadingFileResult && onFileInputChange
                                        ? onFileInputChange
                                        : undefined
                                }
                                className="d-none"
    ...
    return (
        <React.Fragment>
            ...
            <div className="input-group">
                <div
                    className="input-group-prepend mw-50"
                    style={{ maxWidth: '50%' }}>
                    <div
                        ...
                        <label
                            htmlFor="test_file"
                            disabled={loadingFileResult || postFileSuccess}
                            className={
                                'mb-0 py-0 btn btn-link ' +
                                (loadingFileResult || postFileSuccess
                                    ? ' disabled unclickable'
                                    : ' clickable')
                            }>
                        ...
                        {onClearFile && (
                            <i
                                className={`${
                                    clearBtnDisabled ? '' : 'clickable'
                                } icon fas icon-times icon-fw mx-2`}
                                onClick={
                                    clearBtnDisabled ? undefined : onClearFile
                                }
                            />
                        )}
                    </div>
            ...
            {!loadingFileResult && postFileSuccess ? (
                <span className="ml-1 text-success">
                    Success! <i className="icon icon-check fas"></i>
                </span>
            ) : null}
            {!loadingFileResult && postFileSuccess === false ? (
                <span className="ml-1 text-danger">
                    Failure! <i className="icon icon-times-circle fas"></i>
                </span>
            ) : null}
        </React.Fragment>
    );
...
FileAttachmentBtn.propTypes = {
    ...
    file: PropTypes.shape({
        // File data for currently selected file, if available
...
const ProjectDrop = (props) => {
    ...
    const renderedOptions = options.map(function (project) {
        ...
        return (
            <DropdownItem
                ...
                <div className="row">
                    <div className="col">
                        <span className="text-600 d-block">
                            {display_title}
                        </span>
                    </div>
    ...
    const title =
        <span className="text-600">{selectedTitle}</span> || 'Select...';
    ...
    if (
        options.length === 1 &&
        options[0] &&
        selectedTitle === options[0].display_title
    ) {
        tooltip =
            'Only project available for current user is currently selected';
...
const ExcelSubmissionViewPageTitle = React.memo(
    function ExcelSubmissionViewPageTitle({
        context,
        href,
        schemas,
        currentAction,
        ...
        return (
            <PageTitleContainer alerts={alerts} className="container">
                <OnlyTitle>New IngestionSubmission</OnlyTitle>
            </PageTitleContainer>
        );
    }
);

pageTitleViews.register(
    ExcelSubmissionViewPageTitle,
    'IngestionSubmission',
    'create'
);
pageTitleViews.register(
    ExcelSubmissionViewPageTitle,
    'IngestionSubmissionSearchResults',
    'add'
);
