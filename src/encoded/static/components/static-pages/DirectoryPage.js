    ...
    return (
        <div className={"grid-item col-12 col-md-" + (childrenHaveChildren ? '6' : '12') + " col-lg-" + (childrenHaveChildren ? '4' : '12')} key={childID || name}>
            <a href={childID} className="inner">
                ...
                { childrenHaveChildren && childPageCount ? <h6 className="section-page-count mt-07 mb-05 text-400 text-end">{ childPageCount }&nbsp; <i className={"icon icon-fw far icon-file" + (childPageCount > 1 ? "s" : "")}/></h6> : null }
            </a>
        </div>
    );
}
