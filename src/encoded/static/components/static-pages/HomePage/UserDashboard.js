    ...
    return (
        <div className="recent-cases-table-section mb-36">
            <EmbeddedCaseSearchTable
                {...{ searchHref, maxHeight }}
                embeddedTableHeader={<AboveCasesTableOptions />}
                hideFacets={[
...
const AboveCasesTableOptions = React.memo(function AboveCasesTableOptions(
    props
) {
    const {
        context,
    ...
    const { onlyShowCasesWithReports, onlyShowProbandCases } = useMemo(
        function () {
            return {
                onlyShowCasesWithReports:
                    ctxFilters === null
                        ? true
                        : !!_.findWhere(ctxFilters, {
                              field: 'report.uuid!',
                              term: 'No value',
                          }),
                onlyShowProbandCases:
                    ctxFilters === null
                        ? true
                        : !!_.findWhere(ctxFilters, {
                              field: 'proband_case',
    ...
    const { onToggleOnlyShowCasesWithReports, onToggleOnlyShowProbandCases } =
        useMemo(function () {
            return {
                onToggleOnlyShowCasesWithReports: function () {
                    onFilter({ field: 'report.uuid!' }, { key: 'No value' });
                },
                onToggleOnlyShowProbandCases: function () {
            ...
            <div className="container-wide py-0 bg-white">
                <div className="tab-section-title flex-wrap">
                    ...
                    <DropdownButton
                        ...
                        title={
                            <React.Fragment>
                                <i className="icon fas icon-plus mr-1" />
                                Submit New...
                            </React.Fragment>
                        }>
            ...
            <div className="container-wide toggle-reports">
                <AboveTableControlsBaseCGAP
                    ...
                    <div className="col-12 col-lg-4 py-2">
                        <SearchBar
                            {...{ isContextLoading, context, navigate }}
                        />
                    </div>
                    <div className="d-none d-md-block col">&nbsp;</div>
                    <div className="col-12 col-md-auto">
                        <ProjectFilterCheckbox
                            isContextLoading={isContextLoading || !context}
                            onChange={onToggleOnlyShowCasesWithReports}
                            checked={onlyShowCasesWithReports}>
                    ...
                    <div className="col-12 col-md-auto pb-08 pb-md-0">
                        <ProjectFilterCheckbox
                            isContextLoading={isContextLoading || !context}
                            onChange={onToggleOnlyShowProbandCases}
                            checked={onlyShowProbandCases}>
...
class ProjectFilterCheckbox extends React.PureComponent {
    constructor(props) {
        super(props);
        this.onChange = _.throttle(this.onChange.bind(this), 500, {
            trailing: false,
        });
        ...
        return (
            <Checkbox
                ...
                <span className="text-small">
                    {isChanging ? (
                        <i className="icon icon-circle-notch icon-spin fas mr-07 text-small" />
                    ) : null}
                    {children}
                </span>
            </Checkbox>
...
function ProjectSelectDropdown(props) {
    ...
    const { projectFacet, projectFilter } = useMemo(
        function () {
            return {
                projectFacet:
                    _.findWhere(ctxFacets, {
                        field: 'project.display_title',
                    }) || null,
                projectFilter:
                    _.findWhere(ctxFilters, {
                        field: 'project.display_title',
    ...
    const onTermSelect = useCallback(
        function (evtKey, e) {
            ...
            if (!evtKey) {
                ...
                let updatedFilters =
                    searchFilters.contextFiltersToExpSetFilters(ctxFilters, {
                        type: true,
                    });
                // Unset existing first, then set new project filter, then perform virtual nav request.
                updatedFilters = searchFilters.changeFilter(
                    'project.display_title',
                    projectFilterTerm,
                    updatedFilters,
                    null,
                ...
                const updatedSearchHref = searchFilters.filtersToHref(
                    updatedFilters,
                    ctxHref,
                    null,
                    false,
    ...
    return (
        <DropdownButton
            disabled={isContextLoading || facetTerms.length === 0}
            title={projectFilterTerm || 'All Projects'}
