        ...
        if (updateData === null){
            return(
                <div className="text-center mt-5 mb-5" style={{ fontSize: '2rem', opacity: 0.5 }}>
                    <i className="mt-3 icon icon-spin icon-circle-notch far"/>
                </div>
            );
        ...
        return (
            <div className={"overview-blocks-header with-background mb-1" + (open ? ' is-open' : ' is-closed')} style={styleObj}>
                <h5 className="release-section-title clickable with-accent" onClick={this.toggle}>
                    <span><i className={"expand-icon icon icon-" + (open ? 'minus' : 'plus')} data-tip={open ? 'Collapse' : 'Expand'}/>{ updateData.summary } <i className={"icon fas icon-angle-right" + (this.state.open ? ' icon-rotate-90' : '')}/></span>
                </h5>
