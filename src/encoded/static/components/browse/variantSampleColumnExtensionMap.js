...
import {
    getInitialTranscriptIndex,
    getMostSevereConsequence,
} from '../item-pages/VariantSampleView/AnnotationSections';
...
export const variantSampleColumnExtensionMap = {
    ...
    DP: {
        ...
        render: function (result, props) {
            ...
            const rows = [
                ...
                <span key="AF" data-tip="VAF" className="d-block text-truncate">
                    {AF || '-'}
                </span>,
            ];
    ...
    'variant.genes.genes_most_severe_gene.display_title': {
        ...
        render: function (result, props) {
            ...
            const targetHref = link
                ? link
                : atID
                ? atID + '?annotationTab=0'
                : '#';
            ...
            return (
                <button
                    ...
                    data-child-window={resultUUID}
                    className="text-truncate btn btn-link p-0 w-100">
    ...
    'variant.genes.genes_most_severe_consequence.coding_effect': {
        // Coding Effect column
        widthMap: { lg: 140, md: 130, sm: 120 },
        render: function (result, props) {
            const { variant: { genes: [firstGene = null] = [] } = {} } = result;
            const {
                genes_most_severe_consequence: { coding_effect = null } = {},
            } = firstGene || {};
            if (firstGene && coding_effect) {
                return (
                    <StackedRowColumn className="text-center text-truncate">
                        {coding_effect}
                    </StackedRowColumn>
                );
            }
    ...
    'variant.csq_gnomadg_af': {
        ...
        render: function (result, props) {
            ...
            if (csq_gnomadg_af) {
                const csq_gnomadg_af_exp = csq_gnomadg_af
                    ? csq_gnomadg_af.toExponential(3)
                    : null;
                rows.push(
                    <div key="csq_gnomadg_af" className="d-block text-truncate">
                        <span className="text-600">ALL: </span>
                        {csq_gnomadg_af_exp || csq_gnomadg_af || '-'}
                    </div>
                );
            }
            if (csq_gnomadg_af_popmax) {
                const csq_gnomadg_af_popmax_exp = csq_gnomadg_af_popmax
                    ? csq_gnomadg_af_popmax.toExponential(3)
                    : null;
                rows.push(
                    <div
                        key="csq_gnomadg_af_popmax"
                        className="d-block text-truncate">
                        <span className="text-600">MAX: </span>
                        {csq_gnomadg_af_popmax_exp ||
    ...
    'variant.csq_cadd_phred': {
        // Predictors column (csq_cadd_phred, spliceai, phylop100)
        render: function (result, props) {
            const {
                variant: {
                    csq_cadd_phred = null,
                    spliceaiMaxds = null,
                    csq_phylop100way_vertebrate = null,
                } = {},
            } = result;
            const rows = [];
            if (
            ...
            if (csq_cadd_phred) {
                rows.push(
                    <div key="csq_cadd_phred" className="d-block text-truncate">
                        <span className="text-600">Cadd Phred: </span>
                        {csq_cadd_phred || '-'}
                    </div>
                );
            }
            if (spliceaiMaxds) {
                rows.push(
                    <div key="spliceaiMaxds" className="d-block text-truncate">
                        <span className="text-600">SpliceAI MaxDS: </span>
                        {spliceaiMaxds || '-'}
                    </div>
                );
            }
            if (csq_phylop100way_vertebrate) {
                rows.push(
                    <div key="phylop" className="d-block text-truncate">
                        <span className="text-600">PhyloP 100: </span>
                        {csq_phylop100way_vertebrate || '-'}
                    </div>
                );
            }
    ...
    'technical_review.assessment.call': {
        ...
        render: function (result, propsFromSearchTable) {
            return <TechnicalReviewColumn {...{ result }} />;
        },
    },
};
...
export const structuralVariantSampleColumnExtensionMap = {
            ...
            if (highlighted_genes.length === 0) {
                ...
                if (filteredTranscripts.length > 0) {
                    ...
                    const transcriptIndex =
                        getInitialTranscriptIndex(filteredTranscripts);
                    const { csq_consequence = [] } =
                        filteredTranscripts[transcriptIndex || 0];
                    worstConsequence =
                        getMostSevereConsequence(csq_consequence);
                }

                if (!worstConsequence) {
                    return null;
                }
                const { display_title: worstConsequenceDisplay } =
                    worstConsequence;
                return <div>{worstConsequenceDisplay}</div>;
            }
        },
    },
    // Gene, Transcript col
        ...
        render: (result, props) => {
            ...
            return (
                <button
                    ...
                    data-href={targetHref}
                    data-child-window={resultUUID}
                    className="text-truncate text-left btn btn-link p-0">
    ...
    'structural_variant.gnomadg_af': {
        render: function (result, props) {
            const {
                structural_variant: {
                    gnomadg_af = null,
                    unrelated_count = null,
                } = {},
            } = result || {};
            const { align = 'left' } = props;

            const rows = [
                <div className="d-block text-truncate" key="gnomadAF">
                    <span className="text-600">gnomAD: </span>
                    {gnomadg_af !== null ? gnomadg_af : '-'}
                </div>,
                <div className="d-block text-truncate" key="internal">
                    <span className="text-600">Internal: </span>
                    {unrelated_count !== null ? unrelated_count : '-'}
                </div>,
            ];
            return (
                <StackedRowColumn
                    {...{ rows }}
                    className={'text-truncate text-' + align}
                />
            );
        },
    },
    'structural_variant.size': {
        render: function (result, props) {
            const { structural_variant: { size_display = null } = {} } =
                result || {};
            return size_display;
        },
    },
...
export const VariantSampleDisplayTitleColumn = React.memo(
    function VariantSampleDisplayTitleColumn(props) {
        ...
        if (variantID) {
            rows.push(
                <span key={1} className="font-italic">
                    {variantID}
                </span>
            );
        }
...
export const VariantSampleDisplayTitleColumnSV = React.memo(
    function VariantSampleDisplayTitleColumn(props) {
        const { result = null, link, onClick, className = null } = props;
        const {
            '@id': atID,
            uuid: resultUUID,
            structural_variant: { display_title = '', annotation_id = '' } = {},
        } = result || {};

        const cls = 'title-block' + (className ? ' ' + className : '');

        // display title or annotationID structured like <type>_chr...etc; need just the part after underscore
        const [, splitAnnotationIDSuffix] = (
            display_title || annotation_id
        ).split('_');

        const rows = [
            <span key={0} className="d-block text-600 text-truncate">
                {splitAnnotationIDSuffix}
            </span>,
        ];
...
export const GenesMostSevereDisplayTitle = React.memo(
    function GenesMostSevereDisplayTitle({
        result,
        align = 'center',
        showTips = true,
        truncate = true,
        ...
        const geneTitles = genes.map(function (geneItem) {
            const { genes_most_severe_gene: { display_title = null } = {} } =
                geneItem || {};
        ...
        return (
            <StackedRowColumn
                className={
                    'text-' + align + (truncate ? ' text-truncate' : '')
                }>
                <span>{title}</span>
                <br />
                <em
                    data-tip={showTips ? 'Most Severe Transcript' : null}
                    className="font-italic text-small">
...
export const GenesMostSevereHGVSCColumn = React.memo(
    function GenesMostSevereHGVSCColumn({
        gene,
        align = 'center',
        truncate = true,
    }) {
        ...
        if (genes_most_severe_hgvsc) {
            ...
            rows.push(
                <div className={truncate ? 'text-truncate' : null} key="sc">
                    <span className="text-600">{scSplit[0]}.</span>
                    <span>{scSplit[1]}</span>
                </div>
            );
        }
        ...
        if (genes_most_severe_hgvsp) {
            ...
            rows.push(
                <div className={truncate ? 'text-truncate' : null} key="sp">
                    <span className="text-600">{spSplit[0]}.</span>
                    <span>{spSplit[1]}</span>
                </div>
            );
        }
...
export const StructuralVariantTranscriptColumn = React.memo(
    function StructuralVariantTranscriptColumn({ result, align = 'center' }) {
        const {
            highlighted_genes = [],
            structural_variant: { transcript: transcripts = [] } = {},
        } = result || {};
        ...
        if (genes.length <= 2) {
            ...
            rows.push(
                <div key={0}>
                    <span className="text-muted">List:&nbsp;</span>
                    <span data-tip={tipGenes}>{`${genes[0]}...${
        ...
        if (highlighted_genes.length === 0) {
            rows.push(
                <div className="text-muted" key={1}>
                    <i
                        className="icon icon-star fas"
                        data-tip="Highlighted gene"
                    />
                    :&nbsp;
            ...
            // TODO: styles may need to be adjusted to accomodate more than a few at a time
            rows.push(
                <div key={1}>
                    <i
                        className="icon icon-star fas text-primary"
                        data-tip="Highlighted gene"
                    />
                    :&nbsp;
                    {highlighted_genes.map((gene) => {
                        ...
                        return (
                            <span key="atID" className="ml-02">
                                <a href={atID}>{display_title}</a>
                            </span>
                        );
                    })}
                </div>
...
const StructuralVariantTranscriptCSQGeneColumn = React.memo(function ({
    result,
}) {
    const {
        '@id': atID,
    ...
    return (
        <a href={path} target="_blank" rel="noreferrer" data-tip={tipGenes}>{`${
            genes[0]
        }...${genes[genes.length - 1]}`}</a>
    );
});

export const ProbandGenotypeLabelColumn = React.memo(
    function ProbandGenotypeLabelColumn(props) {
        const {
            result,
            align = 'left',
            showTips = true,
        ...
        const rows = [
            ...
            <div key={0} className={rowCls}>
                {showIcon ? (
                    <i
                        className="text-small icon icon-user icon-fw fas mr-04"
                        data-tip={showTips ? 'Proband Genotype' : null}
                    />
                ) : null}
        ...
        if (mother_genotype_label) {
            ...
            rows.push(
                <div key={1} className={rowCls}>
                    {showIcon ? (
                        <i
                            className="text-small icon icon-circle icon-fw far mr-04"
                            data-tip={showTips ? 'Mother Genotype' : null}
                        />
                    ) : null}
        ...
        if (father_genotype_label) {
            ...
            rows.push(
                <div key={2} className={rowCls}>
                    {showIcon ? (
                        <i
                            className="text-small icon icon-square icon-fw far mr-04"
                            data-tip={showTips ? 'Father Genotype' : null}
                        />
                    ) : null}
...
export const BAMSnapshotColumn = React.memo(function BAMSnapshotColumn({
    result,
}) {
    ...
    return (
        <div className="mx-auto text-truncate">
            <button
                ...
                onClick={onClickLinkNavigateChildWindow}
                href={resultAtID + '@@download/'}
                data-child-window={'bam-' + resultUUID}
                data-child-window-message="false"
                data-html
                data-tip="View BAM Snapshot <i class='ml-07 icon-sm icon fas icon-external-link-alt'></i>">
    ...
    return (
        <div className="mx-auto text-truncate">
            <button
                ...
                className="btn btn-outline-dark btn-sm"
                onClick={onClickLinkNavigateChildWindow}
                href={resultAtID + '?annotationTab=3'}
                data-child-window={resultUUID}
                data-tip="View SV/CNV Browser">
                <i className="icon icon-fw icon-image fas" />
            </button>
        </div>
    );
