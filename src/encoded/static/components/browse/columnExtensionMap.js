...
} from './variantSampleColumnExtensionMap';
import QuickPopover from '../item-pages/components/QuickPopover';
import {
    generateRelationshipMapping,
    QCMFlag,
    sortAndAddRolePropsToQCMs,
} from '../item-pages/CaseView';
import {
    CurrentFamilyController,
    findCanonicalFamilyIndex,
} from '../item-pages/CaseView/CurrentFamilyController';
 ...
 */
const MultiLevelColumn = React.memo(
    function MultiLevelColumn(props) {
        const {
            topLeft,
            status,
            statusTip = null,
        ...
        return (
            <div className="multi-field-cell">
                <div className="top-row">
                    ...
                    <i
                        className="status-indicator-dot ml-07"
                        data-status={status}
                        data-tip={
                            statusTip || Schemas.Term.toName('status', status)
                        }
                        data-html
                    />
                </div>
...
function findSelectedCaseSample(allSamples, selectedIndividual) {
    ...
    for (let i = 0; i < samplesLen; i++) {
        const { individual: { '@id': sampleIndividualID = 'N/A' } = {} } =
            allSamples[i];
...
export const DisplayTitleColumnCase = React.memo(
    function DisplayTitleCaseDefault({ result }) {
        const title = itemUtil.getTitleStringFromContext(result); // Gets display_title || title || accession || ...
        const titleTip =
            (typeof title === 'string' && title.length > 20 && title) || null;
        const {
            '@id': caseHref,
            display_title = null,
            accession = null,
        ...
        if (indvID && familyID && spID) {
            status = 'complete';
            statusTip =
                'Case, Individual, Family and Sample Processing are created with required fields.';
        } else {
            status = 'incomplete';
            statusTip =
                'Case exists, but some linked item is missing: Family, Individual, or Sample Processing.';
        }

        return (
            <MultiLevelColumn
                {...{ date, status, statusTip, titleTip }}
                titleTipDelayShow={750}
                dateTitle="Accession Date:"
                topLeft={
                    <span className="accession text-muted">{accession}</span>
                }
                mainTitle={
...
export const columnExtensionMap = {
    ...
    display_title: {
        ...
        render: function renderDisplayTitleColumn(result, parentProps) {
            const { href, context, rowNumber, detailOpen, toggleDetailOpen } =
                parentProps;
            ...
            if (itemTypeList[0] === 'User') {
                ...
                renderElem = (
                    <VariantSampleDisplayTitleColumn {...{ result }} />
                );
            } else {
    ...
    report: {
        ...
        render: function renderReportColumn(result, parentProps) {
            const { '@id': resultHref, report = null } = result;
            const {
            ...
            return (
                <MultiLevelColumn
                    ...
                    statusTip="Not Implemented"
                    dateTitle="Last Modified:"
                    date={date_modified}
                    topLeft={
                        showAccessionSeparately ? (
                            <span className="accession text-muted">
                                {accession}
                            </span>
                        ) : null
                    }
    ...
    family: {
        render: function renderFamilyColumn(result, parentProps) {
            ...
            if (uuid && family_id) {
                ...
                status = 'incomplete';
                statusTip =
                    'Family exists, but some field is missing: uuid or family_id.';
            }

            return (
                <MultiLevelColumn
                    {...{ date, status, statusTip }}
                    dateTitle="Last Modified:"
                    topLeft={
                        <span className="accession text-muted">
                            {accession}
    ...
    individual: {
        ...
        render: function renderIndividualColumn(result, parentProps) {
            ...
            const individualTitle = individual_id
                ? individual_id
                : display_title;
            ...
            if (uuid && individual_id) {
                ...
                status = 'incomplete';
                statusTip =
                    'Individual exists, but some field is missing: uuid or individual_id.';
            }
    ...
    sample: {
        render: function renderSequencingColumn(result, parentProps) {
            ...
            return (
                <MultiLevelColumn
                    {...{ date, status, statusTip }}
                    dateTitle="Sequence Date:"
                    mainTitle={
                        <a
                            href={resultHrefPath + '#case-info.bioinformatics'}
                            className="adv-block-link">
                            {workup_type}
                        </a>
                    }
                />
    ...
    'sample.specimen_type': {
        render: function renderSampleColumn(result, parentProps) {
            ...
            if (uuid && bam_sample_id) {
                ...
                status = 'incomplete';
                statusTip =
                    'Sample exists, but some field is missing: uuid or bam_sample_id.';
            }
            return (
                <MultiLevelColumn
                    {...{ date, status, statusTip }}
                    dateTitle="Collection Date:"
                    topLeft={
                        <span className="accession text-muted">
                            {accession}
                        </span>
        ...
        render: function (result, props) {
            ...
            return (
                <span className="value">
                    {formatPublicationDate(result.date_published)}
                </span>
            );
        },
        order: 504,
    ...
    'google_analytics.for_date': {
        ...
        render: function googleAnalyticsDate(result, props) {
            if (!result.google_analytics || !result.google_analytics.for_date)
                return null;
            return (
                <LocalizedTime
                    timestamp={result.google_analytics.for_date}
                    formatType="date-sm"
                    localize={false}
                />
            );
    ...
    status: {
        ...
        render: function statusIndicator(result, props) {
            const statusFormatted = Schemas.Term.toName(
                'status',
                result.status
            );
            return (
...
const BioinformaticsMultiLevelColumn = React.memo(
    function BioinformaticsMultiLevelColumn({ result }) {
        const {
            sample,
            sample_processing,
            vcf_file,
        ...
        if (filesLen > 0 && status === null) {
            status = 'running';
            const { file_ingestion_status: ingestionStatus = null } =
                vcf_file || {};
...
const QCMultilevelColumn = React.memo(function QCMultilevelColumn({ result }) {
    ...
    const familiesWithViewPermission =
        CurrentFamilyController.filterFamiliesWithViewPermission(spFamilies);
    const canonicalFamilyIdx = findCanonicalFamilyIndex(
        familiesWithViewPermission,
        canonicalFamilyPartialEmbed
    );

    const { relationships = [] } = spFamilies[canonicalFamilyIdx] || {};
            ...
            <div>
                <a
                    ...
                    <span className="mr-05">
                        {fail}{' '}
                        <i className={`icon icon-flag fas text-danger ml-05`} />
                    </span>
                    <span className="ml-05">
                        {warn}{' '}
                        <i
                            className={`icon icon-flag fas text-warning ml-05`}
                        />
                    </span>
                </a>
                <QuickPopover
                    className="ml-05 mb-03 p-0"
                    tooltip="Click for QC Summary">
                    <QCPopover
                        {...{ qualityControlMetrics, relationshipMapping }}
                    />
                </QuickPopover>
            </div>
        );
    ...
    const bottom =
        completed_qcs.length !== 0 ? (
            <div>
                <span className="text-600">{completed_qcs.join(', ')}</span>{' '}
                QC(s) Completed
            </div>
        ) : (
    ...
    return (
        <MultiLevelColumn
            {...{ bottom }}
            showBottomAsDate={false}
            mainTitle={qcFlags}
        />
    );
});

const QCPopover = React.memo(function QCPopover({
    qualityControlMetrics,
    relationshipMapping,
}) {
    if (qualityControlMetrics.length === 0) {
        return 'No Quality Control Metrics Available';
    }
    ...
    const flagClasses =
        'mb-02 d-flex align-items-center justify-content-between';
            ...
            <table className="table table-sm table-borderless">
                <tbody>
                    {sortedQCMS.map((qcm, i) => {
                        ...
                        return (
                            <tr
                                key={atID + bam_sample_id}
                                className={`${
                                    i !== sortedQCMS.length - 1 &&
                                    'border-bottom'
                                }`}>
                                <td className="text-left text-600 text-capitalize text-larger pl-03 align-top align-left p-2">
                                    {role}:
                                </td>
                                <td className="text-left align-top text-larger align-left p-2">
                                    {sequencing_type}
                                </td>
                                <td className="p-2">
                                    {warnFlags} {failFlags}{' '}
