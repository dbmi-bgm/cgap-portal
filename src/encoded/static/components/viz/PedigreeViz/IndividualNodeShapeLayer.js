...
import {
    individualLeftPosition,
    individualTopPosition,
...
export const IndividualNodeShapeLayer = React.memo(
    function IndividualNodeShapeLayer(props) {
 ...
 */
const ClipPathDefinitions = React.memo(function ClipPathDefinitions({
    vizViewID,
    dims: { individualHeight: height, individualWidth: width },
}) {
    const numAppend = '_viewID' + vizViewID;
    return (
        <defs>
            <marker
                id={'pedigree_lineArrow' + numAppend}
                viewBox="0 0 10 10"
                refX="5"
                refY="5"
                markerWidth="4"
            ...
            <marker
                id={'pedigree_clipPath_for_debug_edge_circle' + numAppend}
                viewBox="0 0 10 10"
                refX="5"
                refY="5"
                markerWidth="4"
            ...
            <clipPath
                id={'pedigree_clipPath_for_UndeterminedShape' + numAppend}>
                <UndeterminedShape {...{ height, width }} />
            </clipPath>
            <clipPath
                id={
                    'pedigree_clipPath_for_TerminatedPregnancyShape' + numAppend
                }>
...
const FemaleShape = React.memo(function FemaleShape({
    height,
    width,
    ...passProps
}) {
    return (
        <circle
            r={Math.floor(Math.min(height, width) / 2)}
...
const MaleShape = React.memo(function MaleShape({
    height,
    width,
    ...passProps
}) {
...
const UndeterminedShape = React.memo(function UndeterminedShape({
    height,
    width,
    ...passProps
}) {
...
const TerminatedPregnancyShape = React.memo(function TerminatedPregnancyShape({
    width,
    height,
    ...passProps
}) {
...
export class IndividualNodeShape extends React.PureComponent {
    ...
    render() {
        ...
        const { individualWidth: width, individualHeight: height } = dims;
        const {
            id,
        ...
        const shape = this.memoized.getIndividualShape(
            individual,
            height,
            width
        );
        ...
        return (
            <g
                ...
                className={
                    'pedigree-individual-shape ' +
                    this.memoized.className(
                        individual,
                        isHoveredOver,
                        isSelected
                    )
...
const AffectedBGPieChart = React.memo(function AffectedBGPieChart({
    ...
    const centerY =
        shape.type === TerminatedPregnancyShape ? height / 4 : height / 2;
    const clipID =
        'pedigree_clipPath_for_' +
        shapeTypeToString(shape.type) +
        '_viewID' +
        vizViewID;
    ...
    const arcPaths = visibleDiseases.map(function (disease, idx) {
        ...
        return (
            <path
                ...
                clipPath={'url(#' + clipID + ')'}
                key={idx}
                data-disease-index={diseaseToIndex[disease]}
                title={disease}
...
const UnderlayMarkers = React.memo(function UnderlayMarkers({
    individual,
    width,
    height,
    shape,
    ...
    if (asymptoticDiseases.length > 0) {
        markers.push(
            <ColumnOfDiseases
                {...{ individual, width, height, shape, diseaseToIndex }}
                key="diseases-rects"
            />
        );
    }
...
const OverlayMarkers = React.memo(function OverlayMarkers(props) {
    ...
    const showAsDeceased =
        isDeceased && !(isSpontaneousAbortion && isPregnancy);
    ...
    if (showAsDeceased) {
        ...
        markers.push(
            <path
                d={path.toString()}
                className="deceased-cross-through"
                key="deceased-cross-through"
            />
        );
    }
    ...
    if (showAsConsultand || showAsProband) {
        ...
        markers.push(
            <path
                d={path.toString()}
                markerEnd={`url(#pedigree_lineArrow_viewID${vizViewID})`}
                key="consultand-arrow"
...
function CircleOfDiseaseDots({
    ...
    const centerX = width / 2;
    const centerY =
        shape.type === TerminatedPregnancyShape ? height * 0.3 : height / 2;
    ...
    if (diseaseLen === 1) {
        return (
            <circle
                cx={centerX}
                cy={centerY}
                r={singleCircleRadius}
                className="circle-disease-dot"
                data-disease-index={diseaseToIndex[visibleDiseases[0]]}
            />
        );
    }
    const positioningCircleRadius =
        smallestOuterDim / (shape.type === TerminatedPregnancyShape ? 8 : 4);
    const positioningCircleCircumference =
        Math.PI * 2 * positioningCircleRadius;
    const maxSmallCircleRadius =
        positioningCircleCircumference / diseaseLen / 2 - 5;
    const smallCircleRadius = Math.min(
        singleCircleRadius,
        maxSmallCircleRadius
    );
    const portionRadians = (Math.PI * 2) / diseaseLen;
    const startAngle = Math.PI * -0.5;
    const circleDims = visibleDiseases.map(function (diseaseStr, idx) {
        return [
            positioningCircleRadius *
                Math.cos(startAngle + portionRadians * idx) +
                centerX,
            positioningCircleRadius *
                Math.sin(startAngle + portionRadians * idx) +
                centerY,
        ];
    });
    ...
    const ringOfCircles = circleDims.map(function ([x, y], idx) {
        ...
        return (
            <circle
                ...
                r={smallCircleRadius}
                className="circle-disease-dot"
                data-disease-index={diseaseToIndex[diseaseStr]}
                key={diseaseStr}
            />
        );
    });

    return <React.Fragment>{ringOfCircles}</React.Fragment>;
}

/**
 * Used for showing `individual.asymptoticDiseases`.
 */
function ColumnOfDiseases({
    individual,
    width,
    height,
    shape,
    ...
    const centerX = width / 2;
    const bottomY =
        shape.type === TerminatedPregnancyShape ? height / 2 : height;
    ...
    const colOfRects = visibleDiseases.map(function (diseaseStr, idx) {
        return (
            <rect
                ...
                y={colHeight - (idx + 1) * partitionHeight}
                width={colWidth}
                height={partitionHeight}
                className="rect-disease-partition"
                data-disease-index={diseaseToIndex[diseaseStr]}
                key={diseaseStr}
                clipPath={'url(#' + clipID + ')'}
            />
    ...
    return <React.Fragment>{colOfRects}</React.Fragment>;
}
