...
export class ScaleController extends React.PureComponent {
    ...
    constructor(props) {
        ...
        this.handleInnerContainerMounted =
            this.handleInnerContainerMounted.bind(this);
        this.handleInnerContainerWillUnmount =
            this.handleInnerContainerWillUnmount.bind(this);
        this.state = {
            scale: null,
            minScale: null,
        };
        ...
        const nextMinScale =
            Math.floor(
                Math.min(
                    1,
                    maxScale,
                    Math.max(propMinScale, minScaleUnbounded)
                ) * 95
            ) / 100;
    ...
    componentDidUpdate(pastProps, pastState) {
        ...
        if (this.innerElemReference) {
            if (listenNow && !listenBefore) {
                this.innerElemReference.addEventListener(
                    'wheel',
                    this.handleWheelMove,
                    { passive: false, capture: true }
                );
            } else if (!listenNow && listenBefore) {
                this.innerElemReference.removeEventListener(
                    'wheel',
                    this.handleWheelMove
                );
            }
        ...
        if (
            ...
            if (
                nextMinScale > scale ||
                stateMinScale === scale ||
                (zoomToExtentsOnMount && (!pastHeight || !pastWidth))
            ) {
                retObj.scale = nextMinScale;
            }
            raf(() => {
            ...
            const performZoom = () => {
                ...
                setScale(
                    ...
                    initScale *
                        scaleChangeDownFactor **
                            Math.floor(
                                (Date.now() - start) / scaleChangeInterval
                            )
                );
