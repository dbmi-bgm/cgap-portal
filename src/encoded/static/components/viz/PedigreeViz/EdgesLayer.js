    ...
    return (
        <g className="individuals-edge-shape-layer">
            ...
            <g className="primary-edges">
                {allEdges.map((edge) => {
                    const id =
                        edge.fromVertex.toString() +
                        '-to-' +
                        edge.toVertex.toString();
...
const DebugVizGraphLayer = React.memo(function DebugVizGraphLayer(props) {
    ...
    const clipStyle =
        'url(#pedigree_clipPath_for_debug_edge_circle_viewID' + vizViewID + ')';
    console.log('VIZG', visibilityGraph);
    return (
        <g
            className="visibility-graph"
            style={{ stroke: '#ccc' }}
            transform={
                'translate(' + dims.graphPadding + ',' + dims.graphPadding + ')'
            }>
            {[]
                .concat(vSegments)
                .concat(hSegments)
...
function makeEdgePathDimensions(edgeObj, dims) {
    ...
    for (var i = 1; i < vertices.length; i++) {
        ...
        if (
            (futureCoord &&
                currCoord[0] !== futureCoord[0] &&
                currCoord[1] !== futureCoord[1]) ||
            ...
            futureCoord &&
            futureCoord[0] !== prevCoord[0] &&
            futureCoord[1] !== prevCoord[1]
        ) {
...
class Edge extends React.PureComponent {
    ...
    render() {
        ...
        const cls =
            'edge-path' + (adjustable ? ' is-adjustable' : ' not-adjustable');
        return <path d={edgePathDim} className={cls} id={id} />;
    }
}
