...
export function standardizeObjectsInList(jsonList) {
    ...
    function standardizeFamilialIdentifiers(listOfIndividuals) {
        ...
        listOfIndividuals.forEach(function (indv) {
            ...
            indv.parents.forEach(function (parentID) {
                ...
                parentIndv.children =
                    (parentIndv.children && parentIndv.children.slice(0)) || [];
            ...
            indv.children.forEach(function (childID) {
                ...
                childIndv.parents =
                    (childIndv.parents && childIndv.parents.slice(0)) || [];
    ...
    if (proband) {
        ...
        console.error(
            'No explicit proband found, assuming first item in list is proband.'
        );
    }
    ...
    nextList.forEach(function (indv) {
        ...
        indv.diseases.forEach(function (diseaseStr) {
            if (typeof diseaseStr !== 'string') {
                throw new Error(
                    'Validation - individual has non-string diseases value - ' +
                        indv.id
                );
            }
        ...
        indv.carrierOfDiseases.forEach(function (diseaseStr) {
            if (typeof diseaseStr !== 'string') {
                throw new Error(
                    'Validation - individual has non-string carrierOfDiseases value - ' +
                        indv.id
                );
            }
            if (indv.diseases.indexOf(diseaseStr) > -1) {
                const msg =
                    'Validation - indvidual has same disease in carrierOfDiseases as diseases - ' +
                    indv.id;
                throw new Error(msg);
                //console.warn(msg);
        ...
        indv.asymptoticDiseases.forEach(function (diseaseStr) {
            if (typeof diseaseStr !== 'string') {
                throw new Error(
                    'Validation - individual has non-string asymptoticDiseases value'
                );
            }
            if (indv.diseases.indexOf(diseaseStr) > -1) {
                const msg =
                    'Validation - indvidual has same disease in asymptoticDiseases as diseases - ' +
                    indv.id;
                throw new Error(msg);
                //console.warn(msg);
        ...
        indv.ageString =
            indv.ageString ||
            (typeof indv.age === 'number' ? indv.age + ' years' : null);
        ...
        indv.isDeceased =
            indv.isDeceased ||
            indv.isTerminatedPregnancy ||
            indv.isSpontaneousAbortion ||
            indv.isStillBirth ||
            indv.isEctopic ||
            false;
        indv.isPregnancy =
...
export function isRelationshipNode(node) {
    const { partners, children, _maritalRelationships, _parentalRelationship } =
        node;
    if (
        !_maritalRelationships &&
        !_parentalRelationship &&
        (Array.isArray(partners) || Array.isArray(children))
    ) {
        return true;
    }
    return false;
 ...
 */
export function createObjectGraph(
    jsonList,
    ...
    const remainingIndividuals = new Set(
        jsonList.map(function (indvidual) {
            const { id } = indvidual;
    ...
    while (remainingIndividuals.size > 0) {
        ...
        if (counter > 100) {
            throw new Error(
                'Found 100+ disconnected-from-proband individuals. Exiting.'
            );
        }
        if (counter > 0) {
            ...
            if (filterUnrelatedIndividuals) {
                ...
                console.error(
                    'Found ' +
                        disconnectedIndividuals.length +
                        ' disconnected-from-proband individuals.',
        ...
        while (currIndvQ.length > 0) {
            ...
            const { parents: parentIDs = [], children: childIDs = [] } =
                individual;
...
export function createRelationships(objectGraph, sepVal = '\t') {
    ...
    Object.values(parentRelationshipStringMap)
        .sort(function (a, b) {
    ...
    const relationshipObjects = [...orderedRelationshipArrs].map(function (
        parentIDs
    ) {
        const parents = parentIDs.map(function (pID) {
            return idMap[pID];
        });
