...
export function individualClassName(
    individual,
...
export const IndividualsLayer = React.memo(function IndividualsLayer(props) {
    ...
    const { aboveNodeTextContainerStyle, detailTextContainerStyle } = useMemo(
        function () {
            ...
            return {
                aboveNodeTextContainerStyle: AboveNodeText.textContainerStyle(
                    dims,
                    textScale
                ),
                detailTextContainerStyle: DetailText.textContainerStyle(
                    dims,
                    textScale
                ),
            };
...
export class IndividualDiv extends React.PureComponent {
    ...
    render() {
        ...
        const {
            id,
            name,
            _drawing: { heightIndex, xCoord, yCoord },
        } = individual;
        const elemStyle = {
            width,
            height,
        ...
        return (
            <div
                style={elemStyle}
                id={id}
                data-height-index={heightIndex}
                className={indvNodeCls}
                data-y-coord={yCoord}
                data-node-type="individual">
                {/* actionButtons */}
                {/* currentOption === 'add' ?
                    <NodeOptionsPanel {...this.props} onAddSelect={this.handleAddNewIndividual} /> : null
                */}
                <div
                    className="mouse-event-area"
...
const DetailText = React.memo(function DetailText(props) {
    ...
    const { id, name, ageString, diseases = [], orderBasedName } = individual;
    ...
    if (showNotes) {
        ...
        if (diseasesLen > diseasesLimit) {
            ...
            diseasesRows.push(
                <div className="line-item" key="end">
                    <em>and {difference} more...</em>
                </div>
            );
        }
    ...
    return (
        <div className="detail-text" style={detailTextContainerStyle}>
            <h5
                data-describing="title"
                className={
                    showOrderBasedName ? ' showing-order-based-name' : ''
                }
                style={{ width }}>
...
DetailText.textContainerStyle = function (dims, textScale = 1) {
    ...
    return {
        ...
        transform: `translate3d(0, ${
            individualHeight + individualHeight / 10
        }px, 0)`,
...
const AboveNodeText = React.memo(function AboveNodeText(props) {
    const { individual, maxHeightIndex, aboveNodeTextContainerStyle } = props;
    ...
    return (
        <div
            className="above-node-text"
            data-describing="ancestry"
            style={aboveNodeTextContainerStyle}>
            {showAncestry.join(' • ')}
        </div>
    );
});
