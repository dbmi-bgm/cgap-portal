...
export function createEdges(objectGraph, dims, graphHeight) {
    ...
    const childToMidPointYDifference =
        halfIndvHeight + dims.individualYSpacing / 3;
        ...
        if (parentRelation) {
            ...
            if (children.length === 1) {
                ...
                children.forEach(function (child) {
                    ...
                    biggestYCoord = Math.max(
                        biggestYCoord,
                        child._drawing.yCoord
                    );
                    smallestXCoord = Math.min(
                        smallestXCoord,
                        child._drawing.xCoord
                    );
                    biggestXCoord = Math.max(
                        biggestXCoord,
                        child._drawing.xCoord
                    );
                    console.log(
                ...
                const childEdgeSegmentTopCoord =
                    biggestYCoord - childToMidPointYDifference;
                const childEdgeSegment = {
                    fromVertex: [smallestXCoord, childEdgeSegmentTopCoord],
                    toVertex: [
                ...
                midPoint = [relationXCoord, childEdgeSegmentTopCoord];
            ...
            if (midPoint) {
                const mpToRelationshipEdgeSegment = {
                    fromVertex: [relationXCoord, relationYCoord],
                    toVertex: midPoint,
                    adjustable: false,
                    direct: true, // We won't modify this further
            ...
            partners.forEach(function (partner, partnerIdx) {
                ...
                const toIndividualXAttachment =
                    halfIndvWidth * (attachToIndividualOnLeftSide ? -1 : 1);
                const toIndividualXAttachmentLedge =
                    dims.edgeLedge * (attachToIndividualOnLeftSide ? -1 : 1);
                ...
                parentEdge.vertices = [
                    ...
                    [
                        parentEdge.fromVertex[0] - toIndividualXAttachmentLedge,
                        parentEdge.fromVertex[1],
                    ],
                    // Parent
                    [
                        parentEdge.toVertex[0] + toIndividualXAttachmentLedge,
                        parentEdge.toVertex[1],
                    ],
                    parentEdge.toVertex,
                ];
    ...
    const countDirectEdges =
        seenParentalRelationships.size / objectGraph.length < 0.4;
    let visibilityGraph = computeVisibilityGraph(
        objectGraph,
        directEdges,
        dims,
        graphHeight,
    ...
    function trySubdivisions() {
        ...
        try {
            visibilityGraph = computeVisibilityGraph(
                objectGraph,
                directEdges,
                dims,
                graphHeight,
...
function tracePaths(adjustableEdges, visibilityGraph) {
    const { hSegments: hSegmentQ, vSegments: vSegmentQ } = visibilityGraph;
    ...
    function computeDistance(initCurrV, targetV, initSearchPath = []) {
        ...
        function getNextVQueueSet() {
            ...
            for (i = 0; i < vQueue.length; i++) {
                ...
                adjustableEdges.forEach(function (edge) {
                    ...
                    for (let j2 = 1; j2 < edge._tempNextVertices.length; j2++) {
                        const prevAdjEdge = [
                            edge._tempNextVertices[j2 - 1],
                            edge._tempNextVertices[j2],
                        ];
                        if (
                            isVertical &&
                            prevAdjEdge[0][1] === prevAdjEdge[1][1] &&
                            v[1] === prevAdjEdge[0][1]
                        ) {
                            if (
                                (prevAdjEdge[0][0] >= v[0] &&
                        ...
                        if (
                            !isVertical &&
                            prevAdjEdge[0][0] === prevAdjEdge[1][0] &&
                            v[0] === prevAdjEdge[0][0]
                        ) {
                            if (
                                (prevAdjEdge[0][1] >= v[1] &&
                ...
                if (intersected) {
                    costToTargetEstimate =
                        costToTargetEstimate * (intersected / 4 + 1);
                }
                //*/

                if (
                    prevEdge1[0][1] === targetV[1] &&
                    prevEdge1[1][1] === targetV[1]
                ) {
            ...
            result.heuristicCostTotal =
                (result.heuristicCostTotal || 0) + bestCostEstimate;
            return result;
        }
        ...
        while (vQueue.length) {
            ...
            if (
                existingRes &&
                existingRes.pathLengthCost < currPathLengthCost
            ) {
                continue;
            }
            if (
                existingRes &&
                existingRes.heuristicCostTotal <= heuristicCostTotal
            ) {
                continue;
            }
            ...
            if (currV[0] === 600 && currV[1] === 40) {
                console.log(
                    ...
                    currV,
                    '\nDEPTH',
                    currSearchPath.length,
                    '\nCOSTNOW',
                    currPathLengthCost,
                    heuristicCostTotal,
                    '\nOWNPATH',
                    currSearchPath.map(function (e) {
                        return e[0].join(',') + '-' + e[1].join(',');
                    })
                    //'\nSKIP', [...skip.values()]
                );
            }
            ...
            if (currV[0] === targetV[0] && currV[1] === targetV[1]) {
                if (currPathLengthCost < bestPathTotalCost) {
                    ...
                    console.info(
                        `Found best path between ${initCurrV} and ${targetV} after ${iterations} iterations (0,0 is top left).`
                    );
                    break;
                }
            }

            const nextAvailableSegments = getNextAvailableEdgeSegments(
                currV,
                skip
            );
    ...
    adjustableEdges
        .sort(function (a, b) {
            ...
            return (
                Math.floor(manhattanDistance(fromAV, toAV) / 200) -
                Math.floor(manhattanDistance(fromBV, toBV) / 200)
            );
        })
        .forEach(function (edge, edgeIndex) {
            ...
            const { cost, path } = computeDistance(fromV, toV, [
                edge.vertices.slice(0, 2),
            ]);
            ...
            for (pathIdx = 1; pathIdx < path.length; pathIdx++) {
                // Skip first edge segment (edge ledge)
                const edgeSegmentToRemove = path[pathIdx];
                const useQ =
                    edgeSegmentToRemove[0][0] === edgeSegmentToRemove[1][0]
                        ? vSegmentQ
                        : hSegmentQ;
            ...
            edge._tempNextVertices = path.reduce(
                function (m, edgePart) {
                    const lastAdded = m[m.length - 1];
                    if (
                        lastAdded[0] === edgePart[0][0] &&
                        lastAdded[1] === edgePart[0][1]
                    ) {
                        m.push(edgePart[1]);
                    } else {
                        m.push(edgePart[0]);
            ...
            // Last edgeLedge
            edge._tempNextVertices.push(
                edge.vertices[edge.vertices.length - 1]
            );
            ...
            const { prevV: lastV, vertices: flattenedVertices } =
                edge._tempNextVertices.reduce(
                    function (m, v) {
                        if (!m.prevV) {
                            m.vertices.push(v);
                            return {
...
export function computeVisibilityGraph(
    ...
    objectGraph.forEach(function (indv) {
        ...
        relationships.forEach(function (relationship) {
            const {
                _drawing: { xCoord, yCoord: relationYCoord },
            } = relationship;
    ...
    directEdges.forEach(function (edge) {
        ...
        if (edge.vertices[0][0] === edge.vertices[1][0]) {
            ...
            if (!countDirectEdges) {
                ...
                bb = [
                    [
                        edge.vertices[0][0] - halfRelationSize,
                        edge.vertices[1][1],
                    ],
                    [
                        edge.vertices[0][0] - halfRelationSize,
            ...
            if (edge.vertices[0][0] < edge.vertices[1][0]) {
                bb = [
                    [
                        edge.vertices[0][0],
                        edge.vertices[1][1] - halfRelationSize,
                    ],
                    [
                        edge.vertices[0][0],
                        edge.vertices[1][1] + halfRelationSize,
    ...
    */
    ...
    while (yCoord < graphHeight) {
        ...
        const withinIndvRow =
            yCoord % (dims.individualHeight + dims.individualYSpacing) <
        ...
        boundingBoxes.forEach(function (bb) {
            if (bb.length === 2) {
                if (bb[0][1] === bb[1][1]) {
                    ...
                    if (
                        // Vertical Line which may intersect
        ...
        boundingBoxes.forEach(function (bb) {
            if (bb.length === 2) {
                if (bb[0][0] === bb[1][0]) {
                    ...
                    if (
                        // Horiz Line which may intersect
