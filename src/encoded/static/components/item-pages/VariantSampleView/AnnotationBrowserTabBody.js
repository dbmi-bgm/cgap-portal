...
export const AnnotationBrowserTabBody = React.memo(
    function AnnotationBrowserTabBody(props) {
        const { context, schemas, active = false } = props;
        const higlassContainerRef = useRef(null);
        const {
            variant: { POS_ABS: variantPositionAbsCoord },
        } = context;
