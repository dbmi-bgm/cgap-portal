...
export const VariantTabBody = React.memo(function VariantTabBody(props) {
    ...
    const { getTipForField, clinvarExternalHref } = useMemo(
        function () {
            // TODO: consider moving to AnnotationSections & sharing between SV & SNV

            const ret = {
                getTipForField: function () {
                    return null;
                },
            ...
            if (schemas) {
                ...
                ret.getTipForField = function (field, itemType = 'Variant') {
                    ...
                    const { description, extended_description } =
                        schemaProperty || {};
                ...
                if (variationID) {
                    const clinvarIDSchemaProperty =
                        schemaTransforms.getSchemaProperty(
                            'csq_clinvar',
                            schemas,
                            'Variant'
                        );
                    ret.clinvarExternalHref =
                        clinvarIDSchemaProperty.link.replace(
                            '<ID>',
                            variationID
                        );
                }
    ...
    const titleDict = useMemo(function () {
        return {
            v2: (
                <React.Fragment>
                    <span className="text-600">gnomAD</span> v2 exome
                </React.Fragment>
            ),
            v3: (
                <React.Fragment>
                    <span className="text-600">gnomAD</span> v3
                </React.Fragment>
            ),
        };
    });

    const gnomadExternalLink = useMemo(
        function () {
                ...
                <div className="col-12 col-xl-6 d-flex flex-column">
                    ...
                    <div className="inner-card-section flex-grow-0 pb-2 pb-xl-0">
                        <div className="info-header-title justify-content-start">
                            <DropdownButton
                                size="lg py-1"
                                variant="outline-secondary select-gnomad-version"
                                onSelect={onSelectShowingTable}
                                title={titleDict[showingTable]}>
                                <DropdownItem
                                    eventKey="v3"
                                    active={showingTable === 'v3'}>
                                    {titleDict.v3}
                                </DropdownItem>
                                <DropdownItem
                                    eventKey="v2"
                                    active={showingTable === 'v2'}>
                                    {titleDict.v2}
                                </DropdownItem>
                            </DropdownButton>
                            ...
                            {gnomadExternalLink ? (
                                <h4>
                                    <a
                                        href={gnomadExternalLink}
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="text-small px-1"
                                        data-tip={
                                            'View this variant in gnomAD ' +
                                            showingTable
                                        }>
                        ...
                        <div className="info-body overflow-auto">
                            <GnomADTable
                                {...{ context, schemas, getTipForField }}
                                prefix={
                                    showingTable === 'v2'
                                        ? 'csq_gnomade2'
                                        : 'csq_gnomadg'
                    ...
                    <div className="inner-card-section flex-grow-1 pb-2 pb-xl-1">
                        ...
                        <div className="info-body">
                            ...
                            <ExternalResourcesSection
                                {...{ context, schemas, currentTranscriptIdx }}
                            />
                        </div>
                    </div>
        ...
        <table className="w-100">
            <thead>
                <tr>
                    ...
                    <th data-tip={getTipForField(prefix + '_ac')}>
                        Allele Count
                    </th>
                    <th data-tip={getTipForField(prefix + '_an')}>
                        Allele Number
                    </th>
                    <th data-tip={getTipForField(prefix + '_nhomalt')}>
                        # of Homozygotes
                    </th>
                    <th
                        className="text-left"
                        data-tip={getTipForField(prefix + '_af')}>
                        Allele Frequency
                    </th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr className="border-top">
                    ...
                    <td className="text-left">
                        {gnomad_af_female === 0
                            ? '0.0000'
                            : standardizeGnomadValue(gnomad_af_female)}
                    </td>
                </tr>
                <tr>
                    ...
                    <td className="text-left">
                        {gnomad_af_male === 0
                            ? '0.0000'
                            : standardizeGnomadValue(gnomad_af_male)}
                    </td>
                </tr>
                <tr className="border-top">
                    ...
                    <td className="bg-light text-600">
                        {standardizeGnomadValue(gnomad_ac)}
                    </td>
                    <td className="bg-light text-600">
                        {standardizeGnomadValue(gnomad_an)}
                    </td>
                    <td className="bg-light text-600">
                        {standardizeGnomadValue(gnomad_nhomalt)}
                    </td>
                    <td className="bg-light text-600 text-left">
                        {gnomad_af === 0
                            ? '0.0000'
                            : standardizeGnomadValue(gnomad_af)}
                    </td>
                </tr>
            </tbody>
        </table>
                        ...
                        <tr>
                            <td className="text-left">
                                ...
                                <QuickPopover
                                    ...
                                    htmlContent={getTipForField('csq_gerp_rs')}
                                />
                            </td>
...
function ExternalResourcesSection({ context, schemas, currentTranscriptIdx }) {
    const { variant } = context;
    const { transcript = [], CHROM: hg38CHR, POS: hg38POS } = variant;

    if (!variant) {
        return null;
    }

    const externalDatabaseFieldnames = ['csq_clinvar'];
            ...
            <div className="row mb-03" key="POS">
                <div className="col-12 col-lg">
                    <label
                        ...
                        htmlFor="external_resource_for_ucsc_hg38"
                        data-tip="See position in UCSC Genome Browser">
                        UCSC Genome Browser
                    </label>
                </div>
                <div className="col-12 col-lg-auto">
                    <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        id="external_resource_for_ucsc_hg38">
    ...
    return (
        <ExternalDatabasesSection
            itemType="Variant"
            {...{ currentItem, schemas, externalDatabaseFieldnames }}
            appendItems={externalResourcesAppend}
        />
    );
}
...
const hgmdSearchLink = [
    <div className="row mb-03" key="POS">
        <div className="col-12 col-lg">
            <label
                className="mb-0 black-label"
                htmlFor="external_resource_for_hgmdidsearch"
                data-tip="HGMD Public (Human Gene Mutation Database)">
                HGMD (Professional)
            </label>
        </div>
        <div className="col-12 col-lg-auto">
            <a
                href="https://my.qiagendigitalinsights.com/bbp/view/hgmd/pro/search_mut.php"
                className="d-block"
                target="_blank"
                rel="noopener noreferrer"
                id="external_resource_for_hgmdidsearch">
