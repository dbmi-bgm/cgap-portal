        ...
        <table className="w-100 text-left">
            ...
            <tbody>
                <tr>
                    ...
                    <td>
                        <span data-tip={getTipForField("exp_syn")}>{ ensureIsNumber(exp_syn, fallbackNotPresent)}</span>
                    </td>
                    <td>
                        <span data-tip={getTipForField("exp_mis")}>{ ensureIsNumber(exp_mis, fallbackNotPresent)}</span>
                    </td>
                    <td>
                        <span data-tip={getTipForField("exp_lof")}>{ ensureIsNumber(exp_lof, fallbackNotPresent)}</span>
                    </td>
                </tr>
                <tr>
                    <td className="text-600 text-left">Observed</td>
                    <td>
                        <span data-tip={getTipForField("obs_syn")}>{ ensureIsNumber(obs_syn, fallbackNotPresent)}</span>
                    </td>
                    <td>
                        <span data-tip={getTipForField("obs_mis")}>{ ensureIsNumber(obs_mis, fallbackNotPresent)}</span>
                    </td>
                    <td>
                        <span data-tip={getTipForField("obs_lof")}>{ ensureIsNumber(obs_lof, fallbackNotPresent)}</span>
                    </td>
                </tr>
                <tr>
                    <td className="text-600 text-left">O/E (range)</td>
                    <td>
                        <span data-tip={getTipForField("oe_syn")}>{ ensureIsNumber(shortenToSignificantDigits(oe_syn), fallbackNotPresent)}</span>
                        { oe_syn_lower !== null && oe_syn_upper !== null ? ` (${oe_syn_lower} - ${oe_syn_upper})` : null }
                    </td>
                    <td>
                        <span data-tip={getTipForField("oe_mis")}>{ ensureIsNumber(shortenToSignificantDigits(oe_mis), fallbackNotPresent)}</span>
                        { oe_mis_lower !== null && oe_mis_upper !== null ? ` (${oe_mis_lower} - ${oe_mis_upper})` : null }
                    </td>
                    <td>
                        <span data-tip={getTipForField("oe_lof")}>{ ensureIsNumber(shortenToSignificantDigits(oe_lof), fallbackNotPresent)}</span>
                ...
                <tr>
                    ...
                    <td>
                        <span data-tip={getTipForField("syn_z")}>{ ensureIsNumber(shortenToSignificantDigits(syn_z), fallbackNotPresent)}</span>
                    </td>
                    <td>
                        <span data-tip={getTipForField("mis_z")}>{ ensureIsNumber(shortenToSignificantDigits(mis_z), fallbackNotPresent)}</span>
                    </td>
                    <td>
                        <span data-tip={getTipForField("lof_z")}>{ ensureIsNumber(shortenToSignificantDigits(lof_z), fallbackNotPresent)}</span>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>{ fallbackNotImplemented }</td>
                    <td>
                        <span data-tip={getTipForField("oe_lof_upper")}>{ ensureIsNumber(oe_lof_upper, fallbackNotPresent)}</span>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>{ fallbackNotImplemented }</td>
                    <td>
                        <span data-tip={getTipForField("s_het")}>{ ensureIsNumber(shortenToSignificantDigits(s_het), fallbackNotPresent)}</span>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>{ fallbackNotImplemented }</td>
                    <td>
                        <span data-tip={getTipForField("rvis_exac")}>{ ensureIsNumber(shortenToSignificantDigits(rvis_exac), fallbackNotPresent)}</span>
                    </td>
                    <td>{ fallbackNotImplemented }</td>
                </tr>
            </tbody>
...
/**
 * Takes in a potentially falsy value and if actually not a number, returns fallback
 */
function ensureIsNumber(value, fallback) {
