    ...
    render() {
        ...
        return (
            <GenericInterpretationSidebar headerTitle="SV / CNV Interpretation Space">
                <InterpretationSpaceWrapper
                    {...passProps}
                    tabsToDisable={{ 2: true }}
                    isCNV
                />
            </GenericInterpretationSidebar>
        );
...
export class InterpretationSpaceWrapper extends React.Component {
    ...
    static cleanUpNoteStateForPostPatch(noteState, noteType) {
        ...
        const fieldsToCleanFromInterpretation = [
            'gene_candidacy',
            'variant_candidacy',
        ];
        const fieldsToCleanFromDiscovery = [
            'acmg_rules_invoked',
            'conclusion',
            'classification',
        ];
        const fieldsToCleanFromStandard = [
            'acmg_rules_invoked',
            'conclusion',
            'classification',
            'gene_candidacy',
        ...
        switch (noteType) {
            ...
            default:
                throw new Error(
                    "Failed to cleanup note of type '" +
                        noteType +
                        "' for submission"
                );
        }
    ...
    constructor(props) {
        ...
        this.patchPreviouslySavedNote =
            this.patchPreviouslySavedNote.bind(this);
    ...
    postNewNote(note, noteType) {
        const { context: { institution = null, project = null } = {} } =
            this.props;
        const { '@id': variantSampleInstitutionID } = institution || {};
        const { '@id': variantSampleProjectID } = project || {};

        const noteToSubmit =
            InterpretationSpaceWrapper.cleanUpNoteStateForPostPatch(
                note,
                noteType
            ); // returns a cleaned clone
    ...
    patchNewNoteToVS(noteAtID, saveToField) {
        ...
        return ajax.promise(
            ...
            'PATCH',
            {},
            JSON.stringify({ [saveToField]: noteAtID })
        );
    }

    patchPreviouslySavedNote(noteToPatch, noteType, noteAtID) {
        // ONLY USED FOR DRAFTS -- other notes are cloned
        const { interpretation = null, discovery_interpretation = null } =
            this.state;
        const { classification: previousClassification = null } =
            interpretation || {};
        const {
            variant_candidacy: previousVarCandidacy = null,
            gene_candidacy: previousGeneCandidacy = null,
        } = discovery_interpretation || {};
        const {
            classification = null,
        ...
        switch (noteType) {
            ...
            case 'note_discovery':
                if (
                    !variant_candidacy &&
                    previousVarCandidacy &&
                    !gene_candidacy &&
                    previousGeneCandidacy
                ) {
                    patchURL +=
                        '?delete_fields=variant_candidacy,gene_candidacy';
        ...
        return ajax.promise(
            ...
            'PATCH',
            {},
            JSON.stringify(cleanedNoteToPatch)
        );
    }
    ...
    saveHighlightedGene(highlightedGeneToPatch, successCallback) {
        const {
            context: {
                '@id': vsAtID,
                highlighted_genes: lastSavedHighlighted = [],
            } = {},
        } = this.props;
        let patchURL = vsAtID;
        ...
        if (!highlightedGeneToPatch) {
            // deleting previous value
            if (lastSavedHighlighted.length > 0) {
                patchURL += '?delete_fields=highlighted_genes';
            } else {
                throw new Error(
                    'Attempting to delete highlighted gene when there was no previous value.'
                );
            }
        } else {
            // adding new value
            ...
            payload = JSON.stringify({
                highlighted_genes: [highlightedGeneAtID],
            });
            postPatchState = [highlightedGeneToPatch];
        }

        this.setState({ loading: true }, () => {
            ajax.promise(patchURL, 'PATCH', {}, payload).then((response) => {
                ...
                this.setState(
                    { loading: false, highlighted_genes: postPatchState },
                    successCallback
                );
            });
        });
    ...
    saveAsDraft(
        ...
        if (lastSavedNote) {
            // Patch the pre-existing draft item & overwrite it
            console.log(
                'Note already exists... need to patch pre-existing draft',
                lastSavedNote
            );
            ...
            this.setState({ loading: true }, () => {
                this.patchPreviouslySavedNote(note, noteType, noteAtID)
                    .then((response) => {
                        ...
                        console.log(
                            'Successfully overwritten previous draft of note',
                            response
                        );
                        return ajax.promise(
                            noteAtID + '?datastore=database',
                            'GET'
                        );
                    })
                    .then((noteWithEmbeds) => {
                        console.log(
                            'Successfully retrieved @@embedded representation of note',
                            noteWithEmbeds
                        );
                        this.setState(
                            {
                                loading: false,
                                [stateFieldToUpdate]: noteWithEmbeds,
                            },
                            successCallback
                        );
                    })
            ...
            // Create a whole new item, and patch to VS
            let newNoteID;
            ...
            this.setState({ loading: true }, () => {
                this.postNewNote(note, noteType)
                    ...
                    .then((resp) => {
                        ...
                        console.log(
                            'Successfully linked note object to variant sample',
                            resp
                        );
                        return ajax.promise(
                            newNoteID + '?datastore=database',
                            'GET'
                        );
                    })
                    .then((noteWithEmbeds) => {
...
export class InterpretationSpaceController extends React.Component {
    ...
    static hasNoteChanged(lastSavedNote = null, currNote = null) {
        ...
        if (
            currClassification !== lastSavedClassification ||
     ...
     */
    static hasHighlightedGeneChanged(
        wipHighlightedGene,
        lastSavedHighlightedGene
    ) {
        if (!wipHighlightedGene && !lastSavedHighlightedGene) {
            return false;
        } // probably on snv
        if (wipHighlightedGene.size === 0 && !lastSavedHighlightedGene) {
            ...
            const { 0: { '@id': lastSavedHighlightedGeneID } = [] } =
                lastSavedHighlightedGene || {};
    ...
    constructor(props) {
        super(props);
        const {
            lastSavedVariantNote,
            lastSavedGeneNote,
            lastSavedInterpretation,
            lastSavedDiscovery,
            defaultTab,
        } = props;
        ...
        this.state = {
            ...
            currentTab:
                ...
                defaultTab >= 0 &&
                defaultTab < InterpretationSpaceController.tabNames.length
                    ? defaultTab
                    : 0,
        ...
        this.memoized = {
            haveEditPermission: memoize(
                InterpretationSpaceController.haveEditPermission
            ),
        };
    }
    ...
    render() {
        ...
        const isDraftVariantNoteUnsaved =
            InterpretationSpaceController.hasNoteChanged(
                variant_notes_wip,
                lastSavedVariantNote
            );
        const isDraftGeneNoteUnsaved =
            InterpretationSpaceController.hasNoteChanged(
                gene_notes_wip,
                lastSavedGeneNote
            );
        const isDraftDiscoveryUnsaved =
            InterpretationSpaceController.hasNoteChanged(
                discovery_interpretation_wip,
                lastSavedDiscovery
            );
        ...
        const isHighlightedGeneUnsaved =
            InterpretationSpaceController.hasHighlightedGeneChanged(
                selectedGenes,
                lastSavedHighlightedGene
            );
        const isGeneTabUnsaved = isCNV
            ? isDraftGeneNoteUnsaved || isHighlightedGeneUnsaved
            : isDraftGeneNoteUnsaved;
        ...
        const isDraftInterpretationUnsaved =
            InterpretationSpaceController.hasNoteChanged(
                interpretationWIP,
                lastSavedInterpretation
            );
        ...
        switch (currentTab) {
            case 0: // Gene Notes
                var otherDraftsUnsaved =
                    isDraftInterpretationUnsaved ||
                    isDraftVariantNoteUnsaved ||
                    isDraftDiscoveryUnsaved;
                if (isCNV) {
                    const highlightedGeneID_wip =
                        selectedGenes.keys().next().value || null;
                    panelToDisplay = (
                        <SVGeneNotePanel
                            {...commonProps}
                            {...{
                    ...
                    panelToDisplay = (
                        <GenericInterpretationPanel
                            {...commonProps}
                            {...{ otherDraftsUnsaved }}
                            lastWIPNote={gene_notes_wip}
                            lastSavedNote={lastSavedGeneNote}
                            saveToField="gene_notes"
                            noteType="note_standard"
                        />
                    );
                }
            ...
            case 1: // Variant Notes
                panelToDisplay = (
                    <GenericInterpretationPanel
                        {...commonProps}
                        lastWIPNote={variant_notes_wip}
                        lastSavedNote={lastSavedVariantNote}
                        saveToField="variant_notes"
                        noteType="note_standard"
                        otherDraftsUnsaved={
                            isDraftInterpretationUnsaved ||
                            isGeneTabUnsaved ||
            ...
            case 2: // Interpretation
                panelToDisplay = (
                    <GenericInterpretationPanel
                        {...commonProps}
                        wipACMGSelections={wipACMGSelections}
                        {...{ autoClassification, toggleInvocation }}
                        lastWIPNote={interpretationWIP}
                        lastSavedNote={lastSavedInterpretation}
                        saveToField="interpretation"
            ...
            case 3: // Discovery
                panelToDisplay = (
                    <GenericInterpretationPanel
                        {...commonProps}
                        lastWIPNote={discovery_interpretation_wip}
                        lastSavedNote={lastSavedDiscovery}
                        saveToField="discovery_interpretation"
...
function GenericInterpretationSpaceTabBtn(props) {
    ...
    const className = `${cls} btn btn-xs ${
        isActive ? 'btn-primary-dark' : 'btn-link'
    } ${unsavedDraft ? 'font-italic' : ''}`;
    return (
        <div data-tip={disabled ? 'Coming Soon' : ''}>
            <button
                ...
                {tabName}
                {unsavedDraft && (
                    <span className="text-danger text-600">*</span>
                )}
            </button>
        </div>
    );
...
function InterpretationSpaceTabs(props) {
    ...
    // Maybe memoize?
    const tabsRender = InterpretationSpaceController.tabNames.map(
        (tabName, i) => {
            ...
            return (
                <GenericInterpretationSpaceTabBtn
                    key={i}
                    {...{
                        tooltip,
                        onClick,
...
const refreshParentWindowVariantSampleList = _.debounce(
    function () {
        window.opener.postMessage({ action: 'refresh-variant-sample-list' });
    },
    1200,
    false
); // Debounced to prevent accidental double-clicks & (too-)rapid changes
...
class SVGeneNotePanel extends React.PureComponent {
    constructor(props) {
        ...
        this.saveHighlightedGeneWrapper =
            this.saveHighlightedGeneWrapper.bind(this);
    }
    ...
    onTextAreaChange(value, idx) {
        ...
        const { gene_notes: [{ associated_items = [] } = {}] = [] } =
            this.state;
    ...
    selectNewGene(geneId, idx) {
        ...
        if (!numNotes) {
            ...
            if (idx !== 0 && !idx) {
                throw new Error('No idx passed into selectNewGene');
            }
            if (gene_notes[idx]) {
                // note exists, update old object
                if (gene_notes[idx].note_text) {
                    // check for note text
                    // query user to confirm they want to overwrite old note
                    const okToOverwrite = confirm(
                        'Switching genes will delete current gene note. Is this okay?'
                    );
                    if (okToOverwrite) {
                        const updatedNote = { ...newState[idx] };
                        updatedNote.associated_items[0].item_identifier =
                            geneId;
                ...
                // note doesn't exist yet; create new obj
                const associatedItems = {
                    item_type: 'Gene',
                    item_identifier: geneId,
                };
    ...
    **/
    ...
    render() {
        ...
        const noteTextPresent =
            gene_notes.length > 0 ||
            (gene_notes.length === 1 && gene_notes[0].note_text);

        // Generate a list of genes for passing into other fields
        const {
            structural_variant: { transcript: transcripts = [] } = {},
            highlighted_gene: highlightedGene = [],
        } = context;
        ...
        return (
            <div className="interpretation-panel">
                ...
                <HighlightedGenesDrop
                    selectedGeneID={highlightedGeneID_wip}
                    {...{
                        geneEnsgids,
                        genes,
                        geneAtIDToGeneMap,
                ...
                /> */}
                <SVGeneInterpretationSubmitButton
                    highlightedGeneChangedSinceLastSave={
                        isHighlightedGeneUnsaved
                    }
                    selectedGeneID={highlightedGeneID_wip}
                    {...{ hasEditPermission, geneAtIDToGeneMap }}
                    saveHighlightedGene={this.saveHighlightedGeneWrapper}
                />
                {caseSource ? (
                    <button
                        type="button"
                        className="btn btn-primary btn-block mt-05"
                        onClick={() => onReturnToCaseClick(caseSource)}>
                        Return to Case
                    </button>
                ) : null}
            </div>
        ...
        return (
            <div>
                <GenesDrop
                    ...
                    value={null}
                    {...{ genes, geneAtIDToGeneMap, selectNewGene }}
                />
            </div>
        );
    }
    ...
    return (
        <>
            {notes.map((note, idx) => {
                ...
                const geneDisplayTitle =
                    geneAtIDToGeneMap[item_identifier].display_title;
                ...
                return (
                    <div key={idx}>
                        ...
                        <AutoGrowTextArea
                            ...
                            value={note_text}
                            onChange={(e) => onEditNote(e.target.value, idx)}
...
function GenesDrop(props) {
    ...
    if (genes.length > 0) {
        dropOptions = genes.map((geneID) => {
            ...
            return (
                <Dropdown.Item
                    onClick={() => selectNewGene(atID, noteIdx)}
                    key={geneID}>
                    {display_title}
                </Dropdown.Item>
            );
    ...
    return (
        <>
            <label className="w-100 text-small mt-1">Add Gene Note</label>
...
function HighlightedGenesDrop(props) {
    ...
    function customFilterFunction(
        ...
        const regexQueryForDisplayTitle = SearchAsYouTypeLocal.getRegexQuery(
            currentTextValue,
            filterMethodForDisplayTitle
        );
        ...
        return searchList.filter(function (atID) {
            ...
            const displayTitleMatches = lowerDisplayTitle.match(
                regexQueryForDisplayTitle
            );
    ...
    return (
        <>
            <label className="w-100 text-small">
                <i className="icon icon-star text-primary fas icon-fw"></i>{' '}
                Selected Gene for Interpretation
            </label>
            <div className="w-100 d-flex note-field-drop mb-1">
                <SearchAsYouTypeLocal
                    className={cls}
                    searchList={genes}
                    ...
                    onChange={(atID) => {
                        onSelectGene(geneAtIDToGeneMap[atID]);
                    }}
                    maxResults={3}
                    {...{
                ...
                {selectedGeneID ? (
                    <Button
                        variant="danger"
                        className={cls + ' ml-03'}
                        onClick={() => onResetSelectedGenes(highlightedGene)}>
                        <i className="icon icon-trash-alt fas" />
                    </Button>
                ) : null}
            </div>
...
class GenericInterpretationPanel extends React.PureComponent {
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        const isThisNoteUnsaved = InterpretationSpaceController.hasNoteChanged(
            lastSavedNote,
            savedState
        );
    ...
    componentWillUnmount() {
        // Before unmounting (as in switching tabs), save unsaved changes in controller state
        const { saveToField, retainWIPStateOnUnmount, lastWIPNote } =
            this.props;
    ...
    render() {
        ...
        const { status: savedNoteStatus, last_modified: lastModified = null } =
            lastSavedNote || {};
        const {
            modified_by: {
        ...
        const noteChangedSinceLastSave =
            InterpretationSpaceController.hasNoteChanged(
                lastSavedNote,
                stateToSave
            );
        ...
        return (
            <div className="interpretation-panel">
                <label
                    className={`w-100 ${
                        lastModUsernameFromNew || lastModUsername ? 'mb-0' : ''
                    }`}>
                    {noteLabel}
                </label>
                {lastModUsernameFromNew || lastModUsername ? (
                    <div className="text-muted text-smaller my-1">
                        Last Saved:{' '}
                        <LocalizedTime
                        ...
                        />{' '}
                        by {lastModUsernameFromNew || lastModUsername}{' '}
                    </div>
                ) : null}
                <AutoGrowTextArea
                    disabled={isFallback}
                ...
                {noteType === 'note_interpretation' ? (
                    <GenericFieldForm
                        ...
                        fieldsArr={[
                            ...
                            {
                                field: 'acmg_rules_invoked',
                                value: wipACMGSelections,
                                autoClassification,
                                toggleInvocation,
                            },
                        ]}
                        {...{ schemas, noteType }}
                        onDropOptionChange={this.onDropOptionChange}
                    />
                ) : null}
                {noteType === 'note_discovery' ? (
                    <GenericFieldForm
                        fieldsArr={[
                            { field: 'gene_candidacy', value: gene_candidacy },
                            {
                                field: 'variant_candidacy',
                                value: variant_candidacy,
                            },
                        ]}
                        {...{ schemas, noteType, isFallback }}
                        onDropOptionChange={this.onDropOptionChange}
                    />
                ) : null}
                <GenericInterpretationSubmitButton
                    {...{
                        hasEditPermission,
                        isFallback,
                        isCurrent,
...
function NoteFieldDrop(props) {
    ...
    if (static_enum.length > 0) {
        dropOptions = static_enum.map((option) => (
            <Dropdown.Item
                onClick={() => onOptionChange(field, option)}
                key={option}>
                <i
            ...
            <div className="w-100 d-flex note-field-drop">
                <Dropdown as={ButtonGroup} className={cls}>
                    <Dropdown.Toggle
                        ...
                        variant="outline-secondary text-left"
                        id="dropdown-basic">
                        {value ? (
                            <>
                                <i
                                    className="status-indicator-dot ml-1 mr-07"
                                    data-status={value}
                                />{' '}
                                {value}
                            </>
                ...
                {value ? (
                    <Button
                        ...
                        variant="danger"
                        className={cls + ' ml-03'}
                        onClick={() => onOptionChange(field, null)}>
                        <i className="icon icon-trash-alt fas" />
                    </Button>
                ) : null}
            </div>
...
function GenericFieldForm(props) {
    ...
    const getFieldProperties = useMemo(
        function () {
            if (!schemas)
                return function () {
                    return null;
    ...
    const fieldsJSX = useMemo(
        function () {
            return fieldsArr
                ...
                .sort()
                .reverse(); // Reverse really just to get Variant candidacy to show up last. May need a better solution if more fields added in future.
        },
        [schemas, noteType, fieldsArr]
    );

    return <React.Fragment>{fieldsJSX}</React.Fragment>;
}
...
function ACMGPicker(props) {
    ...
    const picked = selections.map(function (selection, i) {
        return (
            <ACMGPickerOption
                {...{ selection, toggleInvocation, isFallback }}
                key={i}
            />
        );
    });
        ...
        <React.Fragment>
            ...
            <div className="w-100 d-flex flex-wrap acmg-picker mb-08">
                {selections.length > 0 ? (
                    ...
                    <div
                        className="acmg-invoker text-muted"
                        data-tip={'Use the picker above to make invocations.'}
                        data-criteria="none">
                        None
                    </div>
                )}
...
export const ACMGPickerOption = React.memo(function ACMGPickerOption(props) {
    ...
    const cls =
        'acmg-invoker ' +
        (isFallback ? 'unclickable' : 'clickable') +
        (className ? ' ' + className : '');
    ...
    return (
        <div
            ...
            key={rule}
            data-criteria={rule}
            data-invoked={!!strength}
            data-tip={!isFallback ? 'Click to deselect this rule' : null}
            onClick={onClick}>
            {rule}
            {strength && strength !== 'Default'
                ? '_' + (strengthNoSpaces || strength)
                : null}
        </div>
    );
});
...
function SVGeneInterpretationSubmitButton(props) {
    ...
    return (
        <div
            data-tip={
                !hasEditPermission
                    ? 'You must be added to the project to submit a note for this item.'
                    : null
            }>
            <Button
                variant="primary"
                className={'btn-block ' + cls}
                onClick={() =>
                    saveHighlightedGene(geneAtIDToGeneMap[selectedGeneID])
                }
                disabled={
                    !hasEditPermission || !highlightedGeneChangedSinceLastSave
                }>
                {!hasEditPermission
                    ? 'Need Edit Permission'
                    : 'Save Highlighted Gene'}
            </Button>
        </div>
    );
}
...
function GenericInterpretationSubmitButton(props) {
    ...
    const allButtonsDropsDisabled =
        !noteTextPresent || !noteChangedSinceLastSave || isFallback;

    const dataTip = isFallback
        ? 'Unable to retrieve most recent version of this note. Reload the page to try again.'
        : !hasEditPermission
        ? 'You must be added to the project to submit a note for this item.'
        : !noteTextPresent
        ? getTooltipPerNoteType(noteType)
        : null;
    ...
    if (isCurrent || isApproved || !hasEditPermission) {
        ...
        // Brand new draft OR previous draft; allow saving or re-saving as draft
        return (
            <div data-tip={dataTip}>
                <Button
                    variant="primary"
...
function UnsavedInterpretationModal(props) {
    ...
    const discardAndNavigate = useCallback(function () {
        setIsSubmitting(
            false,
            () => navigate(href, { skipRequest: false, replace: false }),
            false
        );
    });
    ...
    return (
        <React.Fragment>
            <Modal show={true} onHide={onHide} centered>
                <Modal.Header
                    closeButton
                    style={{ backgroundColor: '#bdcbd9', color: '#1e435e' }}>
                    <Modal.Title>
                        <div className="modal-title font-italic text-600 h4">
                            Variant Interpretation:{' '}
                            <span className="text-300">Unsaved Changes</span>
                        </div>
                    </Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <div className="text-small text-center mt-2">
                        This variant interpretation is <u>incomplete</u> and
                        contains at least 1:
                    </div>
                    <div
                        className="font-italic text-center text-600 h3 my-3"
                        style={{ color: '#1e435e' }}>
                        Unsaved Variant Classification
                    </div>
                    <div className="text-small text-center mb-2">
