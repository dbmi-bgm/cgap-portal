            ...
            <div className="col-4 col-md-3">
                <div className="pb-2 inner-card-section h-100">
                    ...
                    <div className="info-body text-center overflow-auto d-flex h-100 d-flex justify-content-center flex-column">
                        ...
                        <a
                            href={atID + '@@download'}
                            className="d-block pt-2 text-center mx-auto"
                            target="_blank"
    ...
    return (
        <div className="variant-tab-body card-body">
            <div className="row">
                <div
                    className={
                        bam_snapshot ? 'col-8 col-md-9' : 'col-12 col-md-12'
                    }>
            ...
            <div className="row">
                <div className="col-12 col-md-12 d-flex">
                    <div className="pb-2 w-100 inner-card-section">
                        ...
                        <div className="info-body overflow-auto">
                            <CoverageTable
                                {...{ samplegeno, genotypeLabels, varRef }}
                            />
                        </div>
                    </div>
                </div>
    ...
    return (
        <tr key={sampleID}>
            ...
            <td className="text-left">
                {refAD}{' '}
                {refAD > 0 && refAD != totalCoverage
                    ? ` (${Math.round((refAD / totalCoverage) * 100)}%)`
                    : null}
            </td>
            {altADs.map((ad, i) => (
                <td key={`${(i, ad)}`} className="text-left">
                    {ad || 0}
                    {ad > 0
                        ? ` (${Math.round((ad / totalCoverage) * 100)}%)`
                        : null}
                </td>
            ))}
...
const CoverageTable = React.memo(function CoverageTable({
    samplegeno = [],
    genotypeLabels = [],
    varRef,
}) {
    ...
    samplegeno.forEach((sg) => {
        ...
        if (!samplegenolen) {
            samplegenolen = adSplit.length;
        } // use for verifying col# later

        // If role/label data couldn't be populated by genotypeLabel sort, see if can populate here
        if (
            samplegeno_role &&
            !mapSampleIDToGenotypeLabel[samplegeno_sampleid]['role']
        ) {
            mapSampleIDToGenotypeLabel[samplegeno_sampleid]['role'] =
                samplegeno_role;
            ...
            if (
                labels &&
                !mapSampleIDToGenotypeLabel[samplegeno_sampleid]['labels']
            ) {
                mapSampleIDToGenotypeLabel[samplegeno_sampleid]['labels'] =
                    labels;
            }
        ...
        const row = {
            ...
            totalCoverage: adSplit.reduce(
                (pv, cv) => parseInt(pv) + parseInt(cv)
            ), // these are a little dangerous... but should always be an int...
    ...
    let refAndAltCols;
    if (samplegenolen === numCol) {
        // great, no further effort needed, just sort the keys and use them to populate the rows
        refAndAltCols = numGTKeys
            .sort((a, b) => a - b)
            .map((numGT, i) => {
                const thisGT = mapNumgtToGT[numGT];
                const { bpGT = null, shortGT = null } =
                    mapGTToShortenedTitles[thisGT];
                return (
                    <th key={i} data-tip={thisGT} className="text-left">{`${
                        numGT == 0 ? 'Ref' : 'Alt'
                    } (${bpGT || shortGT || thisGT})`}</th>
                );
            });
    } else {
        // looks like some columns were missing/incomplete, need to ensure # matches
        refAndAltCols = [];
        let emptyColCount = 1;
        for (let i = 0; i < samplegenolen; i++) {
            ...
            if (!mapNumgtToGT[i]) {
                ...
                if (i === 0) {
                    const { bpGT = null, shortGT = null } = shortenGT(
                        varRef,
                        2
                    );
                    refAndAltCols.push(
                        <th key={i} data-Tip={varRef} className="text-left">
                            Ref ({bpGT || shortGT || varRef})
                        </th>
                    );
                } else {
                    // apply placeholder name to empty alt column
                    refAndAltCols.push(
    ...
    return (
        <table className="w-100">
            ...
            <tbody>
                {rows.map((row) => (
                    <CoverageTableRow key={row.sampleID} {...row} />
                ))}
            </tbody>
        </table>
    );
            ...
            <tbody>
                ...
                <tr>
                    <td className="text-600 text-left">
                        Genotype Likelihoods(0/0,0/1,1/1)
                    </td>
    ...
    if (novoPP === null) {
        return (
            <span className="font-italic">
                Novo caller results are not provided for chrX, chrY, and chrM
                because these chromosomes do not meet model assumptions.
            </span>
        );
    }
    return (
...
const CompoundHetTable = React.memo(function CompoundHetTable(props) {
    ...
    if (cmphetArr.length === 0) {
        return (
            <span className="font-italic">
                No other variants on the same gene have passed the CGAP filter
                for rare exonic or splicing variants or clinvar variants.
            </span>
        );
    }
    ...
    function loadStatus(loadstatus) {
        switch (loadstatus) {
            case 'error':
                ...
                return (
                    <span data-tip="An error occurred while pulling this data. Reload page to try again.">
        ...
        <table className="w-100">
            <thead>
                <tr>
                    ...
                    <th
                        data-tip="Location on worst effect transcript"
                        className="text-left">
                        Location
                    </th>
                    <th
                        data-tip="Coding effect on worst effect transcript"
                        className="text-left">
                        Coding Effect
                    </th>
                </tr>
...
class CompoundHetTableWrapper extends React.Component {
    ...
    constructGetMatesQuery() {
        ...
        cmphetArr.forEach((cmphet) => {
            const { comhet_mate_variant: variant = null } = cmphet;
    ...
    getMateData() {
        ...
        return ajax
            .promise(encodedQueryString, 'GET', {})
            .then((response) => {
                ...
                this.setState(function ({ cmphetArr: prevCmphetArr = [] }) {
                    ...
                    if (responseItems.length > 0) {
                        // request succeeded and provided results
                        ...
                        responseItems.forEach(function (svObj) {
                            const { '@id': svAtId = null, variant = null } =
                                svObj;
                            ...
                            if (
                                variantToStateIndexMap[variantTitle] !=
                                undefined
                            ) {
                                const stateIndex =
                                    variantToStateIndexMap[variantTitle];
                                ...
                                genes.forEach(function (gene) {
                                    ...
                                    const associatedGene =
                                        variantToGeneMap[variantTitle];
                                    if (thisGene === associatedGene) {
                                        ...
                                        cmphetArr[stateIndex]['location'] =
                                            location;
                                        cmphetArr[stateIndex]['coding_effect'] =
                                            coding_effect;
                                    } else {
                                        cmphetArr[stateIndex]['location'] =
                                            null;
