...
import {
    ...
    SNVIndelInterpretationSpace,
} from './InterpretationSpaceController';
...
export class VariantSampleOverview extends React.PureComponent {
    ...
    componentDidUpdate(pastProps, pastState) {
        ...
        if (pastTranscriptIndex !== currentTranscriptIdx) {
            const currentGeneID = getCurrentTranscriptGeneID(
                context,
                currentTranscriptIdx
            );
            const pastGeneID = getCurrentTranscriptGeneID(
                pastContext,
                pastTranscriptIndex
            );
        ...
        return (
            <div className="sample-variant-overview sample-variant-annotation-space-body">
                <InterpretationController
                    ...
                    <VariantSampleInfoHeader
                        {...passProps}
                        onSelectTranscript={this.onSelectTranscript}
                    />
...
function getCurrentTranscriptGeneID(context, transcriptIndex) {
    ...
    const { csq_gene: { '@id': geneID = null } = {} } =
        geneTranscriptList[transcriptIndex] || {};
    return geneID;
}
        ...
        VariantSampleOverviewTabView.tabNames.forEach((title, index) => {
            ...
            if (index === annotationTab || this.openPersistentTabs[index]) {
                ...
                switch (index) {
                    ...
                    case 2:
                        tabBodyElements.push(
                            <SampleTabBody {...commonBodyProps} />
                        );
                        break;
                    case 3:
                        tabBodyElements.push(
                            <AnnotationBrowserTabBody {...commonBodyProps} />
                        );
                        this.openPersistentTabs[3] = true; // Persist open after first appearance.
                        break;
                    case 4:
                        tabBodyElements.push(
                            <BamFileBrowserTabBody {...commonBodyProps} />
                        );
...
class InterpretationController extends React.PureComponent {
    constructor(props) {
        ...
        this.memoized = {
            flattenGlobalACMGStateIntoArray: memoize(
                acmgUtil.flattenStateMapIntoArray
            ),
        };
    ...
    componentDidUpdate(pastProps) {
        ...
        const { newContext: pastNC = null, newVSLoading: pastVSLoadStatus } =
            pastProps;
    ...
    initializeACMGFromContext() {
        ...
        if (newContext) {
            // if new context is loaded in
            const { interpretation = {} } = newContext;
            acmg_rules_invoked = interpretation.acmg_rules_invoked || [];
        } else {
            // not successfully loaded in; default to old context
            const { interpretation = {} } = context || {};
            acmg_rules_invoked = interpretation.acmg_rules_invoked || [];
        }

        // Initialize classifier and prepare new state
        const acmgSelections =
            acmgUtil.criteriaArrayToStateMap(acmg_rules_invoked); // object that maps { rule: strength }
    ...
    invokeAtStrength(criteria, callback) {
        ...
        const { acmg_rule_name: rule = criteria, rule_strength: strength } =
            criteria;
        ...
        const newState = {
            globalACMGSelections: newInvocations,
            autoClassification: classification,
        };
        this.setState(newState, () =>
            callback ? callback(newState) : undefined
        );
    }
    ...
    toggleInvocation(criteria, callback) {
        ...
        if (newInvocations[rule] !== undefined) {
            // already set (may have strength)
            ...
            // first time setting (won't have strength)
    ...
    render() {
        const { showACMGInvoker, globalACMGSelections, autoClassification } =
            this.state;
        const {
            newVSLoading,
            newContext = null,
            ...
            schemas,
            children,
            showInterpretation: showInterpretationFromQuery,
            interpretationTab,
            href,
            caseSource,
            setIsSubmitting,
        ...
        const wipACMGSelections =
            this.memoized.flattenGlobalACMGStateIntoArray(globalACMGSelections);

        const showInterpretationSpace =
            showInterpretationFromQuery &&
            !anyNotePermErrors &&
            newContext &&
            !newVSLoading;
        console.log(
            `showInterpretation:${showInterpretationFromQuery}, anyNotePermErrors: ${anyNotePermErrors}, newContext: ${!!newContext}, newVSLoading: ${newVSLoading}`
        );
        ...
        return (
            <React.Fragment>
                <Collapse in={!!(showACMGInvoker && newContext)}>
                    <div>
                        {/** Collapse seems not to work without wrapper element */}
                        <ACMGInvoker
                            invokedFromSavedNote={acmg_rules_invoked}
                            {...{ globalACMGSelections }}
                            toggleInvocation={this.toggleInvocation}
                            invokeAtStrength={this.invokeAtStrength}
                        />
                    </div>
                </Collapse>
                <div className="row flex-column-reverse flex-lg-row flex-nowrap">
                    ...
                    {showInterpretationFromQuery && newVSLoading ? (
                        <LoadingInterpretationSpacePlaceHolder headerTitle="SNV / Indel Interpretation Space" />
                    ) : null}
                    {showInterpretationSpace ? (
                        <div className="col flex-grow-1 flex-lg-grow-0 interpretation-space-wrapper-column">
                            <SNVIndelInterpretationSpace
                                {...{ autoClassification, actions }}
                                context={newContext}
                                toggleInvocation={this.toggleInvocation}
                                wipACMGSelections={wipACMGSelections}
                                {...passProps}
                                toggleACMGInvoker={this.toggleACMGInvoker}
...
export const LoadingInterpretationSpacePlaceHolder = React.memo(
    function LoadingInterpretationSpacePlaceHolder(headerTitle) {
        return (
            <div className="col flex-grow-1 flex-lg-grow-0 interpretation-space-wrapper-column">
...
const ACMGInvoker = React.memo(function ACMGInvoker(props) {
    const {
        globalACMGSelections: invoked = {},
        toggleInvocation,
        invokeAtStrength,
    } = props || {};

    const [acmgStrengthPopover, setACMGStrengthPopover] = useState(null);
    const { target: targetIndicatorRef, jsx: acmgStrengthPopoverJSX } =
        acmgStrengthPopover || {};

    const onRootClickHide = useCallback(
    ...
    return (
        <div className="card flex-row my-3 mt-0">
            <div className="text-600 acmg-guidelines-title">
                ACMG Rules
                <QuickPopover
                    className="p-1"
                    popID="acmg-info-popover"
                    title="Note on ACMG Tooltips and Auto-Classification"
                    placement="right"
                    tooltip="Click for citation info">
                    <div>
                        <div className="mb-05">
                            The algorithm used to autoclassify variants based on
                            ACMG rules, and the information contained within the
                            ACMG tooltips is based on{' '}
                            <a
                                href="https://rdcu.be/cloqS"
                                target="_blank"
                                rel="noreferrer">
                                this publication
                            </a>
                        ...
                        <div>
                            <u>Full Citation</u>: Richards, S., Aziz, N., Bale,
                            S. et al. Standards and guidelines for the
            ...
            {acmgStrengthPopover ? (
                <Overlay
                    target={targetIndicatorRef}
                    show={!!acmgStrengthPopover}
                    transition={true}
                    placement="bottom"
                    rootClose
                    rootCloseEvent="click"
                    onHide={onRootClickHide}>
...
const ACMGInvokableRule = React.memo(function ACMGInvokableRule(props) {
    ...
    const toggleRuleStrengthOptionsPopover = useCallback(
        function (newState) {
            const { globalACMGSelections: { [rule]: newStrength } = {} } =
                newState;
            if (!acmgStrengthPopover) {
                setACMGStrengthPopover({
                    target: thisRef,
                    jsx: generateACMGRulePopover(
                        rule,
                        newStrength,
                        invokeAtStrength,
                        setACMGStrengthPopover
    ...
    const onClick = useCallback(
        function (e) {
            return toggleInvocation(
                { acmg_rule_name: rule, rule_strength: strength },
                toggleRuleStrengthOptionsPopover
            );
        },
        [toggleRuleStrengthOptionsPopover, strength]
    ); // 'rule' is already compared in toggleRuleStrengthOptionsPopover useCallback wrapper.

    return (
        <div
            ref={thisRef}
            className="acmg-invoker clickable ml-02 mr-02 flex-grow-1"
            key={rule}
            data-criteria={rule}
            data-invoked={!!strength}
            onClick={onClick}
            data-html
            data-tip={acmgTip(rule, description)}>
            {rule}
        </div>
    );
});
...
function calculateACMGRuleStrengthOptions(rule, selectedStrength) {
    ...
    const currStrength =
        selectedStrength === undefined || selectedStrength === 'Default'
            ? defaultRuleStrength
            : selectedStrength;
    ...
    possibleStrengths.forEach((strength) => {
        ...
        if (strength === defaultRuleStrength) {
            // if default strength for the rule
            optionData.defaultStr = true;
        }
        if (strength === currStrength) {
            // if currently selected strength for the rule
...
function generateACMGRulePopover(
    rule,
    selectedStrength,
    invokerFx,
    setACMGStrengthPopoverFx
) {
    const strengthOptions = calculateACMGRuleStrengthOptions(
        rule,
        selectedStrength
    );
    ...
    return (
        <Popover id={'acmg-strength-pop-' + rule}>
            <Popover.Title className="m-0" as="h4">
                Select ACMG Rule Strength
            </Popover.Title>
                ...
                <div className="list-group list-group-flush acmg-popover-strengths">
                    {strengthOptions.map((options) => {
                        ...
                        if (strengthOption === 'Very Strong') {
                            strengthOptionNoSpaces = strengthOption
                                .split(' ')
                                .join('');
                        }
                            ...
                            <button
                                ...
                                onClick={() =>
                                    invokerFx(
                                        {
                                            acmg_rule_name: rule,
                                            rule_strength: defaultStr
                                                ? 'Default'
                                                : strengthOption,
                                        },
                                        () => setACMGStrengthPopoverFx(null)
                                    )
                                }
                                key={strengthOption}
                                className={`list-group-item list-group-item-action py-2 text-600 ${
                                    selected ? 'active disabled' : ''
                                }`}
                                data-criteria={rule}
                                data-invoked={selected}>
                                {rule}
                                {defaultStr
                                    ? null
                                    : '_' +
                                      (strengthOptionNoSpaces ||
                                          strengthOption)}
                            </button>
                        );
                    })}
