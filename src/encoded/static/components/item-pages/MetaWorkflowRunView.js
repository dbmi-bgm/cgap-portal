...
} from '@hms-dbmi-bgm/react-workflow-viz';
...
export function checkIfIndirectOrReferenceNodesExist(steps) {
    const graphData = parseAnalysisSteps(steps, {
    ...
    const graphProps = {
        ...
        renderNodeElement: function (node, graphProps) {
            return (
                <WorkflowNodeElement
                    {...graphProps}
                    {..._.pick(props, 'schemas', 'windowWidth')}
                    node={node}
                />
            );
        },
        ...
        nodeClassName: null,
        onNodeClick:
            typeof props.onNodeClick !== 'undefined' ? props.onNodeClick : null,
...
export default class MetaWorkflowRunView extends DefaultItemView {
    ...
    getTabViewContents() {
        ...
        const tabs = [
            {
                tab: (
                    <span>
                        <i className="icon icon-sitemap icon-rotate-90 fas icon-fw" />{' '}
                        Graph
                    </span>
                ),
                key: 'graph',
        ...
        const tabContents = _.map(
            tabs.concat(this.getCommonTabs()),
            (
                tabObj // Common properties
            ) =>
                _.extend(tabObj, {
                    style: {
                        minHeight: Math.max(
                            (mounted && windowHeight && windowHeight - 300) ||
                                0,
                            600
                        ),
                    },
...
export function transformMetaWorkflowRunToSteps(metaWorkflowRunItem) {
    ...
    const incompleteSteps = combinedMWFRs.map(function (workflowRunObject) {
        ...
        const inputFileObjectsGroupedByArgName = _.groupBy(
            wfrItemInputFileObjects,
            'workflow_argument_name'
        );
        const inputParametersGroupedByArgName = _.groupBy(
            wfrItemInputParameters,
            'workflow_argument_name'
        );
        const outputFileObjectsGroupedByArgName = _.groupBy(
            wfrItemOutputFileObjects,
            'workflow_argument_name'
        );
        ...
        input.forEach(function (wfrObjectInputObject) {
            ...
            const filesForThisInput =
                inputFileObjectsGroupedByArgName[argument_name] || [];
            const filesLen = filesForThisInput.length;

            const parametersForThisInput =
                inputParametersGroupedByArgName[argument_name] || [];
            ...
            if (filesLen > 0) {
                filesForThisInput.forEach(function (fileObject) {
                    ...
                    const { '@id': fileAtID, workflow_run_outputs = [] } =
                        fileItem || {};
                    const [{ '@id': outputOfWFRAtID } = {}] =
            ...
            if (filesLen > 0) {
                stepInputObject.run_data.file = _.pluck(
                    filesForThisInput,
                    'value'
                );
                stepInputObject.run_data.meta = filesForThisInput.map(
                    function ({ value, ...remainingProperties }) {
                        return remainingProperties;
                    }
                );
            } else if (paramsLen > 0) {
                // WorkflowViz supports only 1 parameter per input argument at time being.
                const [firstParameterObject] = parametersForThisInput;
                const { value, ...remainingFirstParameterObjectProperties } =
                    firstParameterObject;
                // We can have multiple values but only 1 'meta' object.
                stepInputObject.run_data.value = _.pluck(
                    parametersForThisInput,
                    'value'
                );
                stepInputObject.run_data.meta =
                    remainingFirstParameterObjectProperties;
            }
        ...
        output.forEach(function (wfrOutputObject) {
            ...
            const [outputFileObject] =
                outputFileObjectsGroupedByArgName[argument_name] || [];
            ...
            if (outputFileObject) {
                ...
                const { '@id': fileAtID, workflow_run_inputs = [] } =
                    fileItem || {};
            ...
            if (outputFileObject) {
                ...
                stepOutputObject.run_data.meta = [
...
function MetaWorkflowRunDataTransformer(props) {
    ...
    const steps = useMemo(
        function () {
            return identifiersToIntegers(
                transformMetaWorkflowRunToSteps(context)
            );
        },
        [context]
    );
...
export class WorkflowGraphSection extends React.PureComponent {
    constructor(props) {
        ...
        this.onToggleShowParameters = _.throttle(
            this.onToggleShowParameters.bind(this),
            1000
        );
        this.onToggleReferenceFiles = _.throttle(
            this.onToggleReferenceFiles.bind(this),
            1000
        );
        this.onToggleIndirectFiles = _.throttle(
            this.onToggleIndirectFiles.bind(this),
            1000
        );
        this.onChangeRowSpacingType = _.throttle(
            this.onChangeRowSpacingType.bind(this),
            1000,
            { trailing: false }
        );
        ...
        this.memoized = {
            ...
            checkIfIndirectOrReferenceNodesExist: memoize(
                checkIfIndirectOrReferenceNodesExist
            ),
        };
    ...
    parseAnalysisSteps(steps) {
        const { showReferenceFiles, showParameters, showIndirectFiles } =
            this.state;
    ...
    commonGraphProps() {
        ...
        const { showParameters, showReferenceFiles, rowSpacingType } =
            this.state;
        ...
        const legendItems = _.pick(
            WorkflowDetailPane.Legend.defaultProps.items,
            keepItems
        );
        const commonGraphProps = commonGraphPropsFromProps({
            ...this.props,
            legendItems,
        });
        return {
    ...
    render() {
        ...
        const { anyIndirectPathIONodes, anyReferenceFileNodes } =
            this.memoized.checkIfIndirectOrReferenceNodesExist(context.steps);
            ...
            <div className="tabview-container-fullscreen-capable meta-workflow-view-container meta-workflow-viewing-detail">
                <h3 className="tab-section-title container-wide">
                    ...
                    <WorkflowGraphSectionControls
                        ...
                        isReferenceFilesCheckboxDisabled={
