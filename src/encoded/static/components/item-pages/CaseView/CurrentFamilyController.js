...
export function findCanonicalFamilyIndex(
    familyList,
    canonicalFamilyPartialItem
) {
    const { uuid: canonicalFamilyUUID } = canonicalFamilyPartialItem || {};
    if (!canonicalFamilyUUID) {
        throw new Error(
            'No canonical family UUID provided. Check view permissions?'
        );
    }
...
export class CurrentFamilyController extends React.PureComponent {
    ...
    static filterFamiliesWithViewPermission(spFamilies) {
        ...
        return spFamilies.filter(
            CurrentFamilyController.haveFullViewPermissionForFamily
        );
    }
    ...
    constructor(props) {
        ...
        this.handleFamilySelect = _.throttle(
            this.handleFamilySelect.bind(this),
            1000
        );

        this.memoized = {
            haveFullViewPermissionForFamily: memoize(
                CurrentFamilyController.haveFullViewPermissionForFamily
            ),
            filterFamiliesWithViewPermission: memoize(
                CurrentFamilyController.filterFamiliesWithViewPermission
            ),
        ...
        const familiesWithViewPermission =
            this.memoized.filterFamiliesWithViewPermission(spFamilies);
        const canonicalFamilyIdx = this.memoized.findCanonicalFamilyIndex(
    ...
    render() {
        const { children, context, PedigreeVizLibrary, ...passProps } =
            this.props;
        const {
            family: canonicalFamilyPartialEmbed,
            sample_processing: { families: spFamilies = [] } = {},
        ...
        if (familiesLen > 0) {
            ...
            currPedigreeFamily =
                familiesWithViewPermission[pedigreeFamiliesIdx];
        }
...
export function FamilyItemParser(props) {
    ...
    const { buildPedigreeGraphData = null, isRelationshipNode } =
        PedigreeVizLibrary || {};
    ...
    const graphData = useMemo(
        function () {
            if (
                ...
                typeof buildPedigreeGraphData !== 'function'
            )
                return null;
    ...
    const idToGraphIdentifier = useMemo(
        function () {
            if (!graphData || !isRelationshipNode) return {};
            return calculateIdToGraphIdentifier(
