...
function mapFeaturesToBadges(features = []) {
    ...
    return features.map(function (feature) {
        ...
        return (
            ...
            <a
                className="badge badge-pill badge-info text-400 text-small d-inline-block mr-03 mb-03 pb-04"
                href={featureID}
                target="_blank"
...
export const CaseStats = React.memo(function CaseStats(props) {
    ...
    const { phenotypic_features = [], '@id': individualAtID = null } =
        individual || {};
    const { family_phenotypic_features = [], '@id': familyAtID = null } =
        canonicalFamily || {};

    const renderedPatientPhenotypicFeatures = useMemo(
        function () {
            const onlyPhenotypicFeatures = phenotypic_features.map(
                (feature) => {
    ...
    const renderedFamilyPhenotypicFeatures = useMemo(
        function () {
            ...
            <div className="col-12 col-sm mb-2 mb-sm-0">
                <div className="card h-100">
                    <div className="card-header primary-header d-flex align-items-center">
                        ...
                        {haveCaseEditPermission && individualAtID ? (
                            <a
                                href={
                                    individualAtID +
                                    '?currentAction=edit&callbackHref=' +
                                    caseAtID
                                }
                                className="text-white-50 ml-12 text-small"
                                data-tip="Edit Individual. Changes may take a few minutes to appear.">
                    ...
                    <div className="card-body">
                        <PatientInfo
                            {...{ caseItem, haveCaseEditPermission }}
                        />
                    </div>
                    <div className="card-footer">
                        <label className="py-1 mb-0 text-large">
                    ...
                    <div className="card-header primary-header d-flex align-items-center">
                        ...
                        {haveCaseEditPermission && familyAtID ? (
                            <a
                                ...
                                className="text-white-50 ml-12 text-small"
                                data-tip="Edit Family. Changes may take a few minutes to appear.">
                                <i className="icon icon-fw icon-pencil-alt fas" />
                            </a>
                        ) : null}
                    </div>
...
export const PatientInfo = React.memo(function PatientInfo({
    caseItem = null,
}) {
    const fallbackElem = (
    ...
    if (!individualAtID) {
        return (
            <div className="text-center text-italic">No view permissions</div>
        );
    }
    ...
    return (
        <React.Fragment>
            ...
            <div className="card-text mb-1">
                ...
                <span className="text-monospace text-small">
                    {familyAccession}
                </span>
            </div>
            {/* <div className="card-text mb-1">
