...
export class VariantSampleListController extends React.PureComponent {
     ...
     */
    static activeVariantSampleIDMap(
        variant_samples,
        ...
        function addToRetDict(vsSelection) {
            const {
                variant_sample_item: { '@id': vsAtID = null },
            } = vsSelection;
            if (!vsAtID) {
    ...
    constructor(props) {
        ...
        this.fetchVariantSampleListItem =
            this.fetchVariantSampleListItem.bind(this);
        this.updateVariantSampleListID =
            this.updateVariantSampleListID.bind(this);
        this.windowMessageEventListener =
            this.windowMessageEventListener.bind(this);
        this.updateVariantSampleListSort =
            this.updateVariantSampleListSort.bind(this);
        const { id: vslID } = props;
        this.state = {
            fetchedVariantSampleListItem: null,
            variantSampleListID: typeof vslID === 'string' ? vslID : null,
            isLoadingVariantSampleListItem:
                typeof vslID === 'string' ? true : false,
        ...
        this.memoized = {
            activeVariantSampleIDMap: memoize(
                VariantSampleListController.activeVariantSampleIDMap
            ),
        };
    ...
    componentDidMount() {
        ...
        WindowEventDelegator.addHandler(
            'message',
            this.windowMessageEventListener
        );
    }
    ...
    fetchVariantSampleListItem(fnCallback = null) {
        ...
        const vslFetchCallback = (resp) => {
            ...
            if (!Array.isArray(resp)) {
                ...
                throw new Error(
                    `Couldn't get VSL - malformed response, check if VSL with ID \`${variantSampleListID}\` exists.`
                );
            }
            ...
            this.setState(
                function ({
                    refreshCount: prevRefreshCount,
                    fetchedVariantSampleListItem: prevItem,
                }) {
    ...
    render() {
        ...
        const {
            fetchedVariantSampleListItem: variantSampleListItem,
            isLoadingVariantSampleListItem,
            sortType: vslSortType,
        } = this.state;
        const { variant_samples = [], structural_variant_samples = [] } =
            variantSampleListItem || {};
        const childProps = {
            ...passProps,
            vslSortType,
            variantSampleListItem,
            isLoadingVariantSampleListItem,
            savedVariantSampleIDMap: this.memoized.activeVariantSampleIDMap(
                variant_samples,
                structural_variant_samples
            ),
...
function commonVSEmbeds(prefix) {
    return [
        ...
        prefix +
            '.variant_sample_item.associated_genotype_labels.proband_genotype_label',
        prefix +
            '.variant_sample_item.associated_genotype_labels.mother_genotype_label',
        prefix +
            '.variant_sample_item.associated_genotype_labels.father_genotype_label',

        // Notes
        ...commonNoteEmbeds(prefix + '.variant_sample_item.interpretation'),
        prefix + '.variant_sample_item.interpretation.classification',
        ...commonNoteEmbeds(
            prefix + '.variant_sample_item.discovery_interpretation'
        ),
        prefix + '.variant_sample_item.discovery_interpretation.gene_candidacy',
        prefix +
            '.variant_sample_item.discovery_interpretation.variant_candidacy',
