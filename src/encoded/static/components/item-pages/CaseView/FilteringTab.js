...
} from './CaseViewEmbeddedVariantSampleSearchTable';
        ...
        <React.Fragment>
            <div className="row flex-column-reverse flex-md-row align-items-center">
                ...
                <div className="col-12 col-md-auto my-3 my-md-n3">
                    <FilteringTabTableToggle
                        {...{ currViewIdx, setCurrViewIdx, context }}
                    />
                </div>
            </div>
            <div
                id="snv-filtering"
                className={'mt-36' + (currViewIdx === 0 ? '' : ' d-none')}>
            ...
            <div
                id="cnvsv-filtering"
                className={'mt-36' + (currViewIdx === 1 ? '' : ' d-none')}>
                <TechnicalReviewController>
                    <SelectedItemsController isMultiselect>
...
const FilteringTabTableToggle = React.memo(function FilteringTabTableToggle(
    props
) {
    ...
    const options = useMemo(
        function () {
            ...
            if (analysis_type.startsWith('WES')) {
                cnvTip =
                    'SV/CNV is currently available for only germline WGS data. Additional pipelines are under development.';
            }
            return [
                {
                    onClick: onClickSNV,
...
export function InnerTabToggle(props) {
    ...
    return <div className={'card ' + cardCls}>{renderedOptions}</div>;
}
...
function SNVFilteringTabBody(props) {
    ...
    const { searchHrefBase, blankFilterSetItem, hideFacets } = useMemo(
        function () {
            const {
            ...
            const blankFilterSetItem = createBlankFilterSetItem(
                searchType,
                caseAccession
            );
            ...
            if (initial_search_href_filter_addon) {
                hideFacets = hideFacets.concat(
                    Object.keys(
                        queryString.parse(initial_search_href_filter_addon)
                    )
                );
        ...
        [context]
    ); // Don't memoize on `searchType`; it never changes.

    return (
        // TODO: Maybe rename `activeFilterSetFieldName` to `caseActiveFilterSetFieldName`.. idk.
        <FilteringTabBody
            {...props}
            {...{ searchHrefBase, hideFacets, blankFilterSetItem, searchType }}
    ...
    const { searchHrefBase, blankFilterSetItem, hideFacets } = useMemo(
        function () {
            ...
            if (sv_initial_search_href_filter_addon) {
                hideFacets = hideFacets.concat(
                    Object.keys(
                        queryString.parse(sv_initial_search_href_filter_addon)
                    )
                );
                blankFilterSetItem.flags = [
                    {
...
function FilteringTabBody(props) {
    ...
    const searchHrefBase =
        ...
        (additional_variant_sample_facets.length > 0
            ? '&' +
              additional_variant_sample_facets
                  .map(function (fac) {
                      return 'additional_facet=' + encodeURIComponent(fac);
                  })
                  .join('&')
            : '') +
        '&sort=date_created';
    ...
    const { onClearFiltersVirtual, isClearFiltersBtnVisible } = useMemo(
        function () {
            const onClearFiltersVirtual = function (
                virtualNavigateFxn,
                callback
            ) {
            ...
            const isClearFiltersBtnVisible = function (virtualHref) {
                ...
                return VirtualHrefController.isClearFiltersBtnVisible(
                    virtualHref,
                    searchHrefBase
                );
            };
    ...
    const onFailInitialFilterSetItemLoad = useCallback(
        function () {
            if (session) {
    ...
    const embeddedTableHeader = activeFilterSetID ? (
        <ajax.FetchedItem
            atId={activeFilterSetID + '?datastore=database'}
            fetchedItemPropName="initialFilterSetItem"
            isFetchingItemPropName="isFetchingInitialFilterSetItem"
        ...
        <FilterSetController
            {...fsControllerProps}
            initialFilterSetItem={blankFilterSetItem}>
            {embeddedTableHeaderBody}
        </FilterSetController>
    );
    ...
    const currFilterSet = filterSetControllerInstance
        ? filterSetControllerInstance.state.currFilterSet
        : null;
    ...
    const maxHeight =
        typeof windowHeight === 'number' && windowHeight > 845
            ? windowHeight - 142
            : undefined;
    ...
    if (Array.isArray(children)) {
        throw new Error(
            'Expected single React Component (EmbeddedSearchView or a composed component eventually returning EmbeddedSearchView)'
        );
    }
