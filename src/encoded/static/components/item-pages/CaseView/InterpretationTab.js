...
export class InterpretationTabController extends React.Component {
    ...
    constructor(props) {
        super(props);
        this.toggleVariantSampleSelectionDeletion =
            this.toggleVariantSampleSelectionDeletion.bind(this);
        this.toggleStructuralVariantSampleSelectionDeletion =
            this.toggleStructuralVariantSampleSelectionDeletion.bind(this);
        this.resetVariantSampleSelectionDeletionsAndOrdering =
            this.resetVariantSampleSelectionDeletionsAndOrdering.bind(this);
        this.state = {
            deletedVariantSampleSelections: {},
            deletedStructuralVariantSampleSelections: {},
        };
        this.memoized = {
            deletionsLen: memoize(function (deletedVariantSampleSelections) {
                return Object.keys(deletedVariantSampleSelections).length;
            }),
        };
    }
    ...
    toggleStructuralVariantSampleSelectionDeletion(vsUUIDToDelete) {
        this.setState(function ({ deletedStructuralVariantSampleSelections }) {
            ...
            return {
                deletedStructuralVariantSampleSelections: nextDeletedVSes,
            };
        });
    }
    ...
    render() {
        ...
        const snvDeletionsLen = this.memoized.deletionsLen(
            deletedVariantSampleSelections
        );
        const cnvDeletionsLen = this.memoized.deletionsLen(
            deletedStructuralVariantSampleSelections
        );
            ...
            <div className="d-flex align-items-center justify-content-between mb-2">
                <h1 className="text-300 mb-0">
                    ...
                    {isLoadingVariantSampleListItem ? (
                        <i className="icon icon-circle-notch icon-spin fas text-muted ml-12" />
                    ) : null}
                </h1>
                <div className="d-block d-md-flex">
                    <ExportInterpretationSpreadsheetButton
                        {...{ variantSampleListItem }}
                        disabled={anyUnsavedChanges}
                        className="mr-08"
                    />
                    <SaveVariantSampleListItemDeletionsAndOrderingButton
                        {...{
                            variantSampleListItem,
                            deletedVariantSampleSelections,
                            deletedStructuralVariantSampleSelections,
...
function SaveVariantSampleListItemDeletionsAndOrderingButton(props) {
    ...
    const patchVariantSampleListItem = useCallback(
        function (e) {
            const {
                '@id': vslAtID,
                variant_samples: originalVariantSamplesList = [],
                structural_variant_samples:
                    originalStructuralVariantSamplesList = [],
            } = variantSampleListItem;
            function handleResponse(resp) {
                ...
                if (status !== 'success') {
                    ...
                    Alerts.queue({
                        ...
                        message:
                            'Perhaps no edit permission for this VariantSampleList or Case.',
                        style: 'danger',
                    });
                    setIsPatching(false);
                    return;
                ...
                return originalList
                    .filter(function (vsSelection) {
                        ...
                        if (structural_variant) {
                            return !deletedStructuralVariantSampleSelections[
                                vsUUID
                            ];
                        }
                    ...
                    .map(function (vsSelection) {
                        ...
                        const {
                            variant_sample_item: { uuid },
                        } = vsSelection;
                        return uuid;
                    });
            }

            const variant_samples = createSelectionListPayload(
                originalVariantSamplesList
            );
            const structural_variant_samples = createSelectionListPayload(
                originalStructuralVariantSamplesList
            );
            ...
            if (!hasPermissionToViewAll) {
                Alerts.queue({
                    ...
                    message:
                        'Lack permissions to view at least one item in this list. Cannot proceed.',
                    style: 'danger',
                });
                return;
            }
            ...
            ajax.load(
                vslAtID + '@@order-delete-selections',
                handleResponse,
                'PATCH',
                handleResponse,
                JSON.stringify({ variant_samples, structural_variant_samples })
            );
        },
        [
            variantSampleListItem,
    ...
    const btnCls =
        'btn d-flex align-items-center btn-' +
        (deletionsPresent ? 'danger' : 'primary');
    ...
    return (
        <div className="btn-group" role="group">
            <button
                ...
                className={btnCls}
                disabled={disabled}
                onClick={patchVariantSampleListItem}>
                {iconCls ? (
                    <i className={'icon icon-fw mr-08 icon-' + iconCls} />
                ) : null}
            ...
            <button
                ...
                disabled={disabled}
                onClick={resetVariantSampleSelectionDeletionsAndOrdering}>
...
const ExportInterpretationSpreadsheetButton = React.memo(
    function ExportInterpretationSpreadsheetButton({
        variantSampleListItem,
        disabled,
        className,
    }) {
        // const { accession: caseAccession } = context; // Case Item
        const { '@id': atId, variant_samples: vsObjects = [] } =
            variantSampleListItem || {};
        const baseHref = atId + '/@@spreadsheet/?file_format=';
        return (
            <DropdownButton
                variant="outline-primary"
                disabled={disabled || vsObjects.length === 0}
                className={className}
                title={
                    <span>
