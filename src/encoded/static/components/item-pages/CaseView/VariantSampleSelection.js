...
function getVariantSampleListGroupedByGene(cnvSelections, vsSelections) {
    ...
    allSelections.forEach((selection) => {
        ...
        if (geneUUID && geneUUIDToVariantMap[geneUUID]) {
            ...
            highlightedGeneUUID &&
            geneUUIDToVariantMap[highlightedGeneUUID]
        ) {
            ...
            if (geneUUIDToVariantMap['No Gene']) {
                ...
                geneUUIDToDisplayTitleMap['No Gene'] =
                    'No Highlighted Gene Selected';
            }
        }
    });
 ...
 */
export const VariantSampleSelectionList = React.memo(
    function VariantSampleSelectionList(props) {
        const {
            schemas,
            context,
        ...
        const { snvFacetDict, cnvFacetDict } = useMemo(
            function () {
                const fullSchemaFacetDict = buildSchemaFacetDictionary(schemas);
                const {
                    VariantSample: snvFacetDict = {},
                    StructuralVariantSample: cnvFacetDict = {},
                } = fullSchemaFacetDict;
        ...
        const renderSelectionAsJSX = useCallback(
            function (selection, index) {
                ...
                const unsavedClassification = changedClassificationsByVS
                    ? changedClassificationsByVS[vsUUID]
                    : undefined;
                ...
                if (vsType === 'VariantSample') {
                    isDeleted = deletedVariantSampleSelections
                        ? deletedVariantSampleSelections[vsUUID] || false
                        : undefined;
                    facetDict = snvFacetDict;
                } else if (vsType === 'StructuralVariantSample') {
                    isDeleted = deletedStructuralVariantSampleSelections
                        ? deletedStructuralVariantSampleSelections[vsUUID] ||
                          false
                        : undefined;
                    facetDict = cnvFacetDict;
                }
                return (
                    <VariantSampleSelection
                        {...commonProps}
                        key={vsUUID || index}
                        searchType={vsType}
                        {...{
        ...
        return (
            <div className="row">
                <div className="col-12 align-center justify-center">
                    <VariantSampleListSortSelectDrop
                        {...{ updateVariantSampleListSort, vslSortType }}
                    />
                </div>
                {vslSortType === 'Variant' ? (
                    <VSLSortedByVariantType
                        {...{
                            vsSelections,
                            cnvSelections,
                            renderSelectionAsJSX,
                        }}
                    />
                ) : (
                    <VSLSortedByGeneType
        ...
        <>
            {!!vsSelections.length && (
                <div className="col-12">
                    <h2 className="mb-05 text-600">
                        SNV / Indel - {vsSelections.length} Variant(s)
                    </h2>
            ...
            {!!cnvSelections.length && (
                <div className="col-12">
                    <h2 className="mb-05 text-600">
                        CNV / SV - {cnvSelections.length} Variant(s)
                    </h2>
                    <hr className="mb-2 mt-0" />
                    {cnvOptions}
                </div>
...
const VSLSortedByGeneType = React.memo(function VSLSortedByGeneType(props) {
    ...
    const { genes, displayTitleMap, geneUUIDToVariantMap } = useMemo(
        function () {
            const { displayTitleMap, ...groupedByGene } =
                getVariantSampleListGroupedByGene(cnvSelections, vsSelections);
            ...
            const genes = Object.keys(geneUUIDToVariantMap).sort(
                (geneUUID, nextGeneUUID) => {
                    ...
                    return (
                        displayTitleMap[geneUUID] >
                        displayTitleMap[nextGeneUUID]
                    );
                }
            );
    ...
    return genes.map(function (geneUUID, i) {
        ...
        return (
            <div className="col-12" key={i}>
                <h2 className="mb-05 text-600">
                    {displayTitleMap[geneUUID]} - {geneSelections.length}{' '}
                    Variant(s)
                </h2>
                <hr className="mb-2 mt-0" />
...
function transformSVDisplayTitle(svs) {
    const {
        structural_variant: { END, START, CHROM, SV_TYPE, size_display } = {},
    } = svs || {};
    return `${SV_TYPE} chr${CHROM}:${decorateNumberWithCommas(
        START
    )} - ${decorateNumberWithCommas(END)} [${size_display}]`;
}
...
const {
    'structural_variant.worst_transcript': {
        render: worstTranscriptRenderFunc,
    },
} = structuralVariantSampleColumnExtensionMap;

export const VariantSampleSelection = React.memo(
    function VariantSampleSelection(props) {
        const {
        ...
        const {
            ...
            selected_by: { display_title: selectedByUserDisplayTitle },
        } = selection;

        const toggleSelectedVSDeletionFx =
        ...
        const {
            ...
            uuid: vsUUID,
            variant: {
                display_title: snvVariantDisplayTitle,
                genes: [firstGene = null] = [],
            } = {},
        ...
        const {
            ...
            function () {
                ...
                let lastModifiedInfo =
                    vsLastModified ||
                    (vsDateCreated ? { date_modified: vsDateCreated } : null);

                notes.forEach(function ({ uuid: noteUUID, last_modified }) {
                    if (
                        alreadyInReportNotes &&
                        alreadyInReportNotes[noteUUID]
                    ) {
                        countNotesInReport++;
                    }
                    if (
                        alreadyInProjectNotes &&
                        alreadyInProjectNotes[noteUUID]
                    ) {
                        countNotesInKnowledgeBase++;
                    }
                    if (
                        ...
                        last_modified.date_modified >
                            lastModifiedInfo.date_modified
                    ) {
        ...
        const noSavedNotes =
            clinicalInterpretationNote === null &&
            discoveryInterpretationNote === null &&
            lastVariantNote === null &&
            lastGeneNote === null;
        ...
        if (isExpanded) {
            ...
            expandedNotesSection = (
                <VariantSampleExpandedNotes {...noteSectionProps} />
            );
        }
        ...
        const variantDisplayTitle = variantIsSNV
            ? snvVariantDisplayTitle
            : transformSVDisplayTitle(variantSample);

        return (
            <div
                className="card mb-16 variant-sample-selection"
                        ...
                        <div className="flex-auto mb-08 mb-lg-0">
                            <h4 className="text-600 my-0 selected-vsl-title d-flex align-items-center">
                                {parentTabType === parentTabTypes.CASEREVIEW ? (
                                    <CaseReviewTabVariantSampleTitle
                                        {...{
                                            noSavedNotes,
                                            countNotes,
                                            countNotesInReport,
                                    ...
                                    <InterpretationTabVariantSampleTitle
                                        {...{
                                            noSavedNotes,
                                            anyUnsavedChanges,
                                            isDeleted,
                            ...
                            {parentTabType === parentTabTypes.CASEREVIEW ? (
                                <div className="d-block d-lg-flex align-items-center">
                                    ...
                                    <button
                                        type="button"
                                        className={
                                            'btn btn-sm d-flex align-items-center btn-' +
                                            (noSavedNotes
                                                ? 'outline-secondary'
                                                : isExpanded
                                                ? 'primary-dark'
                                        ...
                                        disabled={noSavedNotes}>
                                        <i
                                            className={
                                                'icon icon-fw fas mr-06 icon-' +
                ...
                <div className="card-body pt-04 pb-08">
                    <div className="row flex-column flex-sm-row">
                        <div className="col col-sm-4 col-lg-2 py-2">
                            <label
                                className="mb-04 text-small d-block"
                                data-tip={
                                    variantIsSNV
                                        ? snvGeneTranscriptColDescription
                                        : svGeneTranscriptColDescription
                                }>
                                {(variantIsSNV
                                    ? snvGeneTranscriptColTitle
                                    : svGeneTranscriptColTitle) ||
                                    'Gene, Transcript'}
                            </label>
                            <button
                                type="button"
                                ...
                                data-href={
                                    vsID +
                                    '?showInterpretation=True&annotationTab=0&interpretationTab=0' +
                                    (caseAccession
                                        ? '&caseSource=' + caseAccession
                                        : '')
                                }
                                data-child-window={vsUUID}
                                className="btn btn-link p-0">
                                {variantIsSNV ? (
                                    <GenesMostSevereDisplayTitle
                                        result={variantSample}
                                        align="left"
                                    />
                                ) : (
                                    <StructuralVariantTranscriptColumn
                        ...
                        {variantIsSNV ? (
                            <div className="col col-sm-4 col-lg-2 py-2">
                                <label
                                    className="mb-04 text-small d-block"
                                    data-tip={snvVariantColDescription}>
                                ...
                                <button
                                    ...
                                    data-href={
                                        vsID +
                                        '?showInterpretation=True&annotationTab=1&interpretationTab=1' +
                                        (caseAccession
                                            ? '&caseSource=' + caseAccession
                                            : '')
                                    }
                                    data-child-window={vsUUID}
                                    className="btn btn-link p-0 mw-100">
                                    <GenesMostSevereHGVSCColumn
                                        gene={firstGene}
                                        align="left"
                                    />
                        ...
                        <div className="col col-sm-4 col-lg-3 py-2">
                            <label
                                ...
                                data-tip={
                                    variantIsSNV
                                        ? snvGenotypeLabelColDescription
                                        : svGenotypeLabelColDescription
                                }>
                                {(variantIsSNV
                                    ? snvGenotypeLabelColTitle
                                    : svGenotypeLabelColTitle) || 'Genotype'}
                            </label>
                            <ProbandGenotypeLabelColumn
                                result={variantSample}
                                align="left"
                        ...
                        <div className="col col-sm-4 col-lg-2 py-2">
                            <label className="mb-04 text-small d-block">
                                ACMG Classification
                            </label>
                            <ACMGClassificationColumn
                ...
                <div className="card-body border-top attribution-section py-2">
                    <div className="row align-items-center">
                        <div className="col d-flex align-items-center">
                            <FilterBlocksUsedPopovers
                                {...{ selection, facetDict }}
                            />
                        </div>
                        <div
                            className="col-auto text-small"
                            data-tip={
                                'Last modified (any note or the sample itself)' +
                                (lastModifiedUserDisplayTitle
                                    ? ' by ' + lastModifiedUserDisplayTitle
                                    : '')
                            }>
                            <i
                                className={
                                    'icon icon-calendar far mr-07' +
                            ...
                            {dateAnyNoteLastModified ? (
                                <LocalizedTime
                                    timestamp={dateAnyNoteLastModified}
                                />
                            ) : (
                                <em>N/A</em>
                            )}
                        ...
                        <div
                            className="col-auto text-small"
                            data-tip={
                                'Date added to interpretation' +
                                (selectedByUserDisplayTitle
                                    ? ' by ' + selectedByUserDisplayTitle
                                    : '')
                            }>
...
export const ACMGClassificationColumn = React.memo(
    function ACMGClassificationColumn({
        clinicalInterpretationNote,
        showIcon = true,
    }) {
        const { classification: acmgClassification = null } =
            clinicalInterpretationNote || {};
        ...
        return (
            <React.Fragment>
                {showIcon ? (
                    <i
                        className="status-indicator-dot mr-1"
                        data-status={acmgClassification}
                    />
                ) : null}
                {acmgClassification}
            </React.Fragment>
...
export const DiscoveryCandidacyColumn = React.memo(
    function DiscoveryCandidacyColumn({
        discoveryInterpretationNote,
        showIcon = true,
    }) {
        const {
            gene_candidacy: geneCandidacy = null,
            variant_candidacy: variantCandidacy = null,
        } = discoveryInterpretationNote || {};
        const labelStyle = useMemo(function () {
            return { width: 70 };
        }); // Don't create new object reference each re-render
        return (
            <React.Fragment>
                <div className="text-left">
                    <span
                        className="font-italic text-muted d-inline-block"
                        style={labelStyle}>
                        Gene:{' '}
                    </span>
                    {geneCandidacy ? (
...
function InterpretationTabVariantSampleTitle(props) {
    ...
    if (anyUnsavedChanges) {
        return (
            <React.Fragment>
                <i
                    className={`icon align-middle icon-fw title-prefix-icon icon-${
                        isDeleted ? 'trash text-danger' : 'check text-success'
                    } fas mr-12`}
                />
                <span className={'text-' + (isDeleted ? 'muted' : 'secondary')}>
                    {variantDisplayTitle}
                </span>
            </React.Fragment>
        );
        ...
        const targetHref =
            vsID +
            '?showInterpretation=True&interpretationTab=1' +
            (caseAccession ? '&caseSource=' + caseAccession : '');
        return (
            <React.Fragment>
                <i
                    className={`icon align-middle icon-fw title-prefix-icon icon-${
                        noSavedNotes ? 'pen' : 'sticky-note'
                    } fas mr-12`}
                    data-tip={
                        noSavedNotes
                            ? 'This sample has no annotations yet'
                            : 'This sample has at least one annotation saved'
                    }
                />
                <button
                    type="button"
                    ...
                    data-child-window={vsUUID}
                    className="btn btn-link p-0 text-larger text-600 text-truncate">
                    {variantDisplayTitle}
                </button>
            </React.Fragment>
        );
...
const CaseReviewTabVariantSampleTitle = React.memo(
    function CaseReviewTabVariantSampleTitle(props) {
        const {
            noSavedNotes,
            countNotes,
            countNotesInReport,
        ...
        if (searchType === 'StructuralVariantSample') {
            ...
            savedNotesTip =
                'No notes saved for this Variant Sample, annotate it under the Interpretation tab.';
        } else {
            savedNotesTip = `This sample has <b>${countNotesInReport}</b> (of ${countNotes}) note${
                countNotesInReport === 1 ? '' : 's'
            } saved to the report`;
            savedNotesTip +=
                countNotesInReport === 0
                    ? ' and thus will be <b>excluded from report</b> entirely.'
                    : '.';
        }
        ...
        return (
            <React.Fragment>
                ...
                {countNotesInKnowledgeBase > 0 ? (
                    <i
                        className="icon align-middle icon-fw icon-database fas ml-12 text-muted"
                        data-html
                        data-tip={`This sample has <b>${countNotesInKnowledgeBase}</b> (of ${countNotesInReport}) note${
                            countNotesInKnowledgeBase === 1 ? '' : 's'
                        } which have been saved to project.`}
                    />
                ) : null}
            </React.Fragment>
        );
...
const ActionsDropdown = React.memo(function ActionsDropdown(props) {
    const { toggleVariantSampleSelectionDeletion, variantSample, isDeleted } =
        props;
    ...
    return (
        <DropdownButton
            ...
            <DropdownItem key={0} eventKey="delete">
                <i
                    className={
                        'icon mr-08 icon-fw fas icon-' +
                        (isDeleted ? 'trash-restore' : 'trash')
                    }
                />
...
function ClassificationDropdown(props) {
    const {
        variantSample,
        tableTagsByID,
        unsavedClassification = undefined,
        updateClassificationForVS,
        searchType,
    } = props;
    const {
    ...
    const isUnsavedClassification =
        typeof unsavedClassification !== 'undefined'; // `null` means explicitly removed
    const viewClassification = isUnsavedClassification
        ? unsavedClassification
        : savedClassification;
    ...
    const haveVSEditPermission = useMemo(
        function () {
    ...
    if (haveVSEditPermission) {
        tags.forEach(function (tagObj, idx) {
            ...
            const existingSavedOption =
                savedClassification === classificationID;
            const active =
                viewClassification && viewClassification === classificationID;
            renderedOptions.push(
                <DropdownItem
                    key={idx}
                    eventKey={classificationID}
                    active={active}
                    className={
                        existingSavedOption && !active
                            ? 'bg-light text-600'
                            : null
                    }>
                    {title}
        ...
        renderedOptions.push(
            <DropdownItem
                key="none"
                eventKey={null}
                active={!viewClassification}
                className={
                    !savedClassification && viewClassification
                        ? 'bg-light text-600'
                        : null
                }>
                <em>None</em>
    ...
    const unsavedIndicator = (
        <i
            className="icon icon-asterisk fas mr-06 text-danger text-smaller"
            data-delay={500}
            data-tip="Not yet saved, click 'Update Findings' to apply."
        />
    );
    ...
    const title =
        !viewClassification || unsavedClassification === null ? (
            <React.Fragment>
                ...
                <span className="text-300">Not a finding</span>
            </React.Fragment>
        ) : (
            (tableTagsByID[viewClassification] && (
                <React.Fragment>
                    {isUnsavedClassification ? unsavedIndicator : null}
                    <span>
                        {tableTagsByID[viewClassification].title ||
                            viewClassification}
                    </span>
                </React.Fragment>
    ...
    return (
        <div className="py-1 py-lg-0 pr-lg-12">
            <DropdownButton
                size="sm"
                variant="outline-dark d-flex align-items-center"
                menuAlign="right"
                title={title}
                onSelect={onOptionSelect}
                disabled={
                    !haveVSEditPermission ||
                    tags.length === 0 ||
...
function FilterBlocksUsedPopovers(props) {
    ...
    return (
        <React.Fragment>
            <i
                className={`icon icon-object-${
                    filterBlocksIntersected ? 'group' : 'ungroup'
                } far mr-07`}
                data-tip={
                    'Filter blocks matched when this sample was added.' +
                    (filterBlocksIntersected
                        ? ' Search result came from intersecting these filter blocks.'
                        : '')
                }
            />
            {links}
...
function FilterBlockPopover(props) {
    ...
    const popover = (
        <Popover className="filterset-block-popover">
            <Popover.Title
                className="m-0 text-400 bg-secondary text-light"
                as="h5">
                {name}
            </Popover.Title>
...
const PlaceHolderStatusIndicator = React.memo(
    function PlaceHolderStatusIndicator({ showIcon = true }) {
        return (
...
const VariantSampleExpandedNotes = React.memo(
    function VariantSampleExpandedNotes(props) {
        ...
        const allNotesToReportSelected =
            ...
            (noDiscoveryNoteSaved ||
                sendToReportStore[discoveryInterpretationNoteUUID]) &&
            (noClinicalNoteSaved ||
                sendToReportStore[clinicalInterpretationNoteUUID]);

        const allNotesToReportAlreadyStored =
            (noVariantNotesSaved ||
                alreadyInReportNotes[lastVariantNoteUUID]) &&
            (noGeneNotesSaved || alreadyInReportNotes[lastGeneNoteUUID]) &&
            (noDiscoveryNoteSaved ||
                alreadyInReportNotes[discoveryInterpretationNoteUUID]) &&
            (noClinicalNoteSaved ||
                alreadyInReportNotes[clinicalInterpretationNoteUUID]);
        ...
        const allNotesToKnowledgeBaseSelected =
            ...
            (noDiscoveryNoteSaved ||
                sendToProjectStore[discoveryInterpretationNoteUUID]) &&
            (noClinicalNoteSaved ||
                sendToProjectStore[clinicalInterpretationNoteUUID]);

        const allNotesToKnowledgeBaseAlreadyStored =
            (noVariantNotesSaved ||
                alreadyInProjectNotes[lastVariantNoteUUID]) &&
            (noGeneNotesSaved || alreadyInProjectNotes[lastGeneNoteUUID]) &&
            (noDiscoveryNoteSaved ||
                alreadyInProjectNotes[discoveryInterpretationNoteUUID]) &&
            (noClinicalNoteSaved ||
                alreadyInProjectNotes[clinicalInterpretationNoteUUID]);
        ...
        function makeNoteSelectionObjects(
            useStore = sendToReportStore,
            allSelected = false
        ) {
            ...
            if (!noDiscoveryNoteSaved) {
                if (allSelected || !useStore[discoveryInterpretationNoteUUID]) {
                    // Add, will uncheck
                    noteSelectionObjects.push([
                        discoveryInterpretationNoteUUID,
                        true,
                    ]);
                }
            }
            if (!noClinicalNoteSaved) {
                if (allSelected || !useStore[clinicalInterpretationNoteUUID]) {
                    // Add, will uncheck
                    noteSelectionObjects.push([
                        clinicalInterpretationNoteUUID,
                        true,
                    ]);
                }
            }
        ...
        const onChangeSendAllNotesToReport = useCallback(
            function (e) {
                e.stopPropagation();
                toggleSendToReportStoreItems(
                    makeNoteSelectionObjects(
                        sendToReportStore,
                        allNotesToReportSelected
                    )
                );
            },
        ...
        const onChangeSendAllNotesToKnowledgeBase = useCallback(
            function (e) {
                e.stopPropagation();
                toggleSendToProjectStoreItems(
                    makeNoteSelectionObjects(
                        sendToProjectStore,
                        allNotesToKnowledgeBaseSelected
                    )
                );
            },
        ...
        return (
            <React.Fragment>
                <div className="card-body bg-light select-checkboxes-section border-top border-bottom">
                    <div>
                        <Checkbox
                            className="d-inline-block mb-08"
                            disabled={allNotesToReportAlreadyStored}
                            checked={
                                !!allNotesToReportSelected ||
                                allNotesToReportAlreadyStored
                            }
                            onChange={onChangeSendAllNotesToReport}
                            indeterminate={
                                someNotesToReportSelected &&
                                !allNotesToReportSelected &&
                                !allNotesToReportAlreadyStored
                            }>
                    ...
                    <div>
                        <Checkbox
                            ...
                            checked={
                                !!allNotesToKnowledgeBaseSelected ||
                                allNotesToKnowledgeBaseAlreadyStored
                            }
                            onChange={onChangeSendAllNotesToKnowledgeBase}
                            indeterminate={
                                someNotesToKnowledgeBaseSelected &&
                                !allNotesToKnowledgeBaseSelected &&
                                !allNotesToKnowledgeBaseAlreadyStored
                            }>
                            ...
                            {!noVariantNotesSaved ? (
                                <React.Fragment>
                                    <NoteCheckboxes
                                        ...
                                        onReportChange={useCallback(
                                            function () {
                                                toggleSendToReportStoreItems([
                                                    [lastVariantNoteUUID, true],
                                                ]);
                                            }
                                        )}
                                        onKBChange={useCallback(function () {
                                            toggleSendToProjectStoreItems([
                                                [lastVariantNoteUUID, true],
                                            ]);
                                        })}
                        ...
                        <div className="col-12 col-md-6 col-lg-3 d-flex flex-column">
                            ...
                            {!noClinicalNoteSaved ? (
                                <React.Fragment>
                                    ...
                                    <div className="note-content-area d-flex flex-column flex-grow-1">
                                        <div className="note-text-content flex-grow-1">
                                            {clinicalInterpretationNoteText || (
                                                <em>Note was left blank</em>
                                            )}
                                        </div>

                                        <div className="clinical-classification flex-grow-0">
                                            <label className="mb-0 mt-08">
                                                Classification
                                            </label>
                                            <div>
                                                {classification ? (
                                                    <React.Fragment>
                                                        <i
                                                            className="status-indicator-dot ml-1 mr-1"
                                                            data-status={
                                                                classification
                                                            }
                                                        />
                        ...
                        <div className="col-12 col-md-6 col-lg-3 d-flex flex-column">
                            ...
                            {!noDiscoveryNoteSaved ? (
                                <React.Fragment>
                                    ...
                                    <div className="note-content-area d-flex flex-column flex-grow-1">
                                        <div className="note-text-content flex-grow-1">
                                            {discoveryInterpretationNoteText || (
                                                <em className="text-secondary">
                                                    Note was left blank
                                                </em>
                                            )}
...
const NoteCheckboxes = React.memo(function NoteCheckboxes({
    onReportChange,
    onKBChange,
    reportChecked,
    reportAlreadyStored,
    ...
    return (
        <div className="d-flex flex-column flex-xl-row text-small">
            <Checkbox
                ...
                labelClassName="mb-0"
                onChange={reportAlreadyStored ? null : onReportChange}
                checked={reportAlreadyStored || reportChecked}
                disabled={reportAlreadyStored}>
                Send to Report
            </Checkbox>
            <Checkbox
                className="flex-grow-1"
                labelClassName="mb-0"
                onChange={kbAlreadyStored ? null : onKBChange}
                checked={kbAlreadyStored || kbChecked}
                disabled={kbAlreadyStored}>
                Save to Project
            </Checkbox>
        </div>
    );
