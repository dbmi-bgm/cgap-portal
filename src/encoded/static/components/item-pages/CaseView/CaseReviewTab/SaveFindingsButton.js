...
export class SaveFindingsButton extends React.Component {
    ...
    onClick(e) {
        ...
        vsObjects.forEach(function (vsObject) {
            ...
            const { [vsUUID]: classificationToSaveForVS = undefined } =
                changedClassificationsByVS;

            const allNotes = getAllNotesFromVariantSample(variant_sample_item);
            const viewPermissionForAllNotes = _.all(
                allNotes,
                function ({ uuid }) {
                    return !!uuid;
                }
            );
            const anySavedToReportNotes = _.any(
                allNotes,
                function ({ uuid: noteUUID }) {
        ...
        const shouldAdjustReportVariantSamples =
            vsItemsToAddToReport.length > 0 ||
            vsItemsToRemoveFromReport.length > 0;
        ...
        if (shouldAdjustReportVariantSamples) {
            ...
            payloads.unshift([
                reportAtID,
                { variant_samples: nextVariantSampleUUIDs },
            ]);
        }
        ...
        patchItems(payloads, (countCompleted, patchErrors) => {
            if (countCompleted > 0) {
                ...
                if (typeof fetchVariantSampleListItem === 'function') {
                    ...
                    throw new Error(
                        'No `props.fetchVariantSampleListItem` supplied to SaveFindingsButton'
                    );
                }
                ...
                if (shouldAdjustReportVariantSamples) {
                    if (typeof fetchReportItem === 'function') {
                        ...
                        throw new Error(
                            'No `props.fetchReportItem` supplied to SaveFindingsButton'
                        );
                    }
                }
            }
    ...
    render() {
        ...
        const applyFindingsTagsBtnText = `Save ${
            changedClassificationsCount > 0
                ? changedClassificationsCount + ' '
                : ''
        }Finding${changedClassificationsCount !== 1 ? 's' : ''}`;
        ...
        return (
            <button
                ...
                className={
                    'btn btn-primary' + (className ? ' ' + className : '')
                }>
