...
export const ReportGenerationView = React.memo(function ReportGenerationView(
    props
) {
    const {
        context,
    ...
    const { analysis_type: analysisTypeFromSampleProcessing } =
        sample_processing || {};
    ...
    const sortedSectionKeys = useMemo(
        function () {
            return Object.keys(report_sections).sort(function (a, b) {
                const {
                    [a]: { order: orderA = Infinity },
                    [b]: { order: orderB = Infinity },
                } = report_sections;
        ...
        switch (sectionKey) {
            ...
            case 'analysis_performed':
                renderedSections.push(
                    <TextAreaGroup
                        {...inputGroupProps}
                        title={title || 'Tests / Analysis Performed'}
                        defaultValue={
                            savedAnalysisPerformed ||
                            analysisTypeFromSampleProcessing
                        }
                    />
                );
                break;
            ...
            case 'findings_table':
                ...
                renderedSections.push(
                    <ReportFindingsTable
                        {...{ variantSamples, visible }}
                        reportItem={fetchedReportItem}
                    />
                );
                break;
            ...
            case 'extra_notes':
                renderedSections.push(
                    <TextAreaGroup
                        {...inputGroupProps}
                        title={title || 'Additional Case Notes'}
                        rows={5}
                ...
                warnings.push(
                    <React.Fragment key={sectionKey}>
                        ...
                        {title ? (
                            <>
                                {' '}
                                &lsquo;{title}&rsquo; (<code>{sectionKey}</code>
                                )
                            </>
                        ) : (
        ...
        function (e) {
            ...
            sortedSectionKeys.forEach(function (sectionKey) {
                ...
                if (sectionKey === 'findings_table') {
                    ...
                    for (var k of formData.keys()) {
                        ...
                        patchPayload.findings_texts[findingTag] =
                            formData.get(k);
                    }
                    return;
                }
            ...
            const targetHref =
                ...
                (deleteFields.length > 0
                    ? '?delete_fields=' + deleteFields.join(',')
                    : '');
            ajax.promise(
                targetHref,
                'PATCH',
            ...
            <div className="row">
                ...
                <div className="col-auto d-flex align-items-center">
                    <div className="last-saved text-small">
                        <i
                            className="icon icon-clock far mr-06"
                            data-tip="Last Modified"
                        />
                        <LocalizedTime
                            timestamp={reportLastModified}
                    ...
                    <div className="ml-12">
                        <button
                            ...
                            disabled={isPatching}>
                            <i
                                className={`icon icon-fw icon-${
                                    isPatching ? 'circle-notch spin' : 'save'
                                } fas mr-08`}
                            />
                            Save
                        </button>
    ...
    return (
        <div className="form-group mb-2 border-bottom pb-24">
            {children ? (
                ...
                <label htmlFor={inputElementID}>{title}</label>
            ) : null}
            <AutoGrowTextArea
                {...passProps}
                id={inputElementID}
                disabled={propDisabled || readonly}
                name={sectionKey}
            />
        </div>
    );
...
const ReportFindingsTable = React.memo(function ReportFindingsTable(props) {
    ...
    const {
        table_tags: { tags: tableTagOptions = [] },
    } = projectReportSettings;
    const { findings_texts = {} } = reportItem || {};

    const findingsTableGroupings = _.groupBy(
        variantSamples,
        'finding_table_tag'
    );
    ...
    const noGroupsFound =
        findingsTableKeysFoundLen === 1 &&
        typeof findingsTableKeysFound[0] === 'undefined';
    ...
    const renderedTables = tableTagOptions
        .map(function (tagOptions, idx) {
            const {
                id,
                title,
                always_visible = false,
                none_tagged_fallback_text,
            } = tagOptions || {};
            ...
            const header = (
                <h5>
                    ...
                    <span className="text-400">{` - ${samplesForTagLen} Variant${
                        samplesForTagLen === 1 ? '' : 's'
                    }`}</span>
                </h5>
            );
            ...
            if (samplesForTagLen === 0) {
                return (
                    <div className="mb-24" key={idx}>
                        <ReportFindingsTableNoSamplesFallback
                            {...{ header, visible, id }}
                            defaultValue={
                                savedTagNoteText || none_tagged_fallback_text
                            }
                        />
                    </div>
                );
            ...
            return (
                <div className="mb-24" key={idx}>
                    {header}
                    <ReportFindingsFindingTable
                        variantSamples={samplesForTag}
    ...
    return (
        <React.Fragment>
            <div className="d-flex align-items-center">
                {header}
                <Checkbox
                    name={'include_finding_tag_text_' + id}
                    onChange={onCheck}
            ...
            <AutoGrowTextArea
                ...
                rows={1}
                disabled={!includeChecked}
                name={'finding_tag_text_' + id}
                className="mb-16"
            />
        </React.Fragment>
    );
...
const ReportFindingsFindingTable = React.memo(
    function ReportFindingsFindingTable(props) {
        ...
        return (
            <table
                className={
                    'report-findings-table table table-striped' +
