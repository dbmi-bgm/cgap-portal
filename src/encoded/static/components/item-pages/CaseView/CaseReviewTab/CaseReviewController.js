...
export class CaseReviewController extends React.Component {
    static savedClassificationsByVS(variantSampleListItem) {
        ...
        vsObjects.forEach(function ({ variant_sample_item }) {
            const { uuid: vsUUID, finding_table_tag = null } =
                variant_sample_item;
            if (!vsUUID) {
    ...
    static getDerivedStateFromProps(props, state) {
        ...
        const {
            pastVSLItem,
            changedClassificationsByVS: origClassificationDict,
        } = state;
        ...
        if (variantSampleListItem !== pastVSLItem) {
            ...
            const changedClassificationsCount = Object.keys(
                origClassificationDict
            ).length;
            if (changedClassificationsCount === 0) {
                return null;
            }
            const savedClassificationsByVS =
                CaseReviewController.savedClassificationsByVS(
            ...
            Object.keys(origClassificationDict).forEach(function (vsUUID) {
                if (savedClassificationsByVS[vsUUID] !== undefined) {
                    ...
                    // as well.
                    const isEqual =
                        (origClassificationDict[vsUUID] || null) ===
                        savedClassificationsByVS[vsUUID];
                    if (!isEqual) {
                        nextChangedClassificationsByVS[vsUUID] =
                            origClassificationDict[vsUUID];
                    } else {
            ...
            if (anyClassificationsUnset) {
                console.log(
                    'Updating `changedClassificationsByVS` - ',
                    origClassificationDict,
                    nextChangedClassificationsByVS
                );
                nextState.changedClassificationsByVS =
                    nextChangedClassificationsByVS;
            }
        }
    ...
    constructor(props) {
        ...
        this.updateClassificationForVS =
            this.updateClassificationForVS.bind(this);
        this.state = {
        ...
        this.memoized = {
            savedClassificationsByVS: memoize(
                CaseReviewController.savedClassificationsByVS
            ),
            changedClassificationsCount: memoize(function (
                changedClassificationsByVS
            ) {
    ...
    fetchProjectItem() {
        const {
            context: { project: { '@id': projectAtID = null } = {} },
        } = this.props;
    ...
    fetchReportItem(callback) {
        ...
        const reportFetchCallback = (res = []) => {
            ...
            this.setState(
                { fetchedReportItem, isFetchingReportItem: false },
                callback
            );
        };
    ...
    updateClassificationForVS(vsUUID, classification, callback) {
        ...
        const savedClassificationsByVS = this.memoized.savedClassificationsByVS(
            variantSampleListItem
        );
        ...
        this.setState(function ({
            ...
            const { [vsUUID]: savedClassification = null } =
            ...
            if (
                (!savedClassification && classification === null) ||
                savedClassification === classification
            ) {
                if (
                    typeof nextChangedClassificationsByVS[vsUUID] ===
                    'undefined'
                ) {
            ...
            return {
                changedClassificationsByVS: nextChangedClassificationsByVS,
            };
        },
        callback);
    }
    ...
    render() {
        ...
        const changedClassificationsCount =
            this.memoized.changedClassificationsCount(
                changedClassificationsByVS
            );
        const childProps = {
            ...passProps,
...
export class CaseReviewSelectedNotesStore extends React.PureComponent {
    static alreadyInProjectNotes(variantSampleListItem) {
        return buildAlreadyStoredNoteUUIDDict(
            variantSampleListItem,
            function ({ is_saved_to_project }) {
        ...
        return buildAlreadyStoredNoteUUIDDict(
            ...
            function (noteItem, vsItem) {
                ...
                const foundVSEntryInReport =
                    _.findWhere(variant_samples, { uuid: vsUUID }) ||
                    _.findWhere(structural_variant_samples, { uuid: vsUUID });
                if (!foundVSEntryInReport) {
                    return false;
                }
                // Ensure this Report is in `Note.associated_items`.
                const foundReportEntryInNote = _.findWhere(
                    noteAssociatedItems,
                    { item_type: 'Report', item_identifier: reportUUID }
                );
    ...
    static getDerivedStateFromProps(props, state) {
        ...
        if (variantSampleListItem !== pastVSLItem) {
            ...
            const alreadyInProjectNotes =
                CaseReviewSelectedNotesStore.alreadyInProjectNotes(
                    variantSampleListItem
                );
            const alreadyInReportNotes =
                CaseReviewSelectedNotesStore.alreadyInReportNotes(
                    variantSampleListItem,
                    fetchedReportItem
                );
    ...
    constructor(props) {
        super(props);
        this.toggleSendToProjectStoreItems = this.toggleStoreItems.bind(
            this,
            'sendToProjectStore'
        );
        this.toggleSendToReportStoreItems = this.toggleStoreItems.bind(
            this,
            'sendToReportStore'
        );
        ...
        this.memoized = {
            alreadyInProjectNotes: memoize(
                CaseReviewSelectedNotesStore.alreadyInProjectNotes
            ),
            alreadyInReportNotes: memoize(
                CaseReviewSelectedNotesStore.alreadyInReportNotes
            ),
        };
    }
    ...
    render() {
        ...
        const alreadyInProjectNotes = this.memoized.alreadyInProjectNotes(
            variantSampleListItem
        );
        const alreadyInReportNotes = this.memoized.alreadyInReportNotes(
            variantSampleListItem,
            fetchedReportItem
        );
 ...
 */
export function buildAlreadyStoredNoteUUIDDict(
    variantSampleListItem,
    checkFunction
) {
    const {
    ...
    snvVSObjects
        .concat(cnvVSObjects)
        .forEach(function ({ variant_sample_item }) {
            getAllNotesFromVariantSample(variant_sample_item).forEach(function (
                noteItem
            ) {
