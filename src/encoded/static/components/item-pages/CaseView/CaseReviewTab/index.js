...
import {
    VariantSampleSelectionList,
    parentTabTypes,
} from './../VariantSampleSelection';
import {
...
export const CaseReviewTab = React.memo(function CaseReviewTab(props) {
    ...
    const { last_modified: { date_modified: reportLastModified = null } = {} } =
        fetchedReportItem || {};
    ...
    const onClickNoteFinalization = useCallback(
        function (e) {
            e.stopPropagation();
            setCurrentViewIdx(0);
        },
    ...
    const onClickReportGeneration = useCallback(
        function (e) {
            e.stopPropagation();
            setCurrentViewIdx(1);
        },
        ...
        <React.Fragment>
            {isActiveDotRouterTab ? (
                <div className="d-flex align-items-center justify-content-between mb-36">
                    <h1 className="text-300 mb-0">
                        ...
                        isLoadingVariantSampleListItem ? (
                            <i
                                className="icon icon-fw icon-circle-notch icon-spin fas ml-12 text-muted"
                                data-tip="Loading Report..."
                            />
                        ) : (
                            <React.Fragment>
            ...
            <div className={currentViewIdx === 1 ? 'd-block' : 'd-none'}>
                <ReportGenerationView
                    ...
                    key={reportLastModified + '_' + resetCounter}
                    visible={currentViewIdx === 1}
                />
            </div>
        </React.Fragment>
    );
...
function countVariantSamplesWithAnySelectionSize(
    variantSampleListItem,
    selectionStore
) {
...
function SaveNotesToProjectButton(props) {
    ...
    const selectionStoreSize = useMemo(
        function () {
    ...
    const variantSamplesWithAnySelections = useMemo(
        function () {
            return countVariantSamplesWithAnySelectionSize(
                variantSampleListItem,
                sendToProjectStore
            );
        },
    ...
    const onClick = useCallback(
        function (e) {
            ...
            const variantSampleItems = snvVSObjects
                .concat(cnvVSObjects)
                .map(function ({ variant_sample_item }) {
            ...
            variantSampleItems.forEach(function (variantSampleItem) {
                const {
                    ...
                    discovery_interpretation: {
                        ...
                        is_saved_to_project:
                            isDiscoveryInterpretationSavedToProject,
                    } = {},
                    gene_notes: {
                ...
                if (
                    interpretationUUID &&
                    sendToProjectStore[interpretationUUID]
                ) {
                    if (!isInterpretationSavedToProject) {
                        ...
                        payload.save_to_project_notes.interpretation =
                            interpretationUUID;
                    }
                }
                if (
                    discoveryInterpretationUUID &&
                    sendToProjectStore[discoveryInterpretationUUID]
                ) {
                    if (!isDiscoveryInterpretationSavedToProject) {
                        payload.save_to_project_notes.discovery_interpretation =
                            discoveryInterpretationUUID;
                    }
                }
                if (lastGeneNoteUUID && sendToProjectStore[lastGeneNoteUUID]) {
                    if (!isGeneNoteSavedToProject) {
                        payload.save_to_project_notes.gene_notes =
                            lastGeneNoteUUID;
                    }
                }
                if (
                    lastVariantNoteUUID &&
                    sendToProjectStore[lastVariantNoteUUID]
                ) {
                    if (!isVariantNoteSavedToProject) {
                        payload.save_to_project_notes.variant_notes =
                            lastVariantNoteUUID;
                    }
                }
            ...
            patchItems(payloads, (countCompleted, patchErrors) => {
                if (countCompleted > 0) {
                    if (typeof fetchVariantSampleListItem === 'function') {
                        console.log(
                            'Refreshing our VariantSampleListItem with updated Note Item statuses.'
                        );
                        fetchVariantSampleListItem();
                    } else {
                        throw new Error(
                            'No `props.fetchVariantSampleListItem` supplied to SaveNotesToProjectButton'
                        );
                    }
                }
                // TODO:
...
class SaveNotesToReportButton extends React.PureComponent {
    static buildPatchPayloads(
        ...
        const newAssociatedItemEntry = {
            item_type: 'Report',
            item_identifier: reportUUID,
        };
        ...
        vslVariantSampleItems.forEach(function (variantSampleItem) {
            const {
                ...
                discovery_interpretation: {
                    uuid: discoveryInterpretationUUID,
                    associated_items:
                        discoveryInterpretationAssociatedItems = [],
                } = {},
                gene_notes: {
                    uuid: lastGeneNoteUUID,
            ...
            if (interpretationUUID && sendToReportStore[interpretationUUID]) {
                const existingEntry = _.findWhere(
                    interpretationAssociatedItems,
                    newAssociatedItemEntry
                );
                if (!existingEntry) {
                    notePayloads.push([
                        '/' + interpretationUUID,
            ...
            if (
                discoveryInterpretationUUID &&
                sendToReportStore[discoveryInterpretationUUID]
            ) {
                const existingEntry = _.findWhere(
                    discoveryInterpretationAssociatedItems,
                    newAssociatedItemEntry
            ...
            if (lastVariantNoteUUID && sendToReportStore[lastVariantNoteUUID]) {
                const existingEntry = _.findWhere(
                    lastVariantNoteAssociatedItems,
                    newAssociatedItemEntry
                );
                if (!existingEntry) {
                    notePayloads.push([
        ...
        const reportVariantSampleUUIDsToPatch = Object.keys(
            reportPatchVariantSampleUUIDs
        );
    ...
    constructor(props) {
        ...
        this.memoized = {
            ...
            variantSamplesWithAnySelections: memoize(function (
                variantSampleListItem,
                sendToReportStore
            ) {
                return countVariantSamplesWithAnySelectionSize(
    ...
    isDisabled() {
        ...
        const selectionStoreSize =
            this.memoized.selectionStoreSize(sendToReportStore);
        return (
            propDisabled ||
            isPatching ||
            isLoadingVariantSampleListItem ||
    ...
    handleClick(e) {
        ...
        } = SaveNotesToReportButton.buildPatchPayloads(
            reportVariantSamples,
            snvVSItems,
            reportUUID,
            sendToReportStore
        );
        const {
            ...
            reportStructuralVariantSamples,
            cnvVSItems,
            reportUUID,
            sendToReportStore
        );
        ...
        if (reportCNVUUIDListLen > 0) {
            if (
                !_.isEqual(
                    reportCNVUUIDList,
                    _.pluck(reportStructuralVariantSamples, 'uuid')
                )
            ) {
                // Skip if is same value to be patched (maybe this is 2nd saving action as result of some prior network error(s))
                reportPayload = reportPayload || {};
        ...
        patchItems(allPayloads, (countCompleted, patchErrors) => {
            if (countCompleted > 0) {
                if (typeof fetchVariantSampleListItem === 'function') {
                    ...
                    throw new Error(
                        'No `props.fetchVariantSampleListItem` supplied to SaveNotesToReportButton'
                    );
                }
                ...
                if (reportPayload !== null) {
                    if (typeof fetchReportItem === 'function') {
                        ...
                        throw new Error(
                            'No `props.fetchReportItem` supplied to SaveNotesToReportButton'
                        );
                    }
                }
            }
    ...
    render() {
        ...
        const selectionStoreSize =
            this.memoized.selectionStoreSize(sendToReportStore);
        const variantSamplesWithAnySelections =
            this.memoized.variantSamplesWithAnySelections();
        const disabled = this.isDisabled();
        const btnCls =
            'btn btn-' +
            (!reportUUID ? 'outline-danger' : 'primary') +
            (className ? ' ' + className : '');
