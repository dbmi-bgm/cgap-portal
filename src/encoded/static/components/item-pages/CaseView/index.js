...
} from './CaseReviewTab/CaseReviewController';
import {
    getAllNotesFromVariantSample,
    NoteSubSelectionStateController,
} from './variant-sample-selection-panels';
...
const CaseInfoTabView = React.memo(function CaseInfoTabView(props) {
    ...
    const {
        case_phenotypic_features: caseFeatures = {
     ...
     * @type {boolean}
     */
    const haveCaseEditPermission = useMemo(
        function () {
    ...
    const secondaryFamilies = useMemo(
        function () {
            return (familiesWithViewPermission || []).filter(function (
                spFamily
            ) {
    ...
    const anyAnnotatedVariantSamples = useMemo(
        function () {
            // checks for notes on SNVs and CNV/SVs
    ...
    const onViewPedigreeBtnClick = useCallback(
        function (evt) {
            evt.preventDefault();
            evt.stopPropagation();
    ...
    if (caseIndividual) {
        caseSearchTables = (
            <React.Fragment>
                ...
                <div className="search-table-wrapper">
                    <EmbeddedCaseSearchTable
                        session={session}
                        facets={null}
                        searchHref={`/search/?type=Case&accession=${caseAccession}`}
                    />
                </div>
            </React.Fragment>
        );
        ...
        caseSearchTables = (
            <div className="mt-3">
                <h4 className="text-400 mb-03">
                    No individual assigned to this case
                </h4>
            </div>
        );
    }
    ...
    let pedBlock = (
        <div
            className="d-none d-lg-block pedigree-placeholder flex-fill"
            onClick={onViewPedigreeBtnClick}
    ...
    const disableInterpretation =
        !isLoadingVariantSampleListItem &&
        vsSelections.length === 0 &&
        cnvSelections.length === 0;
        ...
        () => {
            ...
            if (loadingAccordion && (prevHref || schemas)) {
                ...
                defaultAccordionState =
                    dotPath === '.accessioning' || !dotPath ? '0' : null; // "0" is open, null is close
                            ...
                            <>
                                <div className="container-wide bg-light pt-36 pb-36">
                                    <div className="card-group case-summary-card-row">
                                        ...
                                        <div
                                            id="case-overview-ped-link"
                                            className="col-pedigree-viz">
                                            <div className="card d-flex flex-column">
                                                <div className="pedigree-vis-heading card-header primary-header d-flex justify-content-between">
                                                    ...
                                                    <button
                                                        type="button"
                                                        className="btn btn-primary btn-sm view-pedigree-btn"
                                                        onClick={
                                                            onViewPedigreeBtnClick
                                                        }
                                                        disabled={
    ...
    return (
        <React.Fragment>
            {loadingAccordion && (
                <div
                    className="container-wide d-flex justify-content-center"
            ...
            {canonicalFamily && caseIndividual ? (
                <DotRouter
                    ...
                    navClassName="container-wide pt-36 pb-36"
                    contentsClassName="container-wide bg-light pt-36 pb-36"
                    prependDotPath="case-info">
                    <DotRouterTab
                        dotPath=".accessioning"
                        default
                        tabTitle="Accessioning">
                        <AccessioningTab
                            {...{
                                context,
                                href,
                    ...
                    <DotRouterTab
                        dotPath=".bioinformatics"
                        disabled={disableBioinfo}
                        tabTitle="Bioinformatics">
                        <BioinformaticsTab
                            {...{
                                context,
                                idToGraphIdentifier,
                    ...
                    <DotRouterTab
                        dotPath=".interpretation"
                        cache
                        disabled={disableInterpretation}
                        tabTitle={
                            <span
                                data-tip={
                                    isLoadingVariantSampleListItem
                                        ? 'Loading latest selection, please wait...'
                                        : null
                                }>
                                {isLoadingVariantSampleListItem ? (
                                    <i className="icon icon-spin icon-circle-notch mr-1 fas" />
                                ) : null}
                                Interpretation
                            </span>
                        }>
                    ...
                    <DotRouterTab
                        dotPath=".review"
                        cache
                        disabled={anyAnnotatedVariantSamples ? false : true}
                        tabTitle="Case Review">
                        <CaseReviewController
                            {...{ context, variantSampleListItem }}>
                            <CaseReviewSelectedNotesStore>
                                <NoteSubSelectionStateController>
                                    <CaseReviewTab
                                        {...{
                ...
                <div className="error-placeholder bg-light py-5 px-3 border-top border-bottom">
                    <h4 className="text-400 text-center">
                        No family or no individual defined for this case.
                    </h4>
                </div>
            )}
        </React.Fragment>
...
function CaseInfoToggle({
    ...
    return (
        <>
            <div
                className="container-wide clickable"
                onClick={decoratedOnClick}>
                <h3 className="tab-section-title">
                    <div className="d-flex align-items-center">
                        <i
                            className={`icon icon-${icon} fas mr-2 text-large`}
                        />
                        <div className="pt-12 pb-06">
                            <span>{caseNamedTitle || caseNamedID}</span>
                            <object.CopyWrapper
                                className="text-smaller text-muted text-monospace text-400"
                                value={caseAccession}
                                stopPropagation>
                                {caseAccession}
                            </object.CopyWrapper>
                    ...
                    <button
                        type="button"
                        className="btn btn-primary btn-sm view-pedigree-btn py-2 px-4 rounded"
                        onClick={onViewPedigreeBtnClick}
                        disabled={!currPedigreeFamily}>
                        View Pedigree
                    </button>
...
class DotRouter extends React.PureComponent {
    static getDotPath(href) {
        ...
        const hashString =
            (url.parse(href, false).hash || '#').slice(1) || null;
    ...
    static getDefaultTab(children) {
        ...
        if (childrenLen === 0) {
            throw new Error(
                'Must provide children and ideally default tab to DotRouter via props.'
            );
        }
    ...
    getCurrentTab() {
        ...
        if (dotPath) {
            for (let i = 0; i < children.length; i++) {
                const currChild = children[i];
                if (
                    currChild.props.dotPath === dotPath &&
                    !currChild.props.disabled
                ) {
    ...
    render() {
        ...
        const {
            props: { dotPath: currTabDotPath },
        } = currentTab; // Falls back to default tab if not in hash.
        // const contentClassName = "tab-router-contents" + (contentsClassName ? " " + contentsClassName : "");
        const allTabContents = [];

        const adjustedChildren = React.Children.map(
            children,
            function (childTab, index) {
                const {
                    props: {
                        dotPath,
                ...
                if (active || cache) {
                    ...
                    const transformedChildren = !cache
                        ? tabChildren
        ...
        return (
            <div
                ...
                <nav
                    className={
                        'dot-tab-nav' + (navClassName ? ' ' + navClassName : '')
                    }>
                    <div className="dot-tab-nav-list">{adjustedChildren}</div>
                </nav>
                {allTabContents}
            </div>
        );
...
const AccessioningTab = React.memo(function AccessioningTab(props) {
    ...
    const { display_title: primaryFamilyTitle, '@id': canonicalFamilyAtID } =
        canonicalFamily;
    const [isSecondaryFamiliesOpen, setSecondaryFamiliesOpen] = useState(false);
    const secondaryFamiliesLen = secondaryFamilies.length;

    const viewSecondaryFamiliesBtn =
        secondaryFamiliesLen === 0 ? null : (
            <div className="pt-2">
                <button
                    type="button"
                    className="btn btn-block btn-outline-dark"
                    onClick={function () {
                        setSecondaryFamiliesOpen(function (
                            currentIsSecondaryFamiliesOpen
                        ) {
                            return !currentIsSecondaryFamiliesOpen;
                        });
                    }}>
                    {!isSecondaryFamiliesOpen
                        ? `Show ${secondaryFamiliesLen} more famil${
                              secondaryFamiliesLen > 1 ? 'ies' : 'y'
                          } that proband is member of`
                        : 'Hide secondary families'}
                </button>
            </div>
        );
        ...
        <React.Fragment>
            <h1 className="row align-items-center">
                <div className="col">
                    <span className="text-300">
                        Accessioning Report and History
                    </span>
                </div>
                <div className="col-auto">
                    <span className="current-case text-small text-400 m-0">
                        Current Selection
                    </span>
                </div>
            </h1>
            <div className="tab-inner-container card">
                <div className="card-body">
                    <PartialList
                        className="mb-0"
                        open={isSecondaryFamiliesOpen}
                        persistent={[
                            <div
                                key={canonicalFamilyAtID}
                                className="primary-family">
                        ...
                        collapsible={
                            !isSecondaryFamiliesOpen
                                ...
                                : secondaryFamilies.map(function (family) {
                                      const { display_title, '@id': familyID } =
                                          family;
                                      return (
                                          <div
                                              className="py-4 secondary-family"
                                              key={familyID}>
...
const bioinfoPopoverContent = {
    predictedSexAndAncestry: (
        <div>
            ...
            <a
                href="https://peddy.readthedocs.io/en/latest/"
                target="_blank"
                rel="noreferrer">
                documentation
            </a>
    ...
    filteredSNVIndelVariants: (
        <div>
            During processing,{' '}
            <a
                href="https://cgap-pipeline-main.readthedocs.io/en/latest/Pipelines/Downstream/SNV_germline/Pages/SNV_germline-step-filtering.html"
                target="_blank"
                rel="noreferrer">
                hard filters are applied
            </a>{' '}
            to remove variants that will not be of interest. This lowers the
            number of variants returned from the millions to the thousands.
            Briefly, these filters include: (1) removing intergenic variants;
            (2) including some variants based on VEP, ClinVar, and SpliceAI
            annotations; (3) Removing variants with only intronic consequences;
            and (4) removing common variants based on gnomAD population allele
    ...
    filteredSVVariants: (
        <div>
            During processing,{' '}
            <a
                href="https://cgap-pipeline-main.readthedocs.io/en/latest/Pipelines/Downstream/SV_germline/Pages/SV_germline-step-part-3.html"
                target="_blank"
                rel="noreferrer">
                hard filters are applied
            </a>{' '}
            to remove structural variants (SVs) that will not be of interest.
            This limits the numbers and types of SVs returned from thousands to
            fewer than 500. Briefly, these filters include: (1) including SVs
            based on VEP annotations; (2) removing SVs with only intronic or
            intergenic consequences; (3) selecting SVs based on SV type (e.g.,
            DEL and DUP); (3) removing common variants based on gnomAD-SV
            population allele frequency, and a panel of 20 unrelated samples;
            and (4) removing SVs over a certain size.
            <p>Note: SVs are only available for WGS samples.</p>
        </div>
    ),
    heterozygosity: (
        <div>
            The Heterozygosity/Homozygosity ratio is calculated by bcftools.
            Expected values are between 1.4 - 2.5; higher or lower values can
            indicate lower quality calls.
        </div>
    ),
    transTransRatio: (
        <div>
            The Transition/Transversion ratio is calculated by bcftools.
            Expected values are 1.8-2.1 overall for WGS, and 2.2-3.3 for WES.
            Values outside this range can indicate lower accuracy of calls.
        </div>
    ),
    coverage: (
        <div>
            Coverage is calculated by samtools. For WGS samples, expected values
            are &gt; 25X, and failures are &lt; 10X. For WES samples, expected
            values are &gt; 70X, and failures are &lt; 40X.
        </div>
    ),
};
        ...
        <React.Fragment>
            <div className="row py-0">
                <BioinfoStatsEntry label="Total Number of Reads">
                    {reads.value
                        ? decorateNumberWithCommas(+reads.value)
                        : fallbackElem}
                </BioinfoStatsEntry>
                <BioinfoStatsEntry
                    label="Coverage"
                    popoverContent={bioinfoPopoverContent.coverage}>
                    {coverage.value || fallbackElem}
                    {coverage.value && coverage.flag && (
                        <i
                            className={`icon icon-flag fas text-${flagToBootstrapClass(
                                coverage.flag
                            )} ml-05`}
                        />
                    )}
                ...
                <BioinfoStatsEntry label="Total Number of SNVs/Indels called">
                    {totalSNVIndelVars.value
                        ? decorateNumberWithCommas(+totalSNVIndelVars.value)
                        : fallbackElem}
                </BioinfoStatsEntry>
                <BioinfoStatsEntry
                    label="Transition-Transversion ratio"
                    popoverContent={bioinfoPopoverContent.transTransRatio}>
                    {transTransRatio.value || fallbackElem}
                    {transTransRatio.value && transTransRatio.flag && (
                        <i
                            className={`icon icon-flag fas text-${flagToBootstrapClass(
                                transTransRatio.flag
                            )} ml-05`}
                        />
                    )}
                </BioinfoStatsEntry>
            ...
            <div className="row py-0">
                ...
                <BioinfoStatsEntry
                    label="Predicted Sex"
                    popoverContent={
                        bioinfoPopoverContent.predictedSexAndAncestry
                    }>
                    {mapLongFormSexToLetter(predictedSex.value) || fallbackElem}
                    &nbsp;
                    {!!predictedSex.link && (
                        <a
                            href={predictedSex.link}
                            target="_blank"
                            rel="noreferrer"
                ...
                <BioinfoStatsEntry
                    label="Heterozygosity ratio"
                    popoverContent={bioinfoPopoverContent.heterozygosity}>
                    {heterozygosity.value || fallbackElem}
                    {heterozygosity.value && heterozygosity.flag && (
                        <i
                            className={`icon icon-flag fas text-${flagToBootstrapClass(
                                heterozygosity.flag
                            )} ml-05`}
                ...
                <BioinfoStatsEntry
                    label="SNVs/Indels After Hard Filters"
                    popoverContent={
                        bioinfoPopoverContent.filteredSNVIndelVariants
                    }>
                    {filteredSNVIndelVariants.value
                        ? decorateNumberWithCommas(
                              +filteredSNVIndelVariants.value
                          )
                        : fallbackElem}
            ...
            <div className="row py-0">
                <BioinfoStatsEntry label="Submitted Ancestry">
                    {(submittedAncestry.length > 0 &&
                        submittedAncestry.join(', ')) ||
                        '-'}
                </BioinfoStatsEntry>
                <BioinfoStatsEntry
                    label="Predicted Ancestry"
                    popoverContent={
                        bioinfoPopoverContent.predictedSexAndAncestry
                    }>
                    {predictedAncestry.value || fallbackElem}&nbsp;
                    {!!predictedAncestry.link && (
                        <a
                            href={predictedAncestry.link}
                            target="_blank"
                            rel="noreferrer"
                            className="text-small">
                            (see peddy QC report)
                ...
                <BioinfoStatsEntry
                    label="Structural Variants After Hard Filters"
                    popoverContent={bioinfoPopoverContent.filteredSVVariants}>
                    {filteredSVVariants.value
                        ? decorateNumberWithCommas(+filteredSVVariants.value)
                        : fallbackElem}
                </BioinfoStatsEntry>
            </div>
        </React.Fragment>
...
const BioinformaticsTab = React.memo(function BioinformaticsTab(props) {
    const { context, idToGraphIdentifier, canonicalFamily } = props;
    const {
        sample_processing: sampleProcessing = null,
        sample: caseSample = null,
        vcf_file: vcf = null,
        individual: {
            sex: submittedSex = null,
            ancestry: submittedAncestry = [],
        } = {},
    } = context;
    ...
    const title = (
        <h4 data-family-index={0} className="my-0 d-inline-block w-100">
            ...
            <a
                href={vcfAtId + '#provenance'}
                className="btn btn-sm btn-primary pull-right d-flex align-items-center"
                ...
                <span className="mr-03">View</span>
                <span className="text-600">Provenance Graph</span>
            </a>
        </h4>
    );
    ...
    return (
        <React.Fragment>
            <h1>
                <span className="text-300">Bioinformatics Analysis</span>
            </h1>
            ...
            <div className="tab-inner-container card">
                <h4 className="card-header section-header py-3">
                    Quality Control Metrics (QC)
                </h4>
                <BioinfoStats
                    {...{
                        caseSample,
            ...
            <div className="tab-inner-container card">
                <h4 className="card-header section-header py-3">
                    Multisample Analysis Table
                </h4>
                <div
                    className="card-body family-index-0"
                    ...
                    <CaseSummaryTable
                        ...
                        idx={0}
                        {...{ idToGraphIdentifier, relationshipMapping }}
                    />
                </div>
            </div>
        </React.Fragment>
...
function QCMAccordionToggle({
    ...
    return (
        <div
            onClick={decoratedOnClick}
            className="card-header btn d-flex justify-content-between justify-items-center flex-column flex-sm-row">
...
function QCMAccordion(props) {
    ...
    const { quality_control_metrics = [] } = sampleProcessing;
    ...
    const sortedQCMs = sortAndAddRolePropsToQCMs(
    ...
    return (
        <Accordion defaultActiveKey={sortedQCMs[0].atID} className="w-100">
            {sortedQCMs.map((qcm, i) => (
                <QCMAccordionDrawer
                    key={qcm.individual_accession}
                    idx={i}
                    {...{ idToGraphIdentifier, relationshipMapping, qcmLen }}
                    qualityControlMetrics={qcm}
                />
            ))}
        </Accordion>
    );
...
function QCMAccordionDrawer(props) {
    const { idToGraphIdentifier, qualityControlMetrics, idx, qcmLen } =
        props || {};
    const {
        atID,
        role,
    ...
    const warnFlags = warn.map((flag) => (
        <QCMFlag key={flag} type="warn" title={flag} />
    ));
    const failFlags = fail.map((flag) => (
        <QCMFlag key={flag} type="fail" title={flag} />
    ));
    ...
    return (
        <div
            className={`card border-left-0 border-right-0 ${
                idx === 0 ? 'border-top-0' : ''
            } ${idx === qcmLen - 1 ? 'border-bottom-0' : ''}`}
            key={atID}>
            <QCMAccordionToggle
                eventKey={atID}
                {...{ role, sequencingType, sampleID, specimenType }}>
            ...
            <Accordion.Collapse eventKey={atID}>
                <>
                    <div
                        className="card-body d-flex align-items-center py-1 px-5"
                        style={{
                        ...
                        <a
                            href={atID}
                            className="text-uppercase text-600 d-block text-small mr-2">
                            {individual_id || individual_accession}
                        </a>
                        <span className="gen-identifier text-600 text-serif text-small pt-03">
                            {idToGraphIdentifier[atID]}
                        </span>
                        &nbsp;
                    </div>
...
export function QCMFlag({ type, title, cls = 'm-0 ml-1' }) {
    ...
    return (
        <div
            data-flag={type}
            className={`qcm-flag alert alert-${alertClass} py-1 px-3 text-small border-0 d-flex align-items-center justify-items-center ${cls}`}
            role="alert">
