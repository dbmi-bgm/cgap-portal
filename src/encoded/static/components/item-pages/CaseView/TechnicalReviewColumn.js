...
export class TechnicalReviewColumn extends React.PureComponent {
     ...
     */
    static projectTechnicalReviewInformation(
        projectTechnicalReview,
        lastSavedTechnicalReview
    ) {
        const { '@id': projectTechnicalReviewAtID } =
            projectTechnicalReview || {};
        const {
            '@id': lastSavedTechnicalReviewItemAtID,
            is_saved_to_project: lastSavedIsSavedToProject,
        } = lastSavedTechnicalReview || {};
        ...
        const justRemovedFromProject =
            isTechnicalReviewSavedToProject && !justSavedToProject;

        // Hmm, can rename to: isIndexedToProject, isSavedToProject, isJustRemovedFromProject ?
        return {
            isTechnicalReviewSavedToProject,
            justSavedToProject,
    ...
    constructor(props) {
        ...
        this.updateUnsavedNoteText = _.debounce(
            this.updateUnsavedNoteText.bind(this),
            500
        );
        this.unsetLastSavedTechnicalReviewIfUpdated =
            this.unsetLastSavedTechnicalReviewIfUpdated.bind(this);
        this.handleOpenDropdownCall = this.handleOpenDropdownCall.bind(this);
        this.handleOpenDropdownNoCall =
            this.handleOpenDropdownNoCall.bind(this);
        ...
        this.memoized = {
            compareSavedToCache: memoize(
                TechnicalReviewController.compareSavedToCache
            ),
            projectTechnicalReviewInformation: memoize(
                TechnicalReviewColumn.projectTechnicalReviewInformation
            ),
    ...
    unsetLastSavedTechnicalReviewIfUpdated() {
        ...
        const isSavedTechnicalReviewSynced = this.memoized.compareSavedToCache(
            lastSavedTechnicalReviewForResult,
    ...
    handleOpenDropdownCall() {
        ...
        const {
            [isCNV ? 'cnv' : 'snv']: { call: options },
        } = TechnicalReviewColumn.classificationOptions;
        const projectTechnicalReviewInformation =
            this.memoized.projectTechnicalReviewInformation(
                projectTechnicalReview,
                lastSavedTechnicalReviewForResult || savedTechnicalReview
            );
        ...
        setOpenPopoverData({
            ...
            jsx: (
                <Popover id="technical-review-popover">
                    <Popover.Title
                        className="m-0 text-600 text-uppercase"
                        as="h5">
                        Present
                    </Popover.Title>
                    <Popover.Content className="px-0 py-1">
                        {options.slice(0, 1).map(function (optionName, i) {
                            return (
                                <CallClassificationButton
                                    {...commonBtnProps}
                                    {...{ optionName }}
                                    key={i}
                                    highlightColorStyle="success"
                                />
                            );
                        })}
                    </Popover.Content>
                    <Popover.Title
                        className="m-0 text-600 text-uppercase border-top"
                        as="h5">
    ...
    updateUnsavedNoteText(e) {
        ...
        const { uuid: vsUUID, technical_review: savedTechnicalReview = null } =
            result;
        const { note_text: lastSavedNoteText } =
            lastSavedTechnicalReviewForResult || {};
        ...
        if (nextNoteText === '') {
            if (
                lastSavedNoteText ||
                (typeof lastSavedNoteText === 'undefined' && savedNoteText)
            ) {
    ...
    handleOpenNotesPopover(mouseClickEvent, reRenderOnly = false) {
        ...
        setOpenPopoverData((existingPopoverData) => {
            const { uuid: existingOpenVSUUID, call: existingOpenVSCallType } =
                existingPopoverData || {};

            if (
                reRenderOnly &&
                (existingOpenVSCallType !== null ||
                    existingOpenVSUUID !== vsUUID)
            ) {
            ...
            return {
                ...
                jsx: (
                    ...
                    <Popover id="technical-review-popover">
                        <NotePopoverContents
                            onTextAreaChange={this.updateUnsavedNoteText}
                            disabled={!haveCaseEditPermission}
                            {...{
                                result,
                                lastSavedTechnicalReviewForResult,
    ...
    render() {
        ...
        const { assessment: projectAssessment, note_text: projectNoteText } =
            projectTechnicalReview || {};
        const { call: projectCall, classification: projectClassification } =
            projectAssessment || {};
        const {
        ...
        const showClassification = lastSavedAssessment
            ? lastSavedClassification
            : projectTechnicalReview
        ...
        const noteTextDateLastEdited =
            lastSavedDateTextEdited || savedDateTextEdited || null;
        const callDateLastMade =
            lastSavedDateCallMade || savedDateCallMade || null;
        const isNotePotentiallyOutdated =
            noteTextDateLastEdited &&
            callDateLastMade &&
            noteTextDateLastEdited < callDateLastMade; //lastSavedAssessment && savedTechnicalReviewNoteText && typeof lastSavedNoteText === "undefined";
        ...
        const showNoteText =
            ...
            (!isNoteUnsaved &&
                typeof lastSavedNoteText === 'undefined' &&
                !projectTechnicalReview &&
                savedTechnicalReviewNoteText);

        const notesIconCls =
            'icon text-larger icon-fw icon-sticky-note ' +
            (isNotePotentiallyOutdated
                ? 'far text-danger'
                : showNoteText
                ? 'fas text-secondary'
                : 'far text-muted');

        const recentlySavedAsterisk = (
            <span
                className="text-warning position-absolute ml-n1"
                data-tip="Recently saved and possibly not yet in search results">
                *
            </span>
        );
        ...
        return (
            <div className="w-100 d-flex align-items-center justify-content-around py-1">
                <button
                    ...
                    onClick={this.handleOpenDropdownCall}
                    ref={this.callTrueButtonRef}
                    data-call="true"
                    data-technical-review="true"
                    data-tip={isCallTrue ? showClassification : null}
                    disabled={popoversDisabled && !isCallTrue}>
                    <i
                        className={
                            'icon text-larger icon-fw fas icon-check text-' +
                            callTrueIconColor
                        }
                    />
                    ...
                    {lastSavedAssessment &&
                    (lastSavedCall === true ||
                        (typeof lastSavedCall === 'undefined' &&
                            savedCall === true))
                        ? recentlySavedAsterisk
                        : null}
                </button>
                ...
                <button
                    type="button"
                    className="btn btn-link p-0 text-decoration-none"
                    onClick={this.handleOpenDropdownNoCall}
                    ref={this.callFalseButtonRef}
                    data-call="false"
                    data-technical-review="true"
                    data-tip={isCallFalse ? showClassification : null}
                    disabled={popoversDisabled && !isCallFalse}>
                    <i
                        className={
                            'icon text-larger icon-fw fas icon-times text-' +
                            callFalseIconColor
                        }
                    />
                    {lastSavedAssessment &&
                ...
                <button
                    type="button"
                    className="btn btn-link p-0 text-decoration-none"
                    onClick={this.handleOpenNotesPopover}
                    ref={this.notesButtonRef}
                    data-technical-review="true"
                    data-html
                    data-tip={
                        isNotePotentiallyOutdated
                            ? '<b>This note is potentially outdated:</b><br/>' +
                              showNoteText
                            : showNoteText
                    }
                    ...
                    {typeof lastSavedNoteText !== 'undefined'
                        ? recentlySavedAsterisk
                        : null}
                    {typeof unsavedTechnicalReviewNoteTextForResult !==
                    'undefined' ? (
                        // Will be equal to null if saved text exists but unsaved text is blank (== will remove field upon save).
                        <span
                            className="text-danger text-700 position-absolute ml-n1"
                            data-tip="Note text has not been saved yet.">
                            *
                        </span>
                    ) : null}
                </button>
...
const NotePopoverContents = React.memo(function NotePopover(props) {
    ...
    const showLastTextEditDate =
        lastSavedDateTextEdited || savedDateTextEdited || null;
    const showLastTextEditAuthorName = lastSavedDateTextEdited
        ? lastSavedTextEditedByName
        : savedDateTextEdited
        ? savedTextEditedByName
        : null;
    let showCall;
    ...
    const isSavedToProject =
        justSavedToProject ||
        (projectTechnicalReview && !justRemovedFromProject);
    // If project technical review exists but was not set on this VariantSample, disable it for time being at least.
    const textareaDisabled =
        propDisabled ||
        isSavedToProject ||
        typeof cacheUnsavedTechnicalReviewNoteTextForVSUUID !== 'function' ||
    ...
    function handleSave(e) {
        ...
        const deleteFields = [];
        if (unsavedTechnicalReviewNoteTextForResult === null) {
            // Not 'undefined' (=== in which case button should be disabled)
    ...
    const textareaDefaultValue =
        typeof unsavedTechnicalReviewNoteTextForResult !== 'undefined'
            ...
            : typeof lastSavedNoteText !== 'undefined'
            ? lastSavedNoteText
            : typeof savedTechnicalReviewNoteText !== 'undefined'
            ? savedTechnicalReviewNoteText
            : typeof projectNoteText !== 'undefined'
            ? projectNoteText
            : '';

    return (
        <React.Fragment>
            <Popover.Title className="m-0 text-600" as="h5">
            ...
            <Popover.Content className="p-2">
                ...
                {showCallDate ? (
                    <div className="small">
                        ...
                        <LocalizedTime
                            timestamp={showCallDate}
                            formatType="date-time-sm"
                        />
                        {showCallMadeByName
                            ? ' by ' + showCallMadeByName
                            : null}
                    </div>
                ) : null}
                {/* savedDateModified ?
                ...
                {showCall === null ? (
                    <em>
                        A technical review for this item has yet to be saved
                    </em>
                ) : (
                    <NoteClassificationIndicator
                        {...{ showCall, showClassification }}
                        showAsterisk={isLastSaved}
                    />
                )}
                {isLastSaved ? (
                    <React.Fragment>
                        <div className="small mt-16">
                            {'New Call Made: '}
                            <LocalizedTime
                                timestamp={lastSavedCallDate}
                                formatType="date-time-sm"
                            />
                            {' by you'}
                        </div>
                        <h6 className="mb-04 text-600">
                            Newly-Saved Variant Classification
                        </h6>
                        <NoteClassificationIndicator
                ...
                {showLastTextEditDate ? (
                    <div className="small mt-04">
                        {'Text Last Saved: '}
                        <LocalizedTime
                            timestamp={showLastTextEditDate}
                            formatType="date-time-sm"
                        />
                        {showLastTextEditAuthorName
                            ? ' by ' + showLastTextEditAuthorName
                            : null}
                    </div>
                ) : null}
                <div className="d-flex mt-08">
                    ...
                    <button
                        type="button"
                        className="btn btn-primary ml-04 w-100"
                        disabled>
                        Approve
                    </button>
                </div>
    ...
    return (
        <div
            ...
            {showAsterisk ? (
                <span
                    className="text-700"
                    data-tip="You recently saved this and it may not be yet visible in search results">
                    {' '}
                    *
                </span>
            ) : null}
...
function commonNoteUpsertProcess({
    ...
    const existingTechnicalReviewAtID =
        savedTechnicalReviewItemAtID || lastSavedAtID || null;

    const isCurrentlySavedToProject =
        justSavedToProject ||
        (isTechnicalReviewSavedToProject && !justRemovedFromProject);
    ...
    function postNewNotePromise() {
        console.log(
            'POSTing new Note and PATCHing [Structural]VariantSample.technical_review'
        );
        ...
        return ajax
            .promise(
                ...
                'POST',
                {},
                JSON.stringify(_.omit(createPayload, ...deleteFields))
            )
            .then(function (res) {
                const {
                    '@graph': [{ '@id': newTechnicalReviewAtID }],
                } = res;
                ...
                return ajax.promise(
                    ...
                    'PATCH',
                    {},
                    JSON.stringify({ technical_review: newTechnicalReviewAtID })
                );
            })
            .then(function (vsResponse) {
                // PATCH VariantSample with new technical review.
                const {
                    '@graph': [vsItemFrameObject],
                } = vsResponse;
                const { '@id': vsAtIDRepeated } = vsItemFrameObject;
                if (!vsAtIDRepeated) {
                    throw new Error(
                        'No [Structural]VariantSample @ID returned.'
                    ); // If no error thrown during destructuring ^..
                }
                cacheSavedTechnicalReviewForVSUUID(
                    vsUUID,
                    getNoteDataToCache(payload, techReviewResponse)
                );
            });
    }
    ...
    function patchExistingNotePromise() {
        ...
        const updateHref =
            existingTechnicalReviewAtID +
            '?delete_fields=' +
            ['review'].concat(deleteFields).join(',');
        ...
        return ajax
            .promise(
                ...
                'PATCH',
                {},
                JSON.stringify(_.omit(payload, ...deleteFields))
            )
            .then(function (res) {
                const {
                    '@graph': [{ '@id': technicalReviewAtID }],
                } = res;
        ...
        return ajax
            ...
            .then(function (processItemsResponse) {
                    ...
                    results: {
                        Note: {
                            responses: [
                                {
                                    '@graph': [
                                        technicalReviewPatchResponseItem,
                                    ],
                                },
                            ],
                        },
                ...
                if (endpointResponseStatus !== 'success') {
                    throw new Error(
                        'Failed to update Variant with new Technical Review, check permissions.'
                    );
                }
                const {
                    last_modified: { date_modified },
                ...
                if (is_saved_to_project !== isSavedToProjectExpected) {
                    throw new Error(
                        'We expected Note to have updated its `is_saved_to_project` set to ' +
                            isSavedToProjectExpected
                    );
                }
    ...
    if (shouldRemoveFromProject) {
        ...
        console.log(
            `Will remove technical review ${technicalReviewUUID} from project`
        );
    ...
    if (!existingTechnicalReviewAtID) {
        ...
        if (
            isExistingValue &&
            shouldSaveToProject &&
            !isCurrentlySavedToProject
        ) {
            console.info(
                'Skipping PATCH for same classification value -- will only save to project'
            );
        } else {
    ...
    updatePromise
        .then(function () {
            if (shouldSaveToProject && !shouldRemoveFromProject) {
                ...
                const { '@graph': [technicalReviewItemFrameObject] = [] } =
                    techReviewResponse || {};
                const { uuid: technicalReviewUUIDFromResponse } =
                    technicalReviewItemFrameObject || {};
                const technicalReviewUUID =
                    technicalReviewUUIDFromResponse ||
                    lastSavedUUID ||
                    savedTechnicalReviewUUID;
                if (!technicalReviewUUID) {
                    throw new Error(
                        'No technical review UUID available to be able to save to project'
                    );
                }
                console.log(
                    `Will save technical review ${technicalReviewUUID} to project`
                );
        ...
        .then(function () {
            ...
            try {
                // in case this runs server-side or in a non-standard browser
                skipPopover = JSON.parse(
                    window.sessionStorage.getItem('skip_index_wait_warning')
                );
            } catch (e) {
                // pass
            }
            if (skipPopover) {
                ...
                setOpenPopoverData(function ({ ref: existingBtnRef }) {
                    return {
                        ref: existingBtnRef,
                        jsx: (
                            <SavedTechnicalReviewPopover
                                created={!existingTechnicalReviewAtID}
                            />
                        ),
                    };
                });
                        ...
                        <Popover id="technical-review-popover" key="error">
                            ...
                            <Popover.Content
                                ...
                                <div className="flex-grow-1 px-2">
                                    <h5 className="text-600 my-0">
                                        Failed to save Technical Review
                                    </h5>
                                    <p className="mt-0">
                                        Please check permissions or report to
                                        admins/developers.
                                    </p>
                                </div>
                            </Popover.Content>
...
class CallClassificationButton extends React.PureComponent {
    constructor(props) {
        ...
        this.upsertTechnicalReviewItem =
            this.upsertTechnicalReviewItem.bind(this);
    ...
    upsertTechnicalReviewItem(shouldSaveToProject = false) {
        ...
        const { '@id': lastSavedAtID, assessment: lastSavedAssessment } =
            lastSavedTechnicalReviewForResult || {};
        const { call: lastSavedCall, classification: lastSavedClassification } =
            lastSavedAssessment || {};
        ...
        const isExistingValue =
            existingTechnicalReviewAtID &&
            ((lastSavedCall === callType &&
                lastSavedClassification === optionName) ||
                // Ensure we don't have an empty assessment: {} in lastSavedTechnicalReviewForResult
                (typeof lastSavedAssessment === 'undefined' &&
                    savedCall === callType &&
                    savedClassification === optionName));
    ...
    handleClick(e) {
        ...
        const saveToProject =
            e.target.getAttribute('data-save-to-project') === 'true';
        if (disabled) {
            return false;
        }
    ...
    render() {
        ...
        const { assessment: lastSavedAssessment } =
            lastSavedTechnicalReviewForResult || {};
        const { call: lastSavedCall, classification: lastSavedClassification } =
            lastSavedAssessment || {};
        ...
        const noViewPermissions =
            (savedTechnicalReviewItem && !savedTechnicalReviewItemAtID) ||
            (projectTechnicalReview && !projectTechnicalReviewAtID);
        ...
        const isSavedToVS =
            savedCall === callType && savedClassification === optionName;
        const isAssessmentSavedToProject =
            !justRemovedFromProject &&
            projectCall === callType &&
            projectClassification === optionName;

        // If was recently saved and this existing saved value is now unset (either technical_review.assessment changed or deleted)
        const isLastSaveUnset =
            isSavedToVS && lastSavedAssessment && !isLastSaved;
        ...
        const isDefaultSaveToProject =
            callType === false && optionName === 'Recurrent Artifact';
        ...
        const lastSavedIndicator = isLastSaved ? (
            <span
                ...
                data-tip="You recently saved this value and it may not be yet visible in search results">
                {' '}
                *
            </span>
        ) : isLastSaveUnset ? (
            <i
                className="icon icon-minus-circle fas ml-08"
                data-tip="Previous Value"
            />
        ) : null;
        ...
        if (isDefaultSaveToProject) {
            const saveToProjectBtnClass =
                ...
                (justSavedToProject &&
                    (isLastSaved || (!lastSavedAssessment && isSavedToVS)))
                    ? ` bg-${highlightColorStyle} text-white`
                    : '';

            const saveToProjectBtnTip = `This classification will be ${
                isCurrentlySavedToProject ? 'removed' : 'saved'
            } <b>project-wide</b> for this variant`;

            const saveToVSBtnClass =
                isLastSaved || (isSavedToVS && !isLastSaveUnset)
                    ? ` bg-${highlightColorStyle} text-white`
                    : isLastSaveUnset
            ...
            return (
                <div className="d-flex">
                    <button
                        type="button"
                        className={
                            'dropdown-item pr-16' + saveToProjectBtnClass
                        }
                        onClick={this.handleClick}
                        data-save-to-project={true}
                        disabled={disabled}
                        data-html
                        data-tip={saveToProjectBtnTip}>
                        {optionName}
                        <i className="icon icon-project-diagram fas small ml-16" />
                        {justSavedToProject && !isAssessmentSavedToProject ? ( // Not yet indexed
                            <span
                                className="text-white text-700"
                                data-tip="You recently saved this to project and it may not be yet visible in search results">
                                {' '}
                                *
                            </span>
                        ) : justRemovedFromProject ? (
                    ...
                    <button
                        type="button"
                        className={
                            'px-3 flex-grow-1 dropdown-item border-left' +
                            saveToVSBtnClass
                        }
                        onClick={this.handleClick}
                        disabled={disabled}
                        data-save-to-project={false}
            ...
            return (
                <button
                    type="button"
                    className={'dropdown-item' + saveToVSBtnClass}
                    onClick={this.handleClick}
                    disabled={disabled}>
                    {optionName}
                    {lastSavedIndicator}
...
const SavedTechnicalReviewPopover = React.forwardRef(function (
    { created = false, ...passProps },
    ref
) {
    ...
    return (
        <Popover
            {...passProps}
            id="technical-review-popover"
            key="success-new-review"
            ref={ref}>
            <Popover.Title className="m-0 text-600" as="h5">
                {created ? 'Created' : 'Updated'} Technical Review
            </Popover.Title>
            <Popover.Content style={{ maxWidth: 320 }}>
                <h5 className="my-0">NOTE:</h5>
                <p className="mt-0">
                    It may take some time for your changes to become available
                    in search results, please refresh or search again in a few
                    minutes.
                </p>
                {typeof window !== 'undefined' && window.sessionStorage ? (
                    <label className="d-block text-400 mt-08 pt-08 border-top mb-0">
                        ...
                        />
                        &nbsp;
                        <span className="align-middle ml-04">
                            Check to stop showing this message
                        </span>
                    </label>
                ) : null}
            </Popover.Content>
...
export class TechnicalReviewController extends React.PureComponent {
    ...
    static compareSavedToCache(
        ...
        const {
            last_modified: { date_modified: lastSavedDateModified },
        } = lastSavedTechnicalReviewForResult;
        const {
            last_modified: { date_modified: savedDateModified },
        } = savedTechnicalReview;
    ...
    constructor(props) {
        super(props);
        this.cacheUnsavedTechnicalReviewNoteTextForVSUUID =
            this.cacheUnsavedTechnicalReviewNoteTextForVSUUID.bind(this);
        this.cacheSavedTechnicalReviewForVSUUID =
            this.cacheSavedTechnicalReviewForVSUUID.bind(this);
        this.resetLastSavedTechnicalReview =
            this.resetLastSavedTechnicalReview.bind(this);
        this.state = {
    ...
    cacheUnsavedTechnicalReviewNoteTextForVSUUID(vsUUID, value) {
        this.setState(function ({
            unsavedTechnicalReviewNoteTexts: existingNoteTexts,
        }) {
            const existingValue = existingNoteTexts[vsUUID];
            if (typeof value === 'undefined') {
                if (typeof existingValue !== 'undefined') {
                    return {
                        unsavedTechnicalReviewNoteTexts: _.omit(
                            existingNoteTexts,
                            vsUUID
                        ),
                    };
    ...
    cacheSavedTechnicalReviewForVSUUID(vsUUID, value) {
        this.setState(function ({
            lastSavedTechnicalReview: existingStatePropertyObject,
        }) {
            ...
            if (typeof value === 'undefined') {
                if (typeof existingValue !== 'undefined') {
                    return {
                        lastSavedTechnicalReview: _.omit(
    ...
    render() {
        ...
        const { lastSavedTechnicalReview, unsavedTechnicalReviewNoteTexts } =
            this.state;
        const childProps = {
            ...passProps,
            lastSavedTechnicalReview,
