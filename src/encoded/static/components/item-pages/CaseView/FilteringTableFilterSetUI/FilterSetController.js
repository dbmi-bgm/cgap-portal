...
export class FilterSetController extends React.PureComponent {
    static propTypes = {
        initialFilterSetItem: PropTypes.shape({
            ...
            search_type: PropTypes.oneOf([
                'StructuralVariantSample',
                'VariantSample',
                'Variant',
                'Case',
            ]),
            filter_blocks: PropTypes.arrayOf(
                PropTypes.shape({
    ...
    static updateSelectedFilterBlockQueryFromSearchContextResponse(
        ...
        const { filters: ctxFilters = [], '@id': searchContextAtID } =
            searchContext;
        ...
        if (searchContextAtID) {
            ...
            if (textSearchQuery) {
                ...
                searchContextAtIDParts.search =
                    '?' +
                    queryString.stringify(_.omit(searchContextQuery, 'q'));
        ...
        for (
            var ctxSearchFilterIndx = 0;
            ctxSearchFilterIndx < searchFiltersLen;
            ctxSearchFilterIndx++
        ) {
        ...
        const nextCurrFilterSet = {
            ...currFilterSet,
            filter_blocks: filter_blocks.slice(),
        };
        nextCurrFilterSet.filter_blocks[selectedFilterBlockIdx] = {
            ...nextCurrFilterSet.filter_blocks[selectedFilterBlockIdx],
            query: queryString
                .stringify(searchFiltersQuery)
                .replaceAll('%20', '+'),
        };
     ...
     */
    static createCompoundSearchRequest(
        ...
        if (!currFilterSet) {
            console.error(
                'No current filterset to navigate to. Fine if expected (i.e. initial filterset item still being fetched).'
            );
            return null;
        }
        ...
        const {
            filter_blocks,
            search_type: filterSetSearchType = 'VariantSample',
        } = currFilterSet;

        const { query: globalFlagsQuery = {} } = url.parse(
            searchHrefBase,
            true
        );
        ...
        const selectedFilterBlockQueries =
            selectedIdxCount === 0
                ? filterBlockQueries
                : filterBlockQueries.filter(function (fb, fbIdx) {
                      return selectedFilterBlockIndices[fbIdx];
                  });

        return {
            search_type: searchType,
            global_flags: queryString
                .stringify(globalFlagsQuery)
                .replaceAll('%20', '+'),
    ...
    static getDerivedStateFromProps(props, state) {
        ...
        } else if (filterBlocksLen === 0) {
            // Rare/if-at-all-occuring
            // Clear
            selectedFilterBlockIndices = {};
        } else {
        ...
        let selectedFilterBlockIdxList = Object.keys(
            selectedFilterBlockIndices
        );
        let selectedFilterBlockIdxCount = Object.keys(
        ...
        if (nextCachedCounts[selectedFilterBlockIdx] !== totalCount) {
            // Don't update object reference unless is changed/first-set
            nextCachedCounts = {
                ...nextCachedCounts,
                [selectedFilterBlockIdx]: totalCount,
            };
        }

        // Returns existing `currFilterSet` if no changes in query detected to avoid downstream memoized component re-renders.
        const nextCurrFilterSet =
            FilterSetController.updateSelectedFilterBlockQueryFromSearchContextResponse(
                selectedFilterBlockIdx,
                searchContext,
                currFilterSet,
                excludeFacets
    ...
    static resetState(props) {
        const { initialFilterSetItem, initialSelectedFilterBlockIndices = [] } =
            props;
    ...
    constructor(props) {
        ...
        this.importFromPresetFilterSet = _.throttle(
            this.importFromPresetFilterSet.bind(this),
            500
        );
        // Throttled since usually don't want to add that many so fast..
        this.addNewFilterBlock = _.throttle(
            this.addNewFilterBlock.bind(this),
            750,
            { trailing: false }
        );
        // Throttled, but func itself throttles/prevents-update if still loading last-selected search results.
        this.selectFilterBlockIdx = _.throttle(
            this.selectFilterBlockIdx.bind(this),
            100,
            { trailing: false }
        );
        // Throttled to prevent accidental double-clicks.
        this.removeFilterBlockAtIdx = _.throttle(
            this.removeFilterBlockAtIdx.bind(this),
            250,
            { trailing: false }
        );
        this.setNameOfFilterBlockAtIdx =
            this.setNameOfFilterBlockAtIdx.bind(this);
        this.setTitleOfFilterSet = this.setTitleOfFilterSet.bind(this);
        this.toggleIntersectFilterBlocks = _.throttle(
            this.toggleIntersectFilterBlocks.bind(this),
            250,
            { trailing: false }
        );
    ...
    componentDidUpdate(pastProps, pastState) {
        ...
        const {
            initialFilterSetItem: pastInitialFilterSet,
        ...
        if (searchContext !== pastSearchContext) {
            console.info(
                'FilterSetController: Received new search context / response.'
            );
        ...
        if (initialFilterSetItem !== pastInitialFilterSet) {
            this.setState(
                FilterSetController.resetState(this.props),
                this.navigateToCurrentBlock
            );
            return;
        }
    ...
    importFromPresetFilterSet(presetFilterSet) {
        ...
        if (!currFilterSet) {
            throw new Error(
                "Expected existing current FilterSet to be present; shouldn't be importable til then"
            );
        }

        const { uuid: presetFSUUID, filter_blocks: presetFSBlocks } =
            presetFilterSet;
        ...
        const nextState = FilterSetController.resetState({
            initialFilterSetItem: newFilterSet,
        });
        ...
        this.setState(nextState, this.navigateToCurrentBlock);
    }
    ...
    addNewFilterBlock(newFilterBlock = null) {
        this.setState(function ({ currFilterSet: pastFS }) {
            ...
            if (!name) {
                // Generate new name
                const highestAutoCount = nextFB.reduce(function (
                    m,
                    { name = '' }
                ) {
        ...
        this.setState(
            function (pastState) {
                ...
                if (nextFBLen === 0) {
                    ...
                    throw new Error(
                        'Must have at least one filter block, will not delete last one.'
                    );
                } else if (nextFBLen === 1) {
                    ...
                    Object.keys(pastSelectedIndices).forEach(function (
                        pastSelectedIndex
                    ) {
                ...
                didSelectedIndicesChange = !_.isEqual(
                    pastSelectedIndices,
                    selectedFilterBlockIndices
                );
    ...
    navigateToCurrentBlock() {
        ...
        const virtualCompoundFilterSet =
            FilterSetController.createCompoundSearchRequest(
                selectedFilterBlockIndices,
                currFilterSet,
                searchHrefBase,
                intersectFilterBlocks
        ...
        console.log(
            'Navigating to virtualCompoundFilterSet',
    ...
    selectFilterBlockIdx(index = null, deselectOthers = true) {
        ...
        this.setState(function ({
            ...
            if (index === null) {
                ...
                // Select it
                selectedFilterBlockIndices = deselectOthers
                    ? { [index]: true }
                    : { ...pastSelectedIndices, [index]: true };
            }
            return {
                selectedFilterBlockIndices,
                isSettingFilterBlockIdx: true,
