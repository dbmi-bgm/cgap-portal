...
export const FilterBlock = React.memo(function FilterBlock(props) {
    ...
    const isValidNameContentsToSaveToVariantSampleList = useMemo(
        function () {
            if (allFilterBlockNameQueriesValid) {
                return true;
            }
            const existingSavedFilterBlock =
                savedToVSLFilterBlockQueries[filterName];
            if (!existingSavedFilterBlock) {
                return true;
            }
            return object.compareQueries(
                existingSavedFilterBlock,
                queryString.parse(filterStrQuery)
            );
        },
        [
        ...
        function (e) {
            ...
            if (filterStrQuery && !isDuplicateQuery) {
                const confirmation = window.confirm(
                    'Are you sure you want to delete this filter block? It still has some values.'
                );
    ...
    return (
        <div
            className={cls}
            onClick={!isEditingTitle ? onSelectClick : null}
            data-duplicate-query={isDuplicateQuery}
            data-name-value-differs-in-existing-selection={
                !isValidNameContentsToSaveToVariantSampleList
            }
            tabIndex={!isEditingTitle && filterBlocksLen > 1 ? 0 : null}
            data-tip={
                isDuplicateQuery
                    ? 'Duplicate query of filter block #' +
                      (duplicateQueryIndices[index] + 1)
                    : null
            }>
            {showTitle ? (
                <FilterBlockTitle
                    onRemoveClick={
                        typeof removeFilterBlockAtIdx === 'function'
                            ? onRemoveClick
                            : null
                    }
                    isOnlyFilterBlock={filterBlocksLen === 1}
                    cachedCount={cachedCounts[index]}
                    {...{
                        filterName,
                        index,
                        isEditingTitle,
...
const FilterBlockTitle = React.memo(function FilterBlockTitle(props) {
    ...
    if (isEditingTitle) {
        innerTitle = (
            <form
                className="w-100 d-flex align-items-center mb-0"
                action="#case-info.filtering"
                autoComplete="off"
                onSubmit={function (e) {
                    e.stopPropagation();
                    e.preventDefault();
                ...
                <button
                    type="reset"
                    className="btn btn-sm btn-outline-dark ml-08"
                    onClick={function (e) {
                        e.stopPropagation();
                        e.preventDefault();
                        setIsEditingTitle(false);
                ...
                <button
                    type="submit"
                    className="btn btn-sm btn-outline-success ml-08">
        ...
        const deleteIconCls =
            'icon fas mr-07 clickable icon-times-circle' +
            (!isOnlyFilterBlock ? '' : ' disabled');
        const titleCls =
            'text-small pt-02' +
            (isDuplicateName || !isValidNameContentsToSaveToVariantSampleList
                ? ' text-danger'
                : !filterName
                ? ' text-secondary'
                : '');
            ...
            <React.Fragment>
                {!isLoadingBlock && typeof onRemoveClick === 'function' ? (
                    <i
                        ...
                        onClick={!isOnlyFilterBlock ? onRemoveClick : null}
                        data-tip={
                            !isOnlyFilterBlock
                                ? 'Delete this filter block'
                                : "Can't delete last filter block"
                        }
                    />
                ) : null}
                {isLoadingBlock ? (
                    <i className="icon fas mr-07 icon-circle-notch icon-spin" />
                ) : null}
                <span
                    className={titleCls}
                    data-tip={
                        isDuplicateName
                            ? 'Duplicate title of filter block #' +
                              (duplicateNameIndices[index] + 1)
                            : null
                    }>
                    {filterName || <em>No Name</em>}
                </span>
                {typeof setNameOfFilterBlockAtIdx === 'function' &&
                typeof filterName === 'string' ? (
                    ...
                    <i
                        className="icon icon-pencil-alt fas ml-1 clickable text-smaller align-self-start"
                        onClick={onEditClick}
                    />
                ) : null}
            </React.Fragment>
    ...
    return (
        <div className="d-flex align-items-center px-2 pt-08 pb-04 title-controls-row">
            ...
            <div className="flex-grow-0 pl-16">
                <div
                    className="cached-counts-value"
                    data-count-exists={countExists}
                    data-tip={
                        countExists
                            ? cachedCount +
                              ' results found for this filter block.'
                            : null
                    }>
                    {cachedCount}
                </div>
 ...
 */
export const DummyLoadingFilterBlock = React.memo(
    function DummyLoadingFilterBlock() {
        return (
            <FilterBlock
                filterBlock={{ query: '', name: <em>Please wait...</em> }}
