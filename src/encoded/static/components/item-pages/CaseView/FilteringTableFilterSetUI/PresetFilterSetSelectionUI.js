...
export class PresetFilterSetSelectionUI extends React.PureComponent {
    ...
    static makeCompoundSearchRequest(caseItem, searchType) {
        ...
        const compoundFS = {
            ...
            intersect: false,
            global_flags:
                'sort=default_for_projects&sort=-date_created&limit=250&search_type=' +
                (searchType || 'VariantSample'),
        };

        if (userUUID) {
            compoundFS.filter_blocks.push({
                query: 'preset_for_users=' + encodeURIComponent(userUUID),
                flags_applied: [],
            });
        }
        ...
        if (projectUUID) {
            compoundFS.filter_blocks.push({
                query: 'preset_for_projects=' + encodeURIComponent(projectUUID),
                flags_applied: [],
            });
            compoundFS.filter_blocks.push({
                query:
                    'default_for_projects=' + encodeURIComponent(projectUUID),
                flags_applied: [],
            });
        }
    ...
    constructor(props) {
        super(props);
        this.setPatchingPresetResultUUID =
            this.setPatchingPresetResultUUID.bind(this);
        this.toggleDeletedPresetUUID = this.toggleDeletedPresetUUID.bind(this);
        this.loadInitialResults = _.debounce(
            this.loadInitialResults.bind(this),
            3000
        );
        this.checkForChangedResultsAndRefresh =
            this.checkForChangedResultsAndRefresh.bind(this);
        this.state = {
    ...
    componentDidUpdate(
        { lastSavedPresetFilterSet: pastLastSavedPresetFilterSet },
        { isLoadingPresets: lastIsLoadingPresets }
    ) {
        ...
        if (pastLastSavedPresetFilterSet !== lastSavedPresetFilterSet) {
            ...
            this.checkForChangedResultsAndRefresh(
                `/search/?type=FilterSet&uuid=${lastSavedPresetUUID}&limit=0`
            );
        }
    }
    ...
    loadInitialResults() {
        ...
        const compoundRequest =
            PresetFilterSetSelectionUI.makeCompoundSearchRequest(
                caseItem,
                searchType
            );
        const scopedRequest = (this.currentInitialResultsRequest = ajax
            .promise(
                '/compound_search',
                'POST',
                {},
                JSON.stringify(compoundRequest)
            )
            .then((res) => {
                const { '@graph': presetResults, total: totalResultCount } =
                    res;
    ...
    checkForChangedResultsAndRefresh(
        requestHref,
        conditionCheckFunc = function ({ total: totalCountForThisSearch }) {
            return totalCountForThisSearch > 0;
        },
        delay = 5000
    ) {
            ...
            ajax.promise(requestHref).then((res) => {
                if (conditionCheckFunc(res)) {
                    ...
                    this.setState(function ({
                        checkingForNewFilterSetRequests: oldRequestsObj,
                    }) {
                        const checkingForNewFilterSetRequests = {
                            ...oldRequestsObj,
                        };
                    ...
                    this.setState(function ({
                        ...
                        return {
                            errorMessage:
                                'Timed out waiting/checking for updated preset results. Please come back later to see your changes.',
                            checkingForNewFilterSetRequests,
                        };
                    });
                    console.error(
                        'checkForChangedResultsAndRefresh exceeded request limit',
                        requestCount
                    );
                }
            });
    ...
    render() {
        ...
        const savePresetDropdownCls =
            'btn btn-outline-primary-dark btn-sm text-truncate w-100';
        const btnInner = (
            <div>
                <i className="icon icon-plus-circle fas mr-05" /> Create Preset
                from Current FilterSet
            </div>
        );
        const createPresetBtn = (
            <div
                className={`p-2 bg-white ${
                    isLoadingPresets || !presetResults || !presetResults.length
                        ? ''
                        : 'border-bottom'
                }`}>
                <SaveFilterSetPresetButton
                    {...savePresetDropdownProps}
                    {...{ btnInner }}
                    btnCls={savePresetDropdownCls}
                />
            </div>
        );
        ...
        if (!presetResults || presetResults.length === 0) {
                ...
                body = (
                    <div>
                        <div className="py-4 px-3 bg-white border-bottom">
                            ...
                            <p>
                                Create a FilterSet and then click{' '}
                                <em>Create Preset</em> to generate a preset for
                                your case.
                            </p>
                        </div>
                        {createPresetBtn}
            ...
            const {
                derived_from_preset_filterset:
                    currentCaseDerivedFromPresetUUID = null,
            } = currentCaseFilterSet || {};
            body = (
                <div className="results-container">
                    {presetResults.map(function (presetFilterSet, idx) {
                        const { uuid: thisPresetFSUUID } = presetFilterSet;
                        const isOriginOfCurrentCaseFilterSet =
                            currentCaseDerivedFromPresetUUID ===
                            thisPresetFSUUID;
        ...
        const isCheckingForNewFilterSet =
            Object.keys(checkingForNewFilterSetRequests).length > 0;

        let nextToTitleIcon = null;
        if (errorMessage) {
            nextToTitleIcon = (
                <i
                    className="icon icon-exclamation-triangle fas ml-05 text-small"
            ...
            nextToTitleIcon = (
                <i
                    className="icon icon-circle-notch icon-spin fas ml-07 text-small text-muted"
                    data-tip="Preset(s) below have been updated but this is not yet reflected."
                    data-html
                />
            );
        }
        ...
        return (
            <div className="filterset-preset-selection-body h-100">
                <div className="results-heading my-0 py-2 px-2 bg-light">
                    <div className="row align-items-center">
                        <h5 className="col text-400 my-0">
                            ...
                            <a
                                href="/search/?type=FilterSet"
                                className="text-body"
                                target="_blank"
                                data-delay-show={1000}
                                data-tip="View all saved FilterSets">
                                Presets
                            </a>
                            {nextToTitleIcon}
                        </h5>
                        {!isCheckingForNewFilterSet && !isLoadingPresets ? (
                            <div className="col-auto text-small">
                                ...
                                {(totalResultCount || 0) >= 250 ? (
                                    <i
                                        className="icon icon-exclamation-triangle fas ml-1"
                                        data-tip="Showing most recent 250 results only.<br/><small>(Eventually we'll show more)</small>"
                                        data-html
                                    />
                                ) : null}
                            </div>
                ...
                {!isLoadingPresets && presetResults && presetResults.length > 0
                    ? createPresetBtn
                    : null}
                {body}
            </div>
...
const PresetFilterSetResult = React.memo(function PresetFilterSetResult(props) {
    const {
        ...
        isOriginOfCurrentCaseFilterSet,
        patchingPresetResultUUID,
        setPatchingPresetResultUUID,
        checkForChangedResultsAndRefresh,
        isDeleted,
        toggleDeletedPresetUUID,
    } = props;
    const {
        '@id': presetFSID,
        uuid: presetFSUUID,
    ...
    const isCurrentCaseFilterSetUnchanged =
        isOriginOfCurrentCaseFilterSet &&
        (isOriginalPresetFilterSetLoading ||
            !hasFilterSetChangedFromOriginalPreset);
    ...
    const havePresetFSEditPermission = !!(
        loadedItemView &&
        _.findWhere(loadedItemView.actions || [], { name: 'edit' })
    );
    ...
    const {
        ...
        function () {
            ...
            const { uuid: userUUID = null, project: userProjectUUID } =
                JWT.getUserDetails() || {}; // ( internally calls JSON.parse(localStorage..) ) -- good to memoize
            return {
                userProjectUUID,
                isPresetForUser:
                    userUUID && preset_for_users.indexOf(userUUID) > -1,
                isPresetForProject:
                    userProjectUUID &&
                    preset_for_projects.indexOf(userProjectUUID) > -1,
                isDefaultForProject:
                    userProjectUUID &&
                    default_for_projects.indexOf(userProjectUUID) > -1,
            };
        },
        [presetFilterSet, caseItem]
    );

    const onSelectPresetFilterSet = useCallback(
        function (e) {
            e.preventDefault();
            e.stopPropagation();

            if (warnBeforeImport) {
                const confResult = window.confirm(
                    'You have unsaved changes in your Case FilterSet; copying over blocks from this preset will destroy and overwrite them, continue?'
                );
                if (!confResult) {
                    return false;
                }
    ...
    if (isLoadingItemView) {
        menuOptions.push(
            <div
                className="px-2 py-3 text-larger text-secondary text-center"
                key="loading">
        ...
        if (havePresetFSEditPermission) {
            ...
            menuOptions.push(
                <a
                    key="edit"
                    href={presetFSID + '?currentAction=edit'}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="dropdown-item">
            ...
            if (userProjectUUID && !isPresetForProject) {
                ...
                menuOptions.push(
                    <DropdownItem
                        key="set-project-preset"
                        eventKey="set-project-preset">
    ...
    const onMenuOptionSelect = useCallback(
        function (eventKey, e) {
            e.preventDefault();
            e.stopPropagation();
                    ...
                    .then(function (resp) {
                        ...
                        checkForChangedResultsAndRefresh(
                            ...
                            function (searchResponse) {
                                const { '@graph': [patchedFSFromSearch] = [] } =
                                    searchResponse;
                                const { status: statusFromSearch } =
                                    patchedFSFromSearch || {};
            ...
            if (eventKey === 'set-project-preset') {
                setPatchingPresetResultUUID(presetFSUUID);
                const { preset_for_projects: listOfProjectsPresetFor = [] } =
                    loadedItemView;
                ...
                ajax.promise(
                    ...
                    {},
                    JSON.stringify({
                        preset_for_projects: listOfProjectsPresetFor.concat([
                            userProjectUUID,
                        ]),
                        status: 'in review',
                    })
                    ...
                    .then(function (resp) {
                        ...
                        checkForChangedResultsAndRefresh(
                            ...
                            function (searchResponse) {
                                ...
                                const {
                                    preset_for_projects:
                                        presetForProjectsFromSearch = [],
                                } = patchedFSFromSearch || {};
                                return (
                                    presetForProjectsFromSearch.indexOf(
                                        userProjectUUID
                                    ) > -1
                                );
                            }
    ...
    const presetIconsToShow = (
        <React.Fragment>
            {isPresetForProject ? (
                <i
                    className="mr-05 icon icon-fw icon-user-friends fas text-secondary"
                    data-tip="Project preset"
                />
            ) : null}
            {isPresetForUser ? (
                <i
                    className="mr-05 icon icon-fw icon-user fas text-muted"
                    data-tip="User preset"
                />
            ) : null}
            {isDefaultForProject ? (
                <i
                    className="mr-05 icon icon-fw icon-star fas text-secondary"
                    data-tip="Project default"
                />
            ) : null}
        </React.Fragment>
    );
    ...
    return (
        ...
        <div
            className="preset-filterset-result"
            data-id={presetFSID}
            data-is-origin-of-current-case-filterset={
                isOriginOfCurrentCaseFilterSet
            }
            data-is-current-case-filterset-unchanged={
                isCurrentCaseFilterSetUnchanged
            }
            data-has-been-deleted={isDeleted}>
            <div className="title pl-12 pr-08">
                <h5
                    className="my-0 text-600 flex-grow-1 text-truncate"
                    title={presetFSTitle}>
                    {presetFSTitle}
                </h5>
            </div>
            <div className="info pl-12 pr-08 text-small pb-04">
                <DropdownButton
                    variant="default btn-dropdown-icon mr-05"
                    size="xs"
                    disabled={!!(patchingPresetResultUUID || isDeleted)}
                    onClick={onMenuClick}
                    onSelect={onMenuOptionSelect}
                    title={
                        <i
                            className={
                                'icon text-secondary fas icon-fw icon-' +
                                (isPatchingPreset
                                    ? 'circle-notch icon-spin'
                                    : 'ellipsis-v')
                            }
                            data-tip="View actions"
                        />
                    }>
                    {menuOptions}
                </DropdownButton>
                ...
                <span
                    data-tip={
                        'Created ' +
                        formatDateTime(date_created, 'date-time-md') +
                        ' by ' +
                        presetFSAuthorTitle
                    }
                    data-delay-show={750}>
                ...
                <span className="flex-grow-1 count-indicator-wrapper ml-07 text-right">
                    <CountIndicator
                        count={presetFBLen}
                        data-tip={'Contains ' + presetFBLen + ' filter blocks'}
                        height={18}
                    />
                </span>
                ...
                <div className="pl-08 flex-shrink-0 title-icon-container">
                    <button
                        type="button"
                        className="import-preset-btn btn btn-sm btn-outline-primary-dark"
                        onClick={
                            importBtnDisabled ? null : onSelectPresetFilterSet
                        }
                        disabled={importBtnDisabled}
                        data-tip={
                            importBtnDisabled
                                ? null // Button disabled, effectively
                                : isOriginOfCurrentCaseFilterSet
                                ? 'Reset current case FilterSet blocks to original ones from this preset'
