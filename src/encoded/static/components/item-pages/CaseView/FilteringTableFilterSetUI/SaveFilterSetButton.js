...
export function savedVariantSampleListItemFilterBlockQueryDict(
    ...
    vsSelections.forEach(function (vsSelection) {
        const {
            filter_blocks_used: {
                filter_blocks: selectionBlocksUsed = [],
            } = {},
        } = vsSelection;
        selectionBlocksUsed.forEach(function ({
            name: selectedBlockName,
            query: selectedBlockQuery,
        }) {
            ...
            existingNameDict[selectedBlockName] =
                existingNameDict[selectedBlockName] ||
                queryString.parse(selectedBlockQuery);
        });
    });
 ...
 */
export function validateAllFilterSetBlockNames(
    savedToVSLFilterBlockQueryNameDict,
    filterSetToSave
) {
    ...
    for (var i = 0; i < fbLen; i++) {
        const { name: currentFilterBlockName, query: fbQueryString } =
            currentFilterBlocks[i];
        const existingSavedFilterBlock =
            savedToVSLFilterBlockQueryNameDict[currentFilterBlockName];
        if (existingSavedFilterBlock) {
            const parsedQuery = queryString.parse(fbQueryString);
            const isEqual = object.compareQueries(
                existingSavedFilterBlock,
                parsedQuery
            );
            if (!isEqual) {
                return false;
...
export class SaveFilterSetButtonController extends React.Component {
     ...
     */
    static hasFilterSetChanged(
        savedFilterSet = null,
        currFilterSet = null,
        fieldsToCompare = ['filter_blocks', 'title', 'flags']
    ) {
        if (!savedFilterSet && currFilterSet) {
            ...
            if (
                currFilterBlocks[0].query ||
                currFilterBlocks[0].name !== 'Filter Block 1'
            ) {
                return true;
            }
            return false;
    ...
    constructor(props) {
        ...
        this.memoized = {
            hasFilterSetChanged: memoize(
                SaveFilterSetButtonController.hasFilterSetChanged
            ),
        };
        ...
        this.state = {
            ...
            lastSavedFilterSet:
                filterSet && filterSet['@id'] ? filterSet : null,
    ...
    componentDidUpdate({ currFilterSet: pastFilterSet }) {
        ...
        if (currFilterSet && currFilterSet !== pastFilterSet) {
            const hasFilterSetChanged = this.memoized.hasFilterSetChanged(
                lastSavedFilterSet,
                currFilterSet
            );
            if (hasFilterSetChanged) {
                setIsSubmitting(
                    'Leaving will cause unsaved changes to FilterSet in the "Filtering" tab to be lost. Proceed?'
                );
            } else {
    ...
    saveFilterSet() {
        ...
        this.setState({ isSavingFilterSet: true }, () => {
            if (existingFilterSetID) {
                ...
                if (
                    !activeFilterSetFieldName ||
                    typeof activeFilterSetFieldName !== 'string'
                ) {
                ...
                ajax.promise(
                    ...
                    .then((response) => {
                        const {
                            '@graph': [newFilterSetItemFromResponse],
                        } = response;
                        newFilterSetItemFromPostResponse =
                            newFilterSetItemFromResponse;
                        const { uuid: nextFilterSetUUID } =
                            newFilterSetItemFromResponse;

                        console.info(
                            'POSTed FilterSet, proceeding to PATCH Case[props.activeFilterSetFieldName]',
                            newFilterSetItemFromResponse
                        );
                        ...
                        return ajax.promise(
                            ...
                            {},
                            JSON.stringify({
                                [activeFilterSetFieldName]: nextFilterSetUUID,
                            })
                        );
                    })
                    ...
                    .catch((err) => {
                        console.error(
                            'Error POSTing new FilterSet or PATCHing Case',
                            err
                        );
                        Alerts.queue({
    ...
    render() {
        ...
        const hasCurrentFilterSetChanged = this.memoized.hasFilterSetChanged(
            lastSavedFilterSet,
            currFilterSet
        );
        const childProps = {
...
export function SaveFilterSetButton(props) {
    ...
    return (
        <button
            ...
            data-tip="Save FilterSet for this Case">
            {isSavingFilterSet ? (
