...
export function AddToVariantSampleListButton(props) {
    ...
    const regularTitle = (
        <React.Fragment>
            <strong>{selectedVariantSamples.size}</strong> selected variants
        </React.Fragment>
    );
    ...
    if (isLoadingVariantSampleListItem) {
        ...
        return (
            <button
                ...
                style={style}
                disabled>
                <span>No selected variants</span>
            </button>
        );
        ...
        return (
            <button
                ...
                style={style}
                disabled>
                <span data-tip="No edit permission.">{regularTitle}</span>
            </button>
        );
        ...
        const onButtonClick = function () {
            ...
            function addToSelectionsList(variantSampleSelectionsList) {
                ...
                selectedVariantSamples.forEach(function (
                    variantSampleItem,
                    variantSampleAtID
                ) {
                    const {
                        __matching_filter_block_names:
                            matchingFilterBlockNamesForVS = [],
                        uuid: vsUUID,
                    } = variantSampleItem;
                    const matchingFilterBlocksLen =
                        matchingFilterBlockNamesForVS.length;
                    ...
                    if (matchingFilterBlocksLen === 0) {
                        ...
                        if (selectedFilterBlockIdxCount !== 1) {
                            throw new Error(
                                'Expected only 1 filter block to be used when no `__matching_filter_block_names` present in result.'
                            );
                        }
                        filterBlocksUsed = [
                            filterBlocks[
                                parseInt(selectedFilterBlockIdxList[0])
                            ],
                        ];
                    } else {
                        ...
                        if (selectedFilterBlockIdxCount === 1) {
                            throw new Error(
                                'Expected multiple filter blocks to be selected when `__matching_filter_block_names` is present in result.'
                            );
                        }
                        const matchingFilterBlocksDict = object.listToObj(
                            matchingFilterBlockNamesForVS
                        );
                        filterBlocksUsed = filterBlocks.filter(function (
                            fb,
                            fbIdx
                        ) {
            ...
            if (!variantSampleListItem) {
                ...
                requestPromiseChain = ajax
                    ...
                    .then(function (respVSL) {
                        ...
                        if (vslError || !vslAtID) {
                            throw new Error(
                                "Didn't succeed in creating new VSL Item"
                            );
                        }
                    ...
                    .then(function (respCase) {
                        console.log(
                            'Case PATCH response from after VSL POST',
                            respCase
                        );
                        const {
                            '@graph': [{ '@id': respCaseAtID }],
                            error: caseError,
                        } = respCase;
                        if (caseError || !respCaseAtID) {
                            throw new Error(
                                "Didn't succeed in PATCHing Case Item"
                            );
                        }
                        console.info(
                            'Updated Case.variant_sample_list_id',
                            respCase
                        );
                ...
                requestPromiseChain = ajax
                    ...
                    .then(function (respVSL) {
                        ...
                        const { status, error: vslError } = respVSL;

                        if (status !== 'success' || vslError) {
                            throw new Error(
                                "Didn't succeed in patching VSL Item; check permissions"
                            );
                        }
