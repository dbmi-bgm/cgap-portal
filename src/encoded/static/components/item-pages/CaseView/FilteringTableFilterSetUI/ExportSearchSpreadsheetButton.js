 ...
 */
export const ExportSearchSpreadsheetButton = React.memo(
    function ExportSearchSpreadsheetButton(props) {
        const {
        ...
        const onSelect = useCallback(
            function (eventKey, e) {
                ...
                formRef.current.children[1].value = JSON.stringify(
                    requestedCompoundFilterSet
                );
        ...
        return (
            ...
            <form
                method="POST"
                className="mb-0"
                action="/variant-sample-search-spreadsheet/"
                target="_blank"
                ref={formRef}
                disabled={disabled}>
