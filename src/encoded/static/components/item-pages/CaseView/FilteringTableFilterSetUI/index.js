...
import {
    SaveFilterSetButton,
    validateAllFilterSetBlockNames,
    savedVariantSampleListItemFilterBlockQueryDict,
...
export class FilteringTableFilterSetUI extends React.PureComponent {
     ...
     */
    static buildFacetDictionary(
        schemas = null,
        excludeFacets = null,
        searchType = 'VariantSample'
    ) {
        const schemaFacetDict =
            buildSchemaFacetDictionary(schemas)[searchType] || {};
    ...
    static findDuplicateBlocks(filter_blocks) {
        ...
        filter_blocks.forEach(function ({ name, query }, idx) {
            ...
            for (i = 0; i < idx; i++) {
                if (
                    object.compareQueries(
                        queryString.parse(filter_blocks[i].query),
    ...
    static deriveSelectedFilterBlockIdxInfo(selectedFilterBlockIndices) {
        ...
        const selectedFilterBlockIdxList = Object.keys(
            selectedFilterBlockIndices
        ).map(function (stringIdx) {
    ...
    constructor(props) {
        ...
        this.memoized = {
            buildFacetDictionary: memoize(
                FilteringTableFilterSetUI.buildFacetDictionary,
            ...
            findDuplicateBlocks: memoize(
                FilteringTableFilterSetUI.findDuplicateBlocks
            ),
            deriveSelectedFilterBlockIdxInfo: memoize(
                FilteringTableFilterSetUI.deriveSelectedFilterBlockIdxInfo
            ),
            savedVariantSampleListItemFilterBlockQueryDict: memoize(
                savedVariantSampleListItemFilterBlockQueryDict
            ),
            validateAllFilterSetBlockNames: memoize(
                validateAllFilterSetBlockNames
            ),
        };
    ...
    componentDidUpdate(pastProps, pastState) {
        ...
        const { currFilterSet, selectedFilterBlockIndices, cachedCounts } =
            this.props;
        ...
        if (
            // Rebuild tooltips after stuff that affects tooltips changes.
    ...
    render() {
        const {
            ...
            columnDefinitions,
            sortBy,
            sortColumns, // From SPC/SortController
        ...
        const facetDict = this.memoized.buildFacetDictionary(
            schemas,
            excludeFacets,
            searchType
        );
        const {
            ...
            haveDuplicateQueries,
            haveDuplicateNames,
        } = this.memoized.findDuplicateBlocks(filter_blocks);
        const {
            singleSelectedFilterBlockIdx,
            selectedFilterBlockIdxCount,
            selectedFilterBlockIdxList,
        } = this.memoized.deriveSelectedFilterBlockIdxInfo(
            selectedFilterBlockIndices,
            filterSet
        );

        const filterBlocksLen = filter_blocks.length;
        const allFilterBlocksSelected =
            filterBlocksLen > 0 &&
            (selectedFilterBlockIdxCount === 0 ||
                selectedFilterBlockIdxCount === filterBlocksLen);
        // 0 selectedFilterBlockIdxCount is considered same as all filterblocks selected so we ensure this here.
        const selectedFilterBlockCount = allFilterBlocksSelected
            ? filterBlocksLen
            : selectedFilterBlockIdxCount;

        const { name: currentFilterBlockName = null } =
            (singleSelectedFilterBlockIdx !== null &&
                filter_blocks[singleSelectedFilterBlockIdx]) ||
            {};
        ...
        const savedToVSLFilterBlockQueries =
            this.memoized.savedVariantSampleListItemFilterBlockQueryDict(
                variantSampleListItem
            );
        const allFilterBlockNameQueriesValid =
            this.memoized.validateAllFilterSetBlockNames(
                    ...
                    <AboveTableControlsBaseCGAP
                        ...
                        <div className="col-12 col-lg-4 py-2">
                            <SearchBar
                                ...
                                {...{ isContextLoading }}
                                placeholder={
                                    !currentFilterBlockName
                                        ? 'Select a single filter-block above to search...'
                                        : 'Search...'
                                }
                            />
                        </div>
                        ...
                        <div className="col col-lg-auto pr-06 d-flex">
                            <ExportSearchSpreadsheetButton
                                {...{ requestedCompoundFilterSet, caseItem }}
                            />
                        </div>
                    </AboveTableControlsBaseCGAP>
                </React.Fragment>
        ...
        return (
            ...
            <div className="above-variantsample-table-ui">
                <div
                    className="filterset-outer-container"
                    data-is-open={bodyOpen}>
                    <FilterSetUIHeader
                        {...headerProps}
                    ...
                    <div
                        className={
                            'flex-column flex-md-row d-' +
                            (bodyOpen ? 'flex' : 'none')
                        }>
                        <div className="filterset-preset-selection-outer-column">
                            <PresetFilterSetSelectionUI
                                {...presetSelectionUIProps}
                                {...{ searchType }}
                            />
                        </div>
                        <div className="flex-grow-1">{fsuiBlocksBody}</div>
                    </div>
                </div>
...
const FilterSetUIHeader = React.memo(function FilterSetUIHeader(props) {
        ...
        titleBlock = (
            <button
                ...
                <h4 className="my-0 text-400 text-white">
                    <i
                        className={
                            'small icon icon-fw fas mr-1 icon-' +
                            (bodyOpen ? 'minus' : 'plus')
                        }
                    />
    ...
    if (
        ...
        const err = !allFilterBlockNameQueriesValid
            ? 'Filter block with same name but different query value has been saved to Variant Sample selection list already. Please change Filter Block name below to proceed.'
            : `Filter blocks with duplicate ${
                  (haveDuplicateNames ? 'names' : '') +
                  (haveDuplicateNames && haveDuplicateQueries ? ' and ' : '') +
                  (haveDuplicateQueries ? 'queries' : '')
              } exist below.`;
        warnIcon = (
            <i
                className="icon icon-exclamation-triangle fas align-middle mr-15 text-danger"
                data-tip={err}
            />
        );
    }
    ...
    return (
        <div className="d-flex filter-set-ui-header align-items-center bg-primary-dark text-white pr-16">
            <div className="flex-grow-1 align-self-stretch">{titleBlock}</div>
...
const FilterSetUIBody = React.memo(function FilterSetUIBody(props) {
    ...
    const { filter_blocks = [] } = filterSet || {};
    const { query: currentSingleBlockQuery = null } =
        (singleSelectedFilterBlockIdx !== null &&
            filter_blocks[singleSelectedFilterBlockIdx]) ||
        {};
    ...
    return (
        <div
            className="filterset-blocks-container blocks-outer-container"
            data-all-selected={allFilterBlocksSelected}>
                ...
                <div className="row pb-06 pt-08 px-3 align-items-center text-small">
                    ...
                    <div className="col-12 pb-02 col-sm-auto text-sm-right">
                        {(allFilterBlocksSelected
                            ...
                            : selectedFilterBlockIdxCount +
                              '/' +
                              filterBlocksLen) + ' filter blocks selected'}
                    </div>
                </div>
            ) : null}
            ...
            {filterBlocksLen > 0 ? (
                <div className="blocks-container px-3">
                    {filter_blocks.map(function (filterBlock, index) {
                        const selected =
                            allFilterBlocksSelected ||
                            selectedFilterBlockIndices[index];
                        return (
                            <FilterBlock
                                {...commonProps}
                                {...{ filterBlock, index, selected }}
                                key={index}
                            />
                        );
                    })}
                ...
                <div className="py-3 px-3">
                    <h4 className="text-400 text-center text-danger my-0">
                        No Blocks Defined
                    </h4>
                </div>
            )}

            <FilterSetUIBlockBottomUI
                {...remainingProps}
                {...{
                    selectFilterBlockIdx,
                    allFilterBlocksSelected,
...
function FilterSetUIBlockBottomUI(props) {
    ...
    const onToggleIntersectFilterBlocksBtnClick = useCallback(
        function (e) {
            e.stopPropagation();
            e.preventDefault();
            toggleIntersectFilterBlocks();
        ...
        <div className="row pb-04 pt-16 px-3">
            <div className="col-auto mb-12">
                <div
                    ...
                    <button
                        type="button"
                        className="btn btn-primary-dark d-flex align-items-center fixed-height"
                        onClick={onSelectAllClick}
                        disabled={allFilterBlocksSelected}>
                        <i
                            className={
                                'icon icon-fw far mr-1 icon-' +
                                (allFilterBlocksSelected
                                    ? 'check-square'
                                    : 'square')
                            }
                    ...
                    <button
                        type="button"
                        className="btn btn-primary-dark d-flex align-items-center fixed-height"
                        onClick={onToggleIntersectFilterBlocksBtnClick}
                        disabled={
                            filterBlocksLen < 2 ||
                            singleSelectedFilterBlockIdx !== null
                        }
            ...
            <div className="col-auto mb-12 flex-grow-1 d-flex justify-content-between flex-wrap">
                <div
                    ...
                    <button
                        ...
                        data-tip="Add new blank filter block">
                    ...
                    <button
                        ...
                        disabled={!currentSingleBlockQuery}
