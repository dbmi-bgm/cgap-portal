 ...
 */
export const FieldBlocks = React.memo(function FieldBlocks({
    filterBlock,
    facetDict,
    schemas,
}) {
    ...
    const { correctedQuery, sortedFields, fieldSchemas } = useMemo(
        function () {
            ...
            Object.keys(origQs).forEach(function (k) {
                ...
                if (k.slice(-5) === '.from') {
                    ...
                    if (
                        !facetDict[field] ||
                        typeof facetDict[field].aggregation_type !== 'string'
                    ) {
                        ...
                        field = k;
                        console.error(
                            "Attempted to remove 'from' from field but couldn't succeed",
                            field,
                            facetDict
                        );
                    } else {
                    ...
                    if (
                        ...
                        field = k;
                        console.error(
                            "Attempted to remove 'to' from field but couldn't succeed",
                            field,
                            facetDict
                        );
                    } else {
            ...
            Object.keys(rangeQs).forEach(function (field) {
                ...
                const fieldSchema = (allFieldSchemas[field] = getSchemaProperty(
                    field,
                    schemas,
                    'VariantSample'
                ));
                ...
                const { abbreviation: fieldAbbreviation = null } =
                    fieldSchema || {};
                const title =
                    facetAbbreviation ||
                    fieldAbbreviation ||
                    (facetTitle.length > 5 ? <em>N</em> : facetTitle);
                rangeQs[field] = [
                    <FormattedToFromRangeValue
                        {...{ from, to, facet, title }}
            ...
            termFields.forEach(function (field) {
                ...
                const cleanField = field.endsWith('!')
                    ? field.slice(0, -1)
                    : field;
                allFieldSchemas[field] = getSchemaProperty(
                    cleanField,
                    schemas,
                    'VariantSample'
                );
            ...
            const sortedFields = termFields
                .concat(Object.keys(rangeQs))
                .sort(function (fA, fB) {
        ...
        <div className="d-flex flex-wrap filter-query-viz-blocks px-2">
            {sortedFields.map(function (field, index) {
                return (
                    <FieldBlock
                        {...{ field }}
                        fieldFacet={facetDict[field]}
                        fieldSchema={fieldSchemas[field]}
                        terms={correctedQuery[field]}
                        key={field}
                    />
                );
            })}
...
function FieldBlock({ field, terms, fieldFacet, fieldSchema }) {
    ...
    return (
        <div className="field-block py-1">
            <div className="value-blocks d-flex flex-wrap">{valueBlocks}</div>
