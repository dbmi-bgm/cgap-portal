 ...
 * for example.
 */
function toggleSelectedDiseaseCallable(
    selectedDiseaseIdxMap,
    availableDiseases,
    setSelectedDiseaseIdxMap,
    diseaseStr
) {
    const nextMap = { ...selectedDiseaseIdxMap };
    if (nextMap[diseaseStr]) {
        // Toggle
        ...
        for (
            var lowestUnusedIdx = 1;
            lowestUnusedIdx < 10000;
            lowestUnusedIdx++
        ) {
 ...
 */
export function useDiseaseStrings(
    currFamily,
    ...
    if (diseaseType === 'Phenotypic Features') {
        contextPhenotypicFeatureStrings = getPhenotypicFeatureStrings(
    ...
    const availableDiseases = useMemo(
        function () {
            ...
            return gatherDiseaseItems(currFamily, diseaseType).sort(function (
                { display_title: titleA },
                { display_title: titleB }
            ) {
    ...
    const [selectedDiseaseIdxMap, setSelectedDiseaseIdxMap] = useState(
        initialSelectedDiseaseIdxMap
    );
    ...
    const onToggleSelectedDisease = useCallback(
        function (evt) {
            const diseaseStr = evt.target.getAttribute('name');
            if (!diseaseStr) return;
            return toggleSelectedDiseaseCallable(
                selectedDiseaseIdxMap,
                availableDiseases,
                setSelectedDiseaseIdxMap,
                diseaseStr
...
export const PedigreeTabViewOptionsController = React.memo(
    function PedigreeTabViewOptionsController(props) {
        const { children, ...passProps } = props;
        const { currPedigreeFamily } = passProps;

        const [showOrderBasedName, setShowOrderBasedName] = useState(true);
        const [showAsDiseases, setShowAsDiseases] = useState(
            'Phenotypic Features'
        );
        ...
        const {
            ...
            availableDiseases,
            onToggleSelectedDisease,
        } = useDiseaseStrings(currPedigreeFamily, showAsDiseases);
            ...
            <div className="container-wide">
                <h3 className="tab-section-title">
                    ...
                    <CollapsibleItemViewButtonToolbar windowWidth={windowWidth}>
                        ...
                        <UniqueIdentifiersCheckbox
                            checked={!showOrderBasedName}
                            onChange={onTogglePedigreeOptionCheckbox}
                        />
                        {/* <ShowAsDiseasesDropdown onSelect={setShowAsDiseases} showAsDiseases={showAsDiseases}  /> */}
                        <ShowAsDiseasesToggle
                            onSelect={setShowAsDiseases}
                            showAsDiseases={showAsDiseases}
                        />
                        <FamilySelectionDropdown
                            {...{
                                familiesWithViewPermission,
                                pedigreeFamiliesIdx,
                            }}
...
const UniqueIdentifiersCheckbox = React.memo(
    function UniqueIdentifiersCheckbox({ checked, onChange }) {
        return (
            <Checkbox
                {...{ checked, onChange }}
                className="checkbox-container"
                name="showOrderBasedName">
...
const ShowAsDiseasesToggle = React.memo(function ShowAsDiseasesToggle({
    showAsDiseases,
    onSelect,
}) {
    ...
    options.forEach((opt, idx) => {
        if (opt.diseaseType === showAsDiseases) {
            currIdx = idx;
        }
    });
...
const FamilySelectionDropdown = React.memo(function FamilySelectionDropdown(
    props
) {
    const {
        familiesWithViewPermission,
    ...
    const title = (
        <span>
            Family <strong>{pedigreeFamiliesIdx + 1}</strong>
        </span>
    );
    return (
        <DropdownButton
...
const ShowAsDiseasesDropdown = React.memo(function ShowAsDiseasesDropdown({
    showAsDiseases,
    onSelect,
}) {
    return (
        <DropdownButton
            className="ml-05"
            onSelect={onSelect}
            title={showAsDiseases}
            variant="outline-dark"
            alignRight>
            <DropdownItem
                active={showAsDiseases === 'Phenotypic Features'}
                eventKey="Phenotypic Features">
                Phenotypic Features
            </DropdownItem>
            <DropdownItem
                active={showAsDiseases === 'Disorders'}
                eventKey="Disorders">
                Disorders
            </DropdownItem>
        </DropdownButton>
