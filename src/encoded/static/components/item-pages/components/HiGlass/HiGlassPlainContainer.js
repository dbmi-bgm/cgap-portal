...
export class HiGlassPlainContainer extends React.PureComponent {
    static correctTrackDimensions(hiGlassComponent) {
        requestAnimationFrame(() => {
            _.forEach(
                hiGlassComponent.tiledPlots,
                (tp) => tp && tp.measureSize()
            );
        });
    }
                ...
                () => {
                    setTimeout(() => {
                        this.setState({ higlassInitialized: true }, () => {
                            ...
                            if (
                                onViewConfigUpdated &&
                                typeof onViewConfigUpdated === 'function'
                            ) {
...
const HiGlassPlainContainerBody = React.forwardRef(
    function HiGlassPlainContainerBody(props, ref) {
        const {
            viewConfig,
            options,
            hasRuntimeError,
        ...
        const {
            dependencies: {
                higlass: { version: higlassVersionUsed = null } = {},
            } = {},
        } = packageLockJson || {};
        ...
        if (
            ...
            hiGlassInstance = (
                <div
                    ...
                    ref={instanceContainerRefFunction}>
                    <HiGlassComponent
                        {...{ options, viewConfig, width, height, ref }}
                    />
                </div>
            );
        }
        ...
        return (
            <div
                ...
                style={style}>
                {higlassVersionUsed === null ? null : (
                    <link
                        type="text/css"
                        rel="stylesheet"
                        href={`https://unpkg.com/higlass@${higlassVersionUsed}/dist/hglib.css`}
                        crossOrigin="true"
                    />
                )}
