...
export class ProvenanceGraphStepsFetchingController extends React.PureComponent {
    ...
    static defaultProps = {
        shouldGraphExist: function (context) {
            console.warn(
                'No `shouldGraphExist` prop is set, will make request even if nothing to trace on item.'
            );
            return true;
        },
    };
    ...
    constructor(props) {
        ...
        this.toggleAllRuns = _.throttle(this.toggleAllRuns.bind(this), 1000, {
            trailing: false,
        });
        this.state = {
    ...
    loadGraphSteps() {
        ...
        if (
            typeof shouldGraphExist === 'function' &&
            !shouldGraphExist(context)
        ) {
        ...
        const callback = (res) => {
            if (Array.isArray(res) && res.length > 0) {
                ...
                this.setState({
                    ...
                    isLoadingGraphSteps: false,
                    loadingStepsError:
                        res.message || res.error || 'No steps in response.',
                });
            }
        };
        ...
        const tracingHref =
            ...
            Object.keys(uriOpts)
                .map(function (optKey) {
                    return (
                        encodeURIComponent(optKey) +
                        '=' +
                        encodeURIComponent(uriOpts[optKey])
                    );
                })
                .join('&');
    ...
    toggleAllRuns() {
        let doRequest = false;
        this.setState(
            function ({ includeAllRunsInSteps, isLoadingGraphSteps }) {
                if (isLoadingGraphSteps) {
                    return null;
                }
                doRequest = true;
