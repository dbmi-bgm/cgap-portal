...
export function isNodeQCMetric(node) {
    ...
    if (
        node.meta &&
        node.meta.run_data &&
        node.meta.run_data.type === 'quality_metric'
    )
        return true;
    return false;
}
...
export function doesRunDataExist(node) {
    if (isNodeGroup(node)) {
        return (
            ...
            node.meta.run_data.file.length > 0 &&
            typeof node.meta.run_data.file[0]['@id'] === 'string'
        ...
        // Uncomment this in-line comment once all Workflows have been upgraded and have 'step.inputs[]|outputs[].meta.type'
        return (
            node.meta &&
            node.meta.run_data &&
            node.meta.run_data.file &&
...
export class WorkflowNodeElement extends React.PureComponent {
    ...
    static getFileFormat(node) {
        ...
        const { file: { file_format: fileFileFormat = null } = {} } =
            nodeRunData || {};
    ...
    static getFileFormatString(node) {
        ...
        if (!fileFormatItem) {
            const fileFormatStrDeprecated =
                (node.meta &&
                    typeof node.meta.file_format === 'string' &&
                    node.meta.file_format) ||
                null;
        ...
        return (
            (fileFormatItem &&
                (fileFormatItem.file_format || fileFormatItem.display_title)) ||
            null
        );
    }
    ...
    icon() {
        ...
        const fileFormatAsString =
            WorkflowNodeElement.getFileFormatString(node);
        let iconClass;
        ...
        if (nodeType === 'input-group' || nodeType === 'output-group') {
            ...
            // By file_format
            if (
                fileFormatAsString === 'zip' ||
                fileFormatAsString === 'tar' ||
                fileFormatAsString === 'gz'
            ) {
            ...
            // By meta.type & ioType
                ...
                if (isNodeQCMetric(node)) {
                    ...
                    isNodeParameter(node) ||
                    ioType.indexOf('int') > -1 ||
                    ioType.indexOf('string') > -1
                ) {
    ...
    tooltip() {
        ...
        if (nodeType === 'step') {
            ...
            // Step Title
            output +=
                '<h5 class="text-600 tooltip-title">' +
                ((meta && (meta.display_title || meta.name)) || name) +
                '</h5>';
        }
        ...
        if (nodeType === 'input' || nodeType === 'output') {
            ...
            argumentName =
                argumentName.charAt(0).toUpperCase() + argumentName.slice(1);
            hasRunDataFile = isNodeFile(node) && doesRunDataExist(node);
            const fileTitle =
                hasRunDataFile &&
                (meta.run_data.file.display_title ||
                    meta.run_data.file.accession);
            if (fileTitle) {
                output += '<small>' + argumentName + ' File</small>';
                output +=
                    '<h5 class="text-600 tooltip-title">' + fileTitle + '</h5>';
                output += '<hr class="mt-08 mb-05"/>';
            }
            if (argumentName === 'Input' || argumentName === 'Output') {
                argumentName +=
                    ' Argument &nbsp; <span class="text-500 text-monospace">' +
                    name +
                    '</span>';
            }
            output +=
                '<small class="mb-03 d-inline-block">' +
                argumentName +
                '</small>';
        }
        ...
        const fileSize =
            hasRunDataFile &&
            typeof meta.run_data.file.file_size === 'number' &&
            meta.run_data.file.file_size;
        if (fileSize) {
            output +=
                '<div class="mb-05"><span class="text-300">Size:</span> ' +
        ...
        if (
            ...
            meta &&
            meta.workflow &&
            meta.workflow.display_title
        ) {
            // Workflow
            //title
            output +=
                '<hr class="mt-08 mb-05"/><div class="mb-05 mt-08"><span class="text-600">Workflow: </span><span class="text-400">' +
                node.meta.workflow.display_title +
                '</span></div>';
        }
        ...
        const description =
            ...
            (meta.run_data &&
                meta.run_data.meta &&
                meta.run_data.meta.description) ||
            (meta.run_data &&
                meta.run_data.file &&
                typeof meta.run_data.file === 'object' &&
                meta.run_data.file.description);
        if (description) {
            output += '<hr class="mt-05 mb-05"/>';
            output +=
                '<small class="mb-05 d-inline-block">' +
                description +
                '</small>';
        }
    ...
    aboveNodeTitle() {
        ...
        if (node.nodeType === 'step' && node.meta.uuid) {
            if (
                ...
                return (
                    <div {...elemProps}>
                        {_.map(
                            node.meta.analysis_step_types,
                            valueTransforms.capitalize
                        ).join(', ')}
                    </div>
                );
            }
        ...
        if (typeof node.ioType === 'string') {
            return (
                <div {...elemProps}>
                    {valueTransforms.capitalize(node.ioType)}
                </div>
            );
        }
    ...
    belowNodeTitle() {
        ...
        */
        ...
        if (
            node.nodeType === 'step' &&
            node.meta &&
            Array.isArray(node.meta.software_used) &&
            node.meta.software_used.length > 0 &&
            node.meta.software_used[0].title
        ) {
            return (
                <div {...elemProps}>
                    {_.map(node.meta.software_used, softwareTitle)}
                </div>
            );
        }
        ...
        if (isNodeFile(node) && doesRunDataExist(node)) {
            ...
            if (node.meta && node.meta.file_type) {
                ...
                node.meta.run_data &&
                node.meta.run_data.file &&
                typeof node.meta.run_data.file === 'object' &&
                node.meta.run_data.file.file_type
            ) {
    ...
    nodeTitle() {
            ...
            if (Array.isArray(files)) {
                ...
                return (
                    <div className="node-name">
                        {this.icon()}
                        <b>{files.length - 1}</b> similar file
                        {len === 1 ? '' : 's'}
                    </div>
                );
            }
        ...
        if (isNodeFile(node) && doesRunDataExist(node)) {
            const {
                file: { accession, display_title },
            } = run_data;
            return (
                <div
                    className={
                        'node-name' + (accession ? ' text-monospace' : '')
                    }>
                    {this.icon()}
                    {typeof file === 'string'
                        ? ioType
                        : accession || display_title}
                </div>
            );
        }
        ...
        if (isNodeParameter(node) && doesRunDataExist(node)) {
            return (
                <div className="node-name text-monospace">
                    {this.icon()}
                    {run_data.value}
                </div>
            );
    ...
    qcMarker() {
        ...
        const qcStatus =
            qc.overall_quality_status &&
            qc.overall_quality_status.toLowerCase();
        const markerProps = {
            className: 'qc-present-node-marker',
            'data-tip':
                'This file has a quality control metric associated with it.',
            children: 'QC',
            key: 'qc-marker',
        };
        ...
        if (qcStatus) {
            if (qcStatus === 'pass') markerProps.className += ' status-passing';
                ...
                markerProps.className += ' status-warning';
            else if (qcStatus === 'error')
                markerProps.className += ' status-error';
        }
