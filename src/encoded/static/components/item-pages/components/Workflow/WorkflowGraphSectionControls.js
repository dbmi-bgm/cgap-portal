...
export const WorkflowGraphSectionControls = React.memo(
    function WorkflowGraphSectionControls(props) {
        const {
            showReferenceFiles,
            showParameters,
            showIndirectFiles,
...
function RowSpacingTypeSelect(props) {
    ...
    return (
        <DropdownButton
            onSelect={onSelect}
            title={titleMap[rowSpacingType]}
            variant="outline-dark"
            alignRight>
            <DropdownItem
                active={rowSpacingType === 'compact'}
                eventKey="compact">
                Centered
            </DropdownItem>
            <DropdownItem
                active={rowSpacingType === 'stacked'}
                eventKey="stacked">
                Stacked
            </DropdownItem>
            <DropdownItem active={rowSpacingType === 'wide'} eventKey="wide">
                Spread
            </DropdownItem>
        </DropdownButton>
    );
...
function IndirectFilesCheckbox({ checked, onChange, disabled }) {
    ...
    return (
        <Checkbox
            ...
            onChange={onChange}
            disabled={disabled || checked === null}
            className="checkbox-container for-state-showIndirectFiles"
...
function ParametersFileCheckbox({ checked, onChange, disabled }) {
    ...
    return (
        <Checkbox
            ...
            onChange={onChange}
            disabled={disabled || checked === null}
            className="checkbox-container for-state-showParameters"
            name="showParameters">
            Parameters
        </Checkbox>
    );
