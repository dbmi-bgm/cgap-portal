...
export class BadgesTabView extends React.PureComponent {
    static getTabObject(props) {
        ...
        const badgesByClassification =
            BadgesTabView.badgesByClassification(context);
        ...
        const titleStr =
            ' ' + badgeSingularTitle + (badgeListLen > 1 ? 's' : '');

        return {
            tab: (
                <span>
                    <SummaryIcon context={context} />
                    {titleStr}
                </span>
            ),
    ...
    static getBadgesList = memoize(function (context) {
        var badges =
            context &&
            context['aggregated-items'] &&
            context['aggregated-items'].badges;
    ...
    static classificationMapAsObject = memoize(function (
        classificationMap = BadgesTabView.classificationMap
    ) {
        return object.mapToObject(classificationMap);
    });

    static badgeClassification(
        badge,
        propMapping = BadgesTabView.classificationMap
    ) {
        var badgeItem = badge && badge.item && badge.item.badge,
            propertyToTestPresenceOf,
            classificationTitle;

        for ([propertyToTestPresenceOf, classificationTitle] of propMapping) {
            if (
                typeof badgeItem[propertyToTestPresenceOf] === 'string' &&
                badgeItem[propertyToTestPresenceOf]
            ) {
    ...
    static badgeTitle(
        ...
        if (!badgeClassification) {
            badgeClassification = BadgesTabView.badgeClassification(
                badge,
                propMapping
            );
        }
        const inversedPropMapping = _.invert(
            BadgesTabView.classificationMapAsObject(propMapping)
        );
     ...
     */
    static badgesByClassification = memoize(function (
        context,
        classificationPropMapping = BadgesTabView.classificationMap
    ) {
        const badges = BadgesTabView.getBadgesList(context);
        if (!badges) return null;

        const groupedBadges = _.groupBy(badges, function (badge) {
            return BadgesTabView.badgeClassification(
                badge,
                classificationPropMapping
            );
        });
    ...
    render() {
        ...
        const body = _.map(
            ...
            _.filter(
                [...BadgesTabView.classificationMap.values()],
                function (classificationTitle) {
                    return (
                        typeof badgesByClassification[classificationTitle] !==
                        'undefined'
                    );
                }
            ),
            function (badgeClassification) {
                const badgesForClassification =
                    badgesByClassification[badgeClassification];
                const badgesLen = badgesForClassification.length;
                const className =
                    'badge-classification-group' +
                    (classificationsCount > 1 ? ' col-lg-6' : '');
                return (
                    <div
                        className={className}
                        data-badge-classification={badgeClassification}
                        key={badgeClassification}>
                        {classificationsCount > 1 ? (
                            <h5 className="text-500 mt-0">
                                {badgesLen +
                                    ' ' +
                                    badgeClassification +
                                    (badgesLen > 1 ? 's' : '')}
                        ...
                        {_.map(badgesForClassification, function (badge, idx) {
                            const atId =
                                ...
                                badge.item &&
                                badge.item.badge &&
                                object.itemUtil.atId(badge.item.badge);
                            const parent = badge && badge.parent;
                            const key =
                                atId && parent ? parent + ' / ' + atId : idx;
                            return (
                                <BadgeItem
                                    {...badge}
                                    {...{ context, schemas, key }}
        ...
        const badgeSingularTitle =
            classificationsCount > 1 ? 'Badge' : badgeClassificationList[0];

        return (
            <div className="overflow-hidden">
                <h3 className="tab-section-title">
                    <span className="text-400">{badgeListLen}</span>
                    {' ' + badgeSingularTitle + (badgeListLen > 1 ? 's' : '')}
                </h3>
...
class SummaryIcon extends React.PureComponent {
    static getClassificationRatios = memoize(function (
        context,
        classificationPropMapping = BadgesTabView.propMapping
    ) {
        const badgesGroupedByClassification =
            BadgesTabView.badgesByClassification(
                context,
                classificationPropMapping
            );
        ...
        for (i = 0; i < badgeClassificationsLen; i++) {
            ...
            totalBadges +=
                badgesGroupedByClassification[currClassification].length;
        }

        for (i = 0; i < badgeClassificationsLen; i++) {
            currClassification = badgeClassifications[i];
            classificationRatios[currClassification] =
                badgesGroupedByClassification[currClassification].length /
                totalBadges;
        }
    ...
    static defaultProps = {
        ...
        classificationSingleItemMap: {
            Commendation: function commendationBadge(props) {
                ...
                const countCommendations =
                    badgesByClassification.Commendation.length;
                const tooltip =
                    countCommendations +
                    ' Commendation' +
                    (countCommendations > 1 ? 's' : '');
                return (
                    <div
                        className="d-inline-block pie-chart-icon-container mr-08"
                        data-tip={tooltip}>
                        <svg
                            height={size}
                            width={size}
                            style={{ verticalAlign: 'middle' }}>
                            <EmbeddableSVGBadgeIcon
                                badgeType="gold"
                                size={size}
                            />
                        </svg>
            ...
            Warning: function warningBadge(props) {
                ...
                const tooltip =
                    countWarnings + ' Warning' + (countWarnings > 1 ? 's' : '');
                return (
                    <span className="active">
                        <i
                            className="icon icon-fw icon-exclamation-triangle fas"
                            data-tip={tooltip}
                        />
                    </span>
                );
    ...
    generateArcs(classificationRatioPairs) {
        // memoized in constructor
        const { innerRadius, classificationColorMap, size } = this.props;
        const outerRadius = size / 2;
        const pieChartDims = this.pieGenerator(
            _.pluck(classificationRatioPairs, 1)
        );
        const arcGenerator = arc()
            .innerRadius(innerRadius)
            .outerRadius(outerRadius);
        return _.map(
            classificationRatioPairs,
            function ([classificationTitle, classificationRatio], idx) {
                return (
                    <path
                        d={arcGenerator(pieChartDims[idx])}
                        className={'path-for-' + classificationTitle}
                        style={{
                            fill:
                                classificationColorMap[classificationTitle] ||
                                '#ccc',
                        }}
                    />
                );
    ...
    generatePieChart(classificationRatioPairs) {
        ...
        const tooltip = _.map(
            classificationRatioPairs,
            function ([classificationTitle, classificationRatio]) {
                const badgesForClassificationLen =
                    badgesByClassification[classificationTitle].length;
                return (
                    badgesForClassificationLen +
                    ' ' +
                    classificationTitle +
                    (badgesForClassificationLen > 1 ? 's' : '')
                );
            }
        ).join(' + ');
            ...
            <div
                ...
                <svg
                    ...
                    <g
                        ...
                        {this.generateArcs(classificationRatioPairs)}
                    </g>
                </svg>
            </div>
        );
    ...
    render() {
        ...
        const classificationRatios =
            SummaryIcon.getClassificationRatios(context);
        ...
        if (classificationRatioPairs.length === 1) {
            ...
            if (
                singleClassificationTitle &&
                classificationSingleItemMap &&
                classificationSingleItemMap[singleClassificationTitle]
            ) {
                return classificationSingleItemMap[singleClassificationTitle](
                    this.props
                );
            }
        }
...
class BadgeItem extends React.PureComponent {
    ...
    static getParent(embedded_path, parentID, context) {
        ...
        const embedPath = Array.isArray(embedded_path)
            ? embedded_path.slice(0) // Assume has been called recursively, without 'badges' already.
        ...
        while (embedPath.length > 0) {
            ...
            if (Array.isArray(currEmbedItem)) {
                ...
                for (arrIdx = 0; arrIdx < currEmbedItem.length; arrIdx++) {
                    arrRes = BadgeItem.getParent(
                        embedPath,
                        parentID,
                        currEmbedItem[arrIdx]
                    );
    ...
    render() {
        ...
        if (parentID && object.itemUtil.atId(context) !== parentID) {
            ...
            const parentTypeTitle =
                parentItem &&
                schemaTransforms.getItemTypeTitle(parentItem, schemas);
            const parentDisplayTitle = parentItem && parentItem.display_title;
            if (parentTypeTitle && parentDisplayTitle) {
                // These two props are default embeds. If not present is most likely due to lack of view permissions.
                titleToShow = (
                    <React.Fragment>
                        {parentTypeTitle}{' '}
                        <span className="text-600">{parentDisplayTitle}</span>
                    </React.Fragment>
                );
            ...
            linkMsg = (
                <div className="mt-02">
                    <a href={parentID} data-tip={tooltip}>
                        View {titleToShow}
                    </a>
                </div>
            );
        ...
        return (
            <div className="badge-item">
                <div className="row flexrow">
                    <div className="col-12 col-sm-2 icon-columns fas">
                        {image}
                    </div>
                    <div className="col-12 col-sm-10 title-col">
                        <div className="inner mb-05">
                            <h4 className="text-500 mb-0 mt-0">
                                ...
                                {description ? (
                                    <i
                                        className="icon icon-fw icon-info-circle fas ml-05"
...
export class EmbeddableSVGBadgeIcon extends React.PureComponent {
    static linearGradientDefinition(type = 'gold') {
        // TODO: Implement other colors/types
        ...
        if (type === 'gold-deprecated') {
            ...
            return (
                <linearGradient
                    ...
                    <stop
                        offset="0"
                        style={{ stopOpacity: 1, stopColor: '#dbba67' }}
                    />
                    <stop
                        offset="0.514543"
                        style={{ stopOpacity: 1, stopColor: '#f8e78f' }}
                    />
                    <stop
                        offset="1"
                        style={{ stopOpacity: 1, stopColor: '#e0c26e' }}
                    />
                </linearGradient>
            );
        }
        ...
        if (type === 'gold') {
            ...
            return (
                <linearGradient
                    ...
                    <stop
                        offset="0"
                        style={{ stopOpacity: 1, stopColor: '#d29a11' }}
                    />
                    <stop
                        offset="0.336498"
                        style={{ stopOpacity: 1, stopColor: '#dcae3c' }}
                    />
                    <stop
                        offset="1"
                        style={{ stopOpacity: 1, stopColor: '#eed781' }}
                    />
                </linearGradient>
            );
        }
        ...
        return (
            <g transform="matrix(1.3333333, 0, 0, -1.3333333, -1, 130)">
                <g transform="translate(-6.8762048,-4.9273304)">
                    <path
                        d="m 26.7374,12.0591 23.046,28.77 -15.008,12.022 -23.469,-29.297"
                        style={style}
                    />
                    <path
                        d="m 18.1642,25.7154 11.924,-9.368 -9.368,-11.923"
                        style={style}
                    />
                    <path
                        d="m 5.5362,16.3135 9.492,11.824 11.824,-9.492"
                        style={style}
                    />
                </g>
                <g transform="translate(-6.8762048,-4.9273304)">
                    <path
                        d="m 95.7296,22.6348 -21.179,30.171 -15.738,-11.047 21.565,-30.725"
                        style={style}
                    />
                    <path
                        d="m 80.2208,18.2227 12.328,8.828 8.828,-12.328"
                        style={style}
                    />
                    <path
                        d="m 85.7169,3.4693 -8.698,12.42 12.419,8.698"
                        style={style}
                    />
                </g>
            </g>
    ...
    render() {
        ...
        return (
            <g transform={'scale(' + scale + ')'}>
                <defs>
                    {EmbeddableSVGBadgeIcon.linearGradientDefinition(badgeType)}
                </defs>
                {ribbons && EmbeddableSVGBadgeIcon.ribbons()}
                <path
                    style={{
                        fill: 'url(#embeddable_badge_svg_gradient_definition)',
                    }}
