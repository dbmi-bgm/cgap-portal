...
class CohortStatisticalAnalysisTableComponent extends React.PureComponent {
            ...
            .then((header) => {
                ...
                Promise.all(dataPromises)
                    .then((values) => {
                        ...
                        this.geneListFilteredVcfRecords = Object.values(
                            this.vcfRecords
                        );
                    ...
                    .catch((error) => {
                        console.error(
                            'Error loading data from VCF file: ',
                            error
                        );
                    });
            })
    ...
    showMore(event) {
        ...
        this.setState((prevState) => ({
            numRows: newNumRows,
            vcfRecordsToDisplay: this.state.vcfRecordsFiltered.slice(
                0,
                newNumRows
            ),
        }));
            ...
            selectedGenes.forEach((gene) => {
                ...
                if (record) {
                    ...
                    console.warn(
                        'Gene ' +
                            gene +
                            ' from gene list not found in available genes.'
                    );
                }
            });
        }
            ...
            headerCols.push(
                <th key={test}>
                    ...
                    <i
                        ...
                        role="button"
                        data-testname={test}
                        onClick={this.handleSortIconClick}></i>
                </th>
            );
        });
    ...
    getTableBody() {
        ...
        this.state.vcfRecordsToDisplay.forEach((record) => {
            ...
            this.state.activeTests.forEach((test) => {
                ...
                const val =
                    record[currentName] < 0
                        ? '-'
                        : format('.2f')(record[currentName]);
