...
export class FileWrapper extends React.PureComponent {
    ...
    constructor() {
        super();
        this.memoized = {
            findFieldsWithDocumentsOrImages: memoize(
                FileWrapper.findFieldsWithDocumentsOrImages
            ),
        };
    }
    ...
    render() {
        ...
        const fieldsToRender =
            this.memoized.findFieldsWithDocumentsOrImages(indvSchema);
        ...
        return fieldsToRender.map(function ({ property, linkToItemType }) {
            ...
            const fileSchema =
                linkToItemType === 'Document' ? docSchema : imageSchema;
            return (
                <FileArrayField
                    {...{
                        files,
                    ...
                    key={property}
                    fieldType={property}
                    fieldName={linkToItemType}
                    fieldDisplayTitle={indvProperties[property]['title']}
                />
            );
        });
    }
    ...
    return (
        <div className="detail-row" data-describing={fieldDisplayTitle}>
            ...
            <ul className="mb-05">
                {files.map(function ({
                    '@id': fileID,
                    display_title: fileDisplayTitle,
                }) {
                    return (
                        <li key={fileID}>
            ...
            {haveEditPermission ? (
                <DragAndDropFileUploadController
                    {...{
                        fieldDisplayTitle,
                        fieldType,
                        fieldName,
                    ...
                    lab={null}
                    multiselect
                    cls="btn btn-sm btn-outline-dark mt-05"
                    requestVerificationMsg={
                        <span>
                            I certify that my file(s) do not contain{' '}
                            <a
                                href="https://www.hipaajournal.com/considered-phi-hipaa/"
                                target="_blank"
                                rel="noreferrer">
