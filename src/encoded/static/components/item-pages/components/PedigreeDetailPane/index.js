...
export class PedigreeDetailPane extends React.PureComponent {
    ...
    render() {
        ...
        if (!selectedNode || !isRelationshipNode) {
            // If no isRelationshipNode func, lib hasn't loaded yet (unlikely case).
    ...
    if (!availableDiseases || availableDiseases.length === 0) {
        body = (
            <div className="detail-row text-secondary">
                <em>
                    No <span className="text-lowercase">{showAsDiseases}</span>{' '}
                    have yet been defined for any individuals in this pedigree.
                </em>
            </div>
        );
    } else {
        body = (
            <React.Fragment>
                <div className="detail-row small text-secondary">
                    Toggle which{' '}
                    <span className="text-lowercase">{showAsDiseases}</span> to
                    color-code in the pedigree
                </div>
                <DiseasesLegend
                    {...{
                        availableDiseases,
