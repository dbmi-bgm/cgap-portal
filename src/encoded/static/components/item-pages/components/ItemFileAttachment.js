...
export class ItemFileAttachment extends React.PureComponent {
    ...
    size(){
        ...
        return (
            <div className="mb-1">
                <i className="icon icon-fw far icon-hdd" data-tip={(tip && tip.description) || null} />&nbsp; { valueTransforms.bytesToLargerUnit(attachment.size) }
            </div>
        );
    }
            ...
            contents = (
                <div className={includeTitle ? 'mt-1' : null}>
                    <ViewFileButton filename={download || null}
                        ...
                        className={'text-truncate mb-1 w-100'} />
                    <div>
                        { this.size() }
                        { this.md5sum() }
                        { this.attachmentType() }
