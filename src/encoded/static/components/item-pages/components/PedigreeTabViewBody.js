...
export class PedigreeTabViewBody extends React.PureComponent {
    ...
    static isViewFullscreen(windowHeight) {
        ...
        const fullscreenElem =
            ...
            document.mozFullscreenElement ||
            document.webkitFullscreenElement;
        const pedigreeContainerElem = document.getElementById(
            'pedigree-viz-container-cgap'
        );
        if (
            fullscreenElem &&
            pedigreeContainerElem &&
            pedigreeContainerElem === fullscreenElem
        ) {
            return true;
        }
        /*
    ...
    constructor(props) {
        super(props);
        this.onBrowserSizeChange = _.throttle(
            this.onBrowserSizeChange.bind(this),
            500
        );
    ...
    onBrowserSizeChange() {
        ...
        setTimeout(() => {
            ...
            const isBrowserFullscreen =
                PedigreeTabViewBody.isBrowserFullscreen(wh1);
            ...
            setTimeout(() => {
                ...
                const isPedigreeFullscreen =
                    PedigreeTabViewBody.isViewFullscreen(wh2);
                this.setState({
                    isBrowserFullscreen,
    ...
    renderDetailPane(pedigreeVizProps) {
        ...
        return (
            <PedigreeDetailPane {...pedigreeVizProps} {...passedDownProps} />
        );
    }
    ...
    render() {
        ...
        const { default: PedigreeViz, PedigreeVizView } =
            PedigreeVizLibrary || {};
        ...
        */
        ...
        if (!PedigreeVizLibrary) {
            body = <div className="py-3 text-center">Loading...</div>;
        } else if (graphData) {
        ...
        return (
            <div id={containerId} className={cls}>
                <FullHeightCalculator
                    {...{ windowWidth, windowHeight, propName, heightDiff }}>
                    {body}
                </FullHeightCalculator>
            </div>
        );
...
export class PedigreeFullScreenBtn extends React.PureComponent {
    ...
    render() {
        ...
        return (
            <button
                type="button"
                className="btn btn-outline-dark ml-05 d-none d-md-inline-block"
                onClick={this.onClick}
                data-tip="Maximize pedigree to take up full screen">
