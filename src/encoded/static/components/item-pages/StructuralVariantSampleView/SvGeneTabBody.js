...
function SVGeneDisplayTitleColumnWrapper(props) {
    ...
    if (selectedGenes && onSelectGene && showInterpretation) {
        //savedVariantSampleIDMap
        highlightedSelector = (
            <HighlightedGeneSelector
                {...{
                    selectedGenes,
                    onSelectGene,
                    savedVariantSampleIDMap,
...
const SVGeneDisplayTitleColumn = React.memo(function SVGeneDispalyTitleColumn(
    props
) {
    const {
        transcript,
    ...
    const rows = [
        <span key="gene" className="d-block text-truncate">
            {display_title}{' '}
        </span>,
    ];
    ...
    if (filteredTranscripts.length > 0) {
        ...
        const { csq_mane = null, csq_feature = null } =
            filteredTranscripts[transcriptIndex || 0];
        const transcriptDisplay = csq_mane || csq_feature;
        rows.push(
            <span key="transcript" className="font-italic d-block text-small">
                {transcriptDisplay}{' '}
            </span>
        );
    }
...
export const HighlightedGeneSelector = React.memo(
    function HighlightedGeneSelector(props) {
        const {
            selectedGenes,
            result,
            onSelectGene,
        ...
        return (
            <IconCheckbox
                ...
                checked={isChecked}
                onChange={onChange}
                disabled={isLoadingVariantSampleListItem || isPrevSaved}
            />
        );
    }
);
