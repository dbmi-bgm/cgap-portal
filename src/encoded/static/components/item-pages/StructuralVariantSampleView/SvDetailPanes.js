...
import {
    ExternalDatabasesSection,
    GeneOverview,
    ConstraintScoresSection,
    GeneTranscriptDisplayTitle,
...
export function SvGeneDetailPane(props) {
    ...
    const [currentTranscriptIdx, setTranscriptIdx] = useState(
        getInitialTranscriptIndex(resultTranscripts)
    );

    let usePadWidth = paddingWidth || 0;
    if (paddingWidthMap) {
        usePadWidth =
            paddingWidthMap[responsiveGridState(windowWidth)] || paddingWidth;
    }
                ...
                <div className="col-12 col-md-6 d-flex flex-column">
                    <div className="inner-card-section flex-grow-0 pb-2 pb-md-0">
                        ...
                        <div className="info-body">
                            <ConsequenceOfSVSection
                                currentTranscript={
                                    resultTranscripts[currentTranscriptIdx]
                                }
                                {...{ fallbackElem }}
                            />
                        </div>
                    ...
                    <div className="inner-card-section flex-grow-1 pb-2 mt-1 pb-xl-0">
                        ...
                        <div className="info-body">
                            <GeneOverview
                                currentGeneItem={result}
                                {...{ schemas, fallbackElem, getTipForField }}
                            />
                        </div>
                    </div>
                </div>
                <div className="col-12 col-md-6 d-flex flex-column">
                    <div className="inner-card-section flex-grow-1 pb-2 pb-xl-1">
                        ...
                        <div className="info-body">
                            <ExternalDatabasesSection
                                currentItem={result}
                                {...{ schemas, externalDatabaseFieldnames }}
                            />
                        </div>
                    </div>
...
function TranscriptSelectionDropdown(props) {
    ...
    const geneListOptions = transcripts.map(function (transcript, idx) {
        return (
            <DropdownItem
                ...
                active={idx === currentTranscriptIdx}>
    ...
    return (
        <DropdownButton
            title={dropdownTitleToShow}
            size="sm py-1"
            variant="outline-secondary select-transcript text-truncate"
            onSelect={setTranscriptIdx}
            disabled={geneTranscriptListLen <= 1}
            data-tip={'Select a transcript to view their details'}>
            {geneListOptions}
        </DropdownButton>
    );
}
        ...
        <React.Fragment>
            <div className="row mb-03">
                ...
                <div className="col-12 col-xl-9" id="sv-consequence">
                    {csq_consequence.map((c) => c.display_title).join(', ') ||
                        fallbackElem}
                </div>
            </div>
            <div className="row mb-03">
                <div className="col-12 col-xl-3">
                    <label
                        htmlFor="sv-con-break1"
                        className="mb-0"
                        data-tip={null}>
                        Breakpoint 1:
                        <QuickPopover
                            popID="sv-con-break2-pop"
                            className="p-0 ml-02 icon-sm"
                            tooltip="Click for more information"
                            title={breakpointPopoverTitle}>
                            {breakpointPopoverContent}
                        </QuickPopover>
                    </label>
                </div>
                <div className="col-12 col-xl-9" id="sv-con-break1">
                    {csq_variant_5_prime_location.split('_').join(' ') ||
                        fallbackElem}
                </div>
            </div>
            <div className="row mb-03">
                ...
                <div className="col-12 col-xl-9" id="sv-con-break2">
                    {csq_variant_3_prime_location.split('_').join(' ') ||
                        fallbackElem}
                </div>
            </div>
        </React.Fragment>
...
const breakpointPopoverTitle =
    'Breakpoint locations are determined relative to the transcript based on VEP annotations.';

const breakpointPopoverContent = (
    <div className="popover-content-inner">
        <p>
            Most transcripts have sufficient annotation information to calculate
            where the structural variant starts and ends relative to the 5 prime
            and 3 prime ends of the transcript. Possible values include:
        </p>
        <ul>
            <li>
            ...
            <li>
                <b> Downstream </b>: Breakpoint is located beyond the 3 prime
                UTR.
            </li>
            <li>
                <b> 5 prime UTR </b>: Breakpoint is located within the 5 prime
                UTR.
            </li>
            <li>
                <b> 3 prime UTR </b>: Breakpoint is located within the 3 prime
                UTR.
            </li>
            <li>
                <b> Upstream or 5 prime UTR </b>: Breakpoint is located either
                upstream of or within the 5 prime UTR (insufficient information
                to determine which).
            </li>
            <li>
                <b> 3 prime UTR or Downstream </b>: Breakpoint is located either
                in the 3 prime UTR or downstream of it (insufficient information
                to determine which).
            </li>
            <li>
            ...
            <li>
                <b> Within miRNA </b>: Breakpoint is located within an miRNA
                coding region.
            </li>
            <li>
                <b> Indeterminate </b>: Insufficient information to determine a
                location for the breakpoint relative to the transcript.
            </li>
        </ul>
    </div>
);
