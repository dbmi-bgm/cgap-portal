...
function SvSettingsCheckbox({ label, checked, onChange, disabled, loading=false }){
    ...
    const spinnerClass = loading ? "spinner-border text-secondary ml-1 spinner-border-sm" : "spinner-border text-secondary ml-1 spinner-border-sm d-none"
...
export class SvBrowser extends React.PureComponent {
    ...
    componentDidUpdate(pastProps, pastState){
        ...
        viewconf.views.forEach((view) => {
            ...
            //if(track.type === "sv" && track.options.dataSource === "parliament2"){
            if(track.type === "sv"){
    ...
    updateSvType(event, svType){
        ...
        svViewSettings['showDeletions'] = svType === "del" ? !this.state.svViewSettings.showDeletions : this.state.svViewSettings.showDeletions;
        svViewSettings['showDuplications'] = svType === "dup" ? !this.state.svViewSettings.showDuplications : this.state.svViewSettings.showDuplications;
        svViewSettings['showInsertions'] = svType === "ins" ? !this.state.svViewSettings.showInsertions : this.state.svViewSettings.showInsertions;
        svViewSettings['showInversions'] = svType === "inv" ? !this.state.svViewSettings.showInversions : this.state.svViewSettings.showInversions;
    ...
    updateViewconf(){
        ...
        const { variantStartAbsCoord, variantEndAbsCoord, bamSampleId, samples, svBamVisibility, svVcfVisibility, higlassSvVcf } = this.state;
        ...
        ajax.load(
            ...
            (resp) => {
                // console.log(hgc.api.getViewConfig());
                // console.log(resp.viewconfig);
                // console.log(hgc);
                hgc.api.setViewConfig(resp.viewconfig, true).then((v) => {
                    ...
                    resp.viewconfig.views.forEach((view) => {
                        ...
                        if(track.type === "sv"){
                            ...
                            if(trackObj){
                                // We need to rerender because for some reason the labels in these tracks vanish, when we update the viewconf
