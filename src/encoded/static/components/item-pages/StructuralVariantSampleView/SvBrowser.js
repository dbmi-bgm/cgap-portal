...
function SvSettingsCheckbox({
    ...
    return (
        <Checkbox
            checked={checked}
            onChange={onChange}
            disabled={disabled || checked === null || loading}
...
export class SvBrowser extends React.PureComponent {
        ...
        ajax.load(
            ...
            (resp) => {
                ...
                if (sampleProcessingItem) {
                    const {
                        samples_pedigree: samplesPedigree = null,
    ...
    updateSvType(event, svType) {
        this.setState(function (existingState) {
            ...
            svViewSettings['showDeletions'] =
                svType === 'del' ? !showDeletions : showDeletions;
            svViewSettings['showDuplications'] =
                svType === 'dup' ? !showDuplications : showDuplications;
            svViewSettings['showInsertions'] =
                svType === 'ins' ? !showInsertions : showInsertions;
            svViewSettings['showInversions'] =
                svType === 'inv' ? !showInversions : showInversions;
    ...
    updateSvLength(event, minMax) {
        ...
        this.setState(function (existingState) {
            ...
            if (minMax === 'min') {
                ...
                svViewSettings['maxVariantLength'] = Math.max(
                    parsed,
                    svViewSettings['minVariantLength']
                );
            }
    ...
    updateBamVisibility(accession) {
        this.setState(function ({
            svBamVisibility: existingVisibility,
            svBamLoadingStatus: existingLoadingStatus,
        }) {
            const svBamVisibility = {
                ...existingVisibility,
                [accession]: !existingVisibility[accession],
            };
            const svBamLoadingStatus = {
                ...existingLoadingStatus,
                [accession]: true,
            };
    ...
    updateVcfVisibility(accession) {
        this.setState(function ({
            svVcfVisibility: existingVisibility,
            svVcfLoadingStatus: existingLoadingStatus,
        }) {
            const svVcfVisibility = {
                ...existingVisibility,
                [accession]: !existingVisibility[accession],
    ...
    resetLoadingStatus() {
        this.setState(function ({
            svVcfLoadingStatus: existingVcfLoadingStatus,
            svBamLoadingStatus: existingBamLoadingStatus,
        }) {
            ...
            (resp) => {
                hgc.api.setViewConfig(resp.viewconfig, true).then((v) => {
                    ...
                    resp.viewconfig.views.forEach((view) => {
                        view.tracks.top.forEach((track) => {
                            if (track.type === 'sv') {
                                const trackObj = hgc.api.getTrackObject(
                                    viewId,
                                    track.uid
                                );
                                if (trackObj) {
        ...
        if (hasError) {
            return (
                <div className="text-center my-5">
                    We had trouble loading the required data.
                </div>
            );
        }
        ...
        samples.forEach((sample) => {
            ...
            bamCheckboxes.push(
                <SvSettingsCheckbox
                    key={accession}
                    label={label}
                    loading={svBamLoadingStatus[accession]}
                    checked={svBamVisibility[accession]}
                    onChange={(e) => this.updateBamVisibility(accession)}
                />
            );
            vcfCheckboxes.push(
                <SvSettingsCheckbox
                    key={accession}
                    label={label}
                    loading={svVcfLoadingStatus[accession]}
                    checked={svVcfVisibility[accession]}
                    onChange={(e) => this.updateVcfVisibility(accession)}
                />
            );
        });
        vcfCheckboxes.push(
            <SvSettingsCheckbox
                key="vcf"
                label="gnomAD-SV"
                checked={svVcfVisibility['gnomad-sv']}
                onChange={(e) => this.updateVcfVisibility('gnomad-sv')}
            />
        );
