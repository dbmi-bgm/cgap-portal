        ...
        const vsFetchCallback = (resp) => {
            ...
            if (!atID) {
                Alerts.queue({
                    ...
                    style: 'warning',
                    message:
                        'Could not retrieve the most recent version of this variant and its notes due to a server error. Please try refreshing the page in a few minutes.',
                });
            }

            this.setState({
                newVSLoading: false,
                newestVariantSample: resp[0],
            });
        };
        ...
        return (
            <div>
                ...
                <div className="container-wide bg-light py-3 mh-inner-tab-height-full">
                    <StructuralVariantSampleOverview
                        {...this.props}
                        newContext={newestVariantSample}
                        {...{ newVSLoading }}
                    />
                </div>
            </div>
