...
export const metadata = {
    ...
    BP2: {
        ...
        order: 3,
        description:
            "Observed in <span class='font-italic'>trans</span> with a pathogenic variant for a fully penetrant dominant gene/disorder or observed in <span class='font-italic'>cis</span> with a pathogenic variant in any inheritance pattern",
    },
    BP3: {
        type: 'benign',
        strength: 'Supporting',
    ...
    BP4: {
        ...
        order: 3,
        description:
            'Multiple lines of computational evidence suggest no impact on gene or gene product (conservation, evolutionary, splicing impact, etc.)<br/><br/><ul><li>Caveat: Because many in silico algorithms use the same or very similar input for their predictions, each algorithm cannot be counted as an independent criterion. BP4 can be used only once in any evaluation of a variant.</li></ul>',
    },
    BP5: {
        type: 'benign',
        strength: 'Supporting',
    ...
    PP1: {
        ...
        order: 4,
        description:
            'Cosegregation with disease in multiple affected family members in a gene definitively known to cause the disease<br/><br/><ul><li>Note: May be used as stronger evidence with increasing segregation data</li></ul>',
    },
    PP2: {
        type: 'pathogenic',
        strength: 'Supporting',
    ...
    PP3: {
        ...
        order: 4,
        description:
            'Multiple lines of computational evidence support a deleterious effect on the gene or gene product (conservation, evolutionary, splicing impact, etc.)<br/><ul><li>Caveat: Because many in-silico algorithms use the same or very similar input for their predictions, each algorithm should not be counted as an independent criterion. PP3 can be used only once in any evaluation of a variant.</li></ul>',
    },
    PP4: {
        type: 'pathogenic',
        strength: 'Supporting',
    ...
    PM2: {
        ...
        order: 5,
        description:
            'Absent from controls (or at extremely low frequency if recessive) (Table 6) in Exome Sequencing Project, 1000 Genomes Project, or Exome Aggregation Consortium<br/><ul><li>Caveat: Population data for insertions/deletions may be poorly called by next-generation sequencing</li></ul>',
    },
    PM3: {
        type: 'pathogenic',
        strength: 'Moderate',
        order: 5,
        description:
            "For recessive disorders, detected in <span class='font-italic'>trans</span> with a pathogenic variant<br/><br/><em>Note: This requires testing of parents (or offspring) to determine phase.</em>",
    },
    PM4: {
        type: 'pathogenic',
        strength: 'Moderate',
    ...
    PS1: {
        ...
        order: 6,
        description:
            'Same amino acid change as a previously established pathogenic variant regardless of nucleotide change<br/><ul><li>Example: Val→Leu caused by either G>C or G>T in the same codon</li><li>Caveat: Beware of changes that impact splicing rather than at the amino acid/protein level</li></ul>',
    },
    PS2: {
        type: 'pathogenic',
        strength: 'Strong',
        order: 6,
        description:
            'De novo (<u>both</u> maternity and paternity confirmed) in a patient with the disease and no family history<br/><br/><em>Note: Confirmation of paternity only is insufficient. Egg donation, surrogate motherhood, errors in embryo transfer, and so on, can contribute to nonmaternity.</em>',
    },
    PS3: {
        type: 'pathogenic',
        strength: 'Strong',
        order: 6,
        description:
            'Well-established in vitro or in vivo functional studies supportive of a damaging effect on the gene or gene product<br/><br/><em>Note: Functional studies that have been validated and shown to be reproducible and robust in a clinical diagnostic laboratory setting are considered the most well established.</em>',
    },
    PS4: {
        type: 'pathogenic',
        strength: 'Strong',
...
export function criteriaArrayToStateMap(arr) {
    ...
    arr.forEach((criteria) => {
        const { acmg_rule_name: rule, rule_strength: strength } =
            criteria || {};
...
export class AutoClassify {
    ...
    static isPathogenic(vstrong, strong, moderate, supporting) {
        ...
        if (strong === 1) {
            ...
            if (
                moderate >= 3 || //      a) ≥3 Moderate (PM1–PM6) OR
    ...
    static isLikelyPathogenic(vstrong, strong, moderate, supporting) {
        if (
            (vstrong === 1 && moderate === 1) || // (i) 1 Very strong (PVS1) AND 1 moderate (PM1–PM6) OR
    ...
    static isBenign(standalone, strong) {
        if (standalone || strong >= 2) {
            // (i) 1 Stand-alone (BA1) OR (ii) ≥2 Strong (BS1–BS4)
            return true;
        }
        return false;
    }
    ...
    static isLikelyBenign(strong, supporting) {
        if (
            (strong === 1 && supporting >= 1) || // (i) 1 Strong (BS1–BS4) and 1 supporting (BP1–BP7) OR
    ...
    constructor(invoked) {
        ...
        this.initializeEvidenceFromInvoked =
            this.initializeEvidenceFromInvoked.bind(this);
    ...
    initializeEvidenceFromInvoked(invoked) {
        ...
        invokedFlat.forEach((invoked) => {
            ...
            const selectedStrength =
                strength && strength !== 'Default' ? strength : defaultStrength;

            if (type === 'pathogenic') {
                if (
                    this.evidenceOfPathogenicity[selectedStrength] === undefined
                ) {
                ...
                if (
                    this.evidenceOfBenignImpact[selectedStrength] === undefined
                ) {
    ...
    classify() {
        ...
        let isPathogenic;
        if (isBenign) {
            isPathogenic = this.memoized.isPathogenic(
                vstrong,
                strongp,
                moderatep,
                supportingp
        ...
        const isLikelyBenign = this.memoized.isLikelyBenign(
            strongb,
            supportingb
        );
        let isLikelyPathogenic;
        if (isLikelyBenign) {
            isLikelyPathogenic = this.memoized.isLikelyPathogenic(
                vstrong,
                strongp,
                moderatep,
                supportingp
