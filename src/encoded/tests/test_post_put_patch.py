...
def test_patch_delete_fields_restriced_fields_admin(link_targets, testapp):
    res = testapp.post_json(COLLECTION_URL, item_with_link[0], status=201)
    url = res.location
...
def test_patch_delete_fields_restriced_fields_submitter(content, testapp, submitter_testapp):
    """
    ...
    assert res_errors[0]['description'] == "permission 'restriced_fields' required"
