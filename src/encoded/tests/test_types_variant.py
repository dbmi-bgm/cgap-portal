...
def make_transcript(exon=None, intron=None, distance=None, consequences=None,
        most_severe=False):
    """"""
    result = {}
    if exon:
...
@pytest.fixture
def location_consequences(testapp, project, institution):
    """"""
    ...
    for consequence in LOCATION_CONSEQUENCES:
        ...
        post = testapp.post_json("/variant_consequence", consequence,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([make_transcript(exon="2/6", most_severe=True,
            consequences=[UTR_5_CONSEQUENCE])], "Exon 2/6 (5' UTR)"),
        ([make_transcript(exon="2/6", most_severe=True,
            consequences=[UTR_3_CONSEQUENCE])], "Exon 2/6 (3' UTR)"),
        ([make_transcript(distance="1000")], None),
        ([make_transcript(distance="1000", most_severe=True)], "1000 bp"),
        ([make_transcript(distance="1000", most_severe=True,
            consequences=[UPSTREAM_CONSEQUENCE])], "1000 bp upstream"),
        ([make_transcript(distance="1000", most_severe=True,
            consequences=[DOWNSTREAM_CONSEQUENCE])], "1000 bp downstream"),
    ]
)
def test_most_severe_location(transcripts, expected, testapp, variant,
        location_consequences):
    """"""
    for transcript in transcripts:
        ...
        if consequences:
            for idx, consequence in enumerate(consequences):
                consequences[idx] = location_consequences[consequence]
    variant_atid = variant.get("@id")
    patch_body = {"transcript": transcripts}
    patch_response = testapp.patch_json(variant_atid, patch_body,
