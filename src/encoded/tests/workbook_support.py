...
from dcicutils.misc_utils import environ_bool, PRINT, camel_case_to_snake_case, check_true
...
class ElasticSearchDataCache:
    ...
    def assure_data_once_loaded(cls, es_testapp, datadir, indexer_namespace,
        ...
        """
        if DEBUG_WORKBOOK_CACHE:
            PRINT(level * "  ", level, "Entering %s.assure_data_once_loaded at %s" % (cls.__name__, datetime.datetime.now()))
        ...
        if DEBUG_WORKBOOK_CACHE:
            PRINT(level * "  ", level, "Exiting %s.assure_data_once_loaded at %s" % (cls.__name__, datetime.datetime.now()))
