...
from .. import (
    batch_download as batch_download_module,
    batch_download_utils as batch_download_utils_module,
)
...
EXPECTED_VARIANT_SAMPLE_SPREADSHEET_COLUMNS = [
    (
        "# URL path to the variant",
        "ID",
        "/variant-samples/e43be20e-5dda-4db2-bb23-8c103323fc0f/",
        "/variant-samples/d62836a0-2de3-4970-bf14-eba3ca758a82/",
    ),
    ...
    (
        ...
        "HGVSG",
        "NC_000001.11:g.2030666G>A",
        "NC_000016.10:g.23630011_23630012insTTA",
    ),
    (
        "HGVS cPos nomenclature",
        "HGVSC",
        "ENST00000378585.7:c.*384G>A",
        "ENST00000261584.9:c.2142_2143insTAA",
    ),
    (
        "HGVS pPos nomenclature",
        "HGVSP",
        "ENST00000378585.7:p.V>A",
        "ENSP00000261584.4:p.Asp714_Asp715insTer",
    ),
    ...
    (
        ...
        "This variant has been reported in the ClinVar database as Unknown Significance.",
        "",
    ),
    (
    ...
    (
        ...
        "This variant has been reported in the ClinVar database as Pathogenic.",
        "",
    ),
    (
    ...
    (
        ...
        "This gene is not a real discovery...",
        "",
    ),
    (
...
@contextmanager
def patch_variant_sample_spreadsheet_column_tuples(
    **kwargs,
...
@contextmanager
def patch_variant_sample_spreadsheet_evaluate_item(
    **kwargs,
) -> Iterator[mock.MagicMock]:
    with patch_context(
...
@contextmanager
def patch_variant_sample_spreadsheet_get_header_lines(
    **kwargs,
) -> Iterator[mock.MagicMock]:
    with patch_context(
...
@contextmanager
def patch_variant_sample_spreadsheet_get_note_by_subrequest(
    **kwargs,
) -> Iterator[mock.MagicMock]:
    with patch_context(
...
@contextmanager
def patch_variant_sample_spreadsheet_get_note_properties(
    **kwargs,
) -> Iterator[mock.MagicMock]:
    with patch_context(
...
def patch_get_values_for_field(**kwargs) -> Iterator[mock.MagicMock]:
    with patch_context(
        batch_download_module.get_values_for_field, **kwargs
...
def test_validate_spreadsheet_search_parameters(
    compound_search: str, expected_exception: bool
) -> None:
...
@pytest.mark.workbook
def test_case_search_spreadsheet(
    html_es_testapp: TestApp, es_testapp: TestApp, workbook: None
) -> None:
...
class TestVariantSampleSpreadsheet:
    ...
    def test_get_headers(
        self, header_lines: List[List[str]], expected: List[List[str]]
    ) -> None:
        with patch_variant_sample_spreadsheet_get_header_lines(
            return_value=header_lines
        ):
    ...
    def test_get_available_header_lines(self) -> None:
        ...
        with patch_spreadsheet_request_case_accession(return_value=case_accession):
            with patch_spreadsheet_request_case_title(return_value=case_title):
                with patch_spreadsheet_request_compound_search(
                    return_value=compound_search
                ):
        ...
        with patch_variant_sample_spreadsheet_columns(
            ...
            with patch_variant_sample_spreadsheet_evaluate_item() as mock_evaluate_item_with_column:
                with patch_variant_sample_spreadsheet_add_embeds() as mock_add_embeds:
                    with patch_variant_sample(return_value=self.SOME_VARIANT_SAMPLE):
                        spreadsheet = self.get_variant_sample_spreadsheet()
                        result_generator = spreadsheet._get_row_for_item(
                            self.SOME_VARIANT_SAMPLE_PROPERTIES
                        )
                        result = list(result_generator)
                        assert len(result) == expected_result_count
                        assert (
                            len(mock_evaluate_item_with_column.call_args_list)
                            == expected_result_count
                        )
                        mock_add_embeds.assert_called_once_with(
                            self.SOME_VARIANT_SAMPLE_PROPERTIES
                        )
    ...
    def test_evaluate_item_with_column(
        self,
        is_property_evaluator: bool,
        is_callable_evaluator: bool,
        expected_exception: bool,
        expected_field_for_item_call: Any,
    ) -> None:
        spreadsheet = self.get_variant_sample_spreadsheet()
        ...
        if expected_exception:
            ...
            result = spreadsheet._evaluate_item_with_column(
                column, self.SOME_VARIANT_SAMPLE
            )
            assert result == column.get_field_for_item.return_value
            column.get_field_for_item.assert_called_once_with(
                expected_field_for_item_call
            )
    ...
    def test_get_canonical_transcript_consequence_names(self) -> None:
        ...
        assert (
            result
            == variant_sample.get_canonical_transcript_consequence_names.return_value
        )
    ...
    def test_get_most_severe_transcript_consequence_names(self) -> None:
        spreadsheet = self.get_variant_sample_spreadsheet()
        variant_sample = mock_variant_sample()
        result = spreadsheet._get_most_severe_transcript_consequence_names(
            variant_sample
        )
        assert (
            result
            == variant_sample.get_most_severe_transcript_consequence_names.return_value
        )
    ...
    def test_get_note_of_same_project(self) -> None:
        ...
        with patch_variant_sample_spreadsheet_get_note_properties() as mock_get_note_properties:
            result = spreadsheet._get_note_of_same_project(
                property_location, property_to_retrieve
            )
