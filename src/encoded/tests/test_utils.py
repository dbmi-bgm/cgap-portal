...
def test_ignorable():
    assert ignorable() == True
    assert ignorable(3, "foo") == True
    x = 3
    assert ignorable("the x on the previous line is unused, but for this statement", x) == True
