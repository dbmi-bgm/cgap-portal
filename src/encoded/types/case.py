...
class Case(Item):
    ...
    @calculated_property(schema={
        ...
        "description": "Additional facets used to help with search",
        "type": "array",
        "items": {
            "title": "Additional Facet",
            "type": "string"
        }
    })
    def additional_variant_sample_facets(self, request, family=None):
        ret_set = set()
        ...
        proband_id = family_info.get("proband")
        mother_id = family_info.get("mother")
        father_id = family_info.get("father")

        if proband_id:
            ret_set.add("proband_genotype_label")
        if mother_id:
            ret_set.add("mother_genotype_label")
        if father_id:
            ret_set.add("father_genotype_label")

        for member_id in family_info.get("members", []):
            # Is there more performant way to grab all family members instead of 1-by-1 requests?
            # Redis soon hopefully :-D.
            member_info = get_item_or_none(request, member_id, 'individual')
            member_sex = member_info.get("sex", "U")

            if proband_id and (member_info.get("mother") == proband_id or member_info.get("father") == proband_id):
                # This family member is child of proband

                if member_sex == "F":
                    if "daughter_genotype_label" in ret_set:
                        ret_set.add("daughter_II_genotype_label")
                    else:
                        ret_set.add("daughter_genotype_label")

                elif member_sex == "M":
                    if "son_genotype_label" in ret_set:
                        ret_set.add("son_II_genotype_label")
                    else:
                        ret_set.add("son_genotype_label")

                else:
                    ret_set.add("child_genotype_label") # ??

            if mother_id and father_id and member_info.get("mother") == mother_id and member_info.get("father") == father_id:
                # This family member is sibling of proband

                if member_sex == "F":
                    ret_set.add("sister_genotype_label")
                elif member_sex == "M":
                    ret_set.add("brother_genotype_label")
                else:
                    ret_set.add("sibling_genotype_label") # ??

            ## idk how to figure out 'co-parent' ...

        return [ "associated_genotype_labels." + af for af in ret_set ]
