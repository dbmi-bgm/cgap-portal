...
Cypress.Commands.add(
    'loginCGAP',
    function (options = { useEnvToken: false, email: null }) {
        ...
        const email =
            options.email ||
            options.user ||
            'cypress-test-user@cgap.hms.harvard.edu';
        const auth0client = Cypress.env('Auth0Client');
        const auth0secret = Cypress.env('Auth0Secret');

        if (!auth0client || !auth0secret)
            throw new Error(
                'Cannot test login if no Auth0Client & Auth0Secret in ENV vars.'
            );
...
Cypress.Commands.add('logoutCGAP', function (options = { useEnvToken: true }) {
    cy.get('#user_account_nav_button')
        .click()
        .wait(100)
        .end()
        .get('#logoutbtn')
