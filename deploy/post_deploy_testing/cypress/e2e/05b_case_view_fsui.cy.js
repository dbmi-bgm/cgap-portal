...
function onSupersededAjaxRequestHandler(err, runnable) {
    ...
    if (
        err.message.includes(
            'Did not get back a search response, request was potentially aborted.'
        )
    ) {
        Cypress.log({
...
describe('Case View - FSUI', function () {
    ...
    it('We should be on right tab upon page initialization/login', function () {
        cy.visit('/cases/GAPCAJQ1L99X/#case-info.filtering', {
            ...
            .end()
            .get(
                '#case-info\\.filtering #snv-filtering .above-variantsample-table-ui .filter-set-ui-header h4'
            )
            .should('not.have.text', 'Loading Filter Set')
            .end()
            .get(
                '#case-info\\.filtering #snv-filtering .above-variantsample-table-ui .blocks-container .filterset-block'
            )
            .should('have.length.greaterThan', 0)
            .then(function ($fsBlocks) {
    ...
    it('Save buttons disabled initially', function () {
        cy.get(
            'div#case-info\\.filtering #snv-filtering .above-variantsample-table-ui .filter-set-ui-header div.pl-16 button:first-child'
        )
            .should(function ($btn) {
                expect($btn.children('span')).to.have.text(
                    'Save Case FilterSet'
                );
            ...
            .end()
            .get(
                'div#case-info\\.filtering #snv-filtering .above-variantsample-table-ui .filter-set-ui-header div.pl-16 button:last-child'
            )
            .should(function ($btn) {
    ...
    it('Add Filter Block button creates new empty block', function () {
        ...
        cy.once('uncaught:exception', onSupersededAjaxRequestHandler)
            .end()
            .get('#case-info\\.filtering #snv-filtering')
            .within(function () {
                cy.get(
                    '.above-variantsample-table-ui div[aria-label="Creation Controls"] button:first-child'
                )
                    .should('have.text', 'Add Filter Block')
                    .click()
                    .end()
                    .get(
                        '.above-variantsample-table-ui .filterset-blocks-container'
                    )
                    .should('have.attr', 'data-all-selected', 'false')
                    .end()
                    .get(
                        '.above-variantsample-table-ui .filterset-blocks-container .blocks-container .filterset-block'
                    )
                    .should('have.length', countFBInitial + 1)
                    .eq(-1)
                    ...
                    .within(function ($fb) {
                        ...
                        return cy
                            .get('i.icon')
                            .should('have.class', 'icon-times-circle');
                    })
                    .contains('No Filters Selected')
                    .end();
            });
    });
    ...
    it('Selecting term from FacetList adds to the new filter block', function () {
        cy.once('uncaught:exception', onSupersededAjaxRequestHandler)
            ...
            .within(function () {
                ...
                cy.get(
                    '.facets-column > .facets-container > .facets-body div.facet[data-field="associated_genotype_labels.proband_genotype_label"] .facet-title'
                )
                    .click()
                    .get(
                        '.facets-column > .facets-container > .facets-body div.facet[data-field="associated_genotype_labels.proband_genotype_label"] .facet-list-element[data-key="Heterozygous"]'
                    )
                    ...
                    .end()
                    .get(
                        '.filterset-blocks-container .blocks-container .filterset-block:last-child .field-block'
                    )
                    .should('have.length', 1)
                    .within(function ($fb) {
                        cy.get('.field-name')
                            .should('have.text', 'Proband Genotype')
                            .end()
